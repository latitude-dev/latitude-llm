...
- Header comments for that file (including any `eslint-disable`) come from `apps/web/tsr.config.json` (`routeTreeFileHeader`); regenerate via dev/build instead of hand-editing the `.gen.ts` file
