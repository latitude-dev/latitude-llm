...
- Header comments for that file (including any `eslint-disable`) come from `apps/web/tsr.config.json` (`routeTreeFileHeader`); regenerate via dev/build instead of hand-editing the `.gen.ts` file. Do not put the substring `*/` inside that block comment (except the closing `*/`) or TanStack Router’s generator will fail to parse the file.
...
## ESLint directives (rare)

Linting in-repo is Biome (`biome.json`). If an `eslint-disable` comment is needed (e.g. third-party patterns, IDE ESLint), include a **short justification** after the directive: `// eslint-disable-next-line <rule> -- <why>` or `/* eslint-disable -- <why> */`. Prefer removing the disable by fixing the code when that is straightforward.
