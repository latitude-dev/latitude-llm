...
- Production deploys use the single-branch release-tag flow: `development` is trunk and deploys to staging by default; production is triggered only by pushing a monorepo release tag (`vX.Y.Z`) created from a reviewed root `CHANGELOG.md` release PR. Use `scripts/release.sh [version]` to open that PR; without a version it bumps the latest merged `vX.Y.Z` tag to the next minor version. Do not promote by merging `development` into `main`.
