...
const BASE_URL =
  'http://localhost:8787/api/v3/projects/11/versions/92dd6684-26d0-464e-acfd-3297199bd5a0/documents/run'
...
const makeRequest = async (index) => {
  try {
    ...
    successfulRequests++
    console.log(
      `Request ${index + 1}/${NUM_REQUESTS} completed successfully. Total completed: ${completedRequests}`,
    )
    return fullResponse
  } catch (error) {
...
const runLoadTest = async () => {
  ...
  const requests = Array(NUM_REQUESTS)
    .fill()
    .map((_, index) => limit(() => makeRequest(index)))
  ...
  console.log(
    `Success Rate: ${((successfulRequests / NUM_REQUESTS) * 100).toFixed(2)}%`,
  )
