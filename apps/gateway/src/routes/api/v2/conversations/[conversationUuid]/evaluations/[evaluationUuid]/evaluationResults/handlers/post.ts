...
const postEvaluationResultSchema = z.object({
  result: z.number().or(z.string()).or(z.boolean()),
  reason: z.string().optional().default(''),
})

export const postHandler = factory.createHandlers(
  zValidator('json', postEvaluationResultSchema),
  async (c) => {
    const { conversationUuid, evaluationUuid } = c.req.param()
