...
import type { IntegrationLegacyWebhookRoute, IntegrationWebhookRoute } from './webhook.route'
...
async function resolveWorkspaceAndCommit(
  ...
  if (!rawTrigger) {
    return Result.error(new NotFoundError(`Trigger not found with uuid '${triggerUuid}'`))
  }
  const workspace = await unsafelyFindWorkspace(rawTrigger.workspaceId)
  if (!workspace) {
    return Result.error(
      new NotFoundError(`Workspace not found with id '${rawTrigger.workspaceId}'`),
    )
  }
  ...
  if (!commit) {
    return Result.error(
      new NotFoundError(`Head commit not found for project '${rawTrigger.projectId}'`),
    )
  }
