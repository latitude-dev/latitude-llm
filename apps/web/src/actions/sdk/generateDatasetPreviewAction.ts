...
export async function generateDatasetPreviewAction({
  ...
  if (!env.DATASET_GENERATOR_PROMPT_PATH) {
    throw new BadRequestError('DATASET_GENERATOR_PROMPT_PATH is not set')
  }
  ...
  const response = await sdk.prompts.run(env.DATASET_GENERATOR_PROMPT_PATH, {
    stream: true,
    parameters: {
      row_count: 10,
      parameters,
