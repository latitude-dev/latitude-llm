...
describe('updateWebhookAction', () => {
  ...
  describe('unauthorized', () => {
    it('errors when the user is not authenticated', async () => {
      ...
      const { serverError } = await updateWebhookAction({
        id: webhook.id,
  ...
  describe('authorized', () => {
    ...
    it('successfully updates a webhook name', async () => {
      const { data, serverError, validationErrors } = await updateWebhookAction(
        {
          id: webhook.id,
          name: 'Updated Webhook',
        },
    ...
    it('rejects invalid URL format', async () => {
      const { validationErrors } = await updateWebhookAction({
        id: webhook.id,
        url: 'not-a-url',
      })
