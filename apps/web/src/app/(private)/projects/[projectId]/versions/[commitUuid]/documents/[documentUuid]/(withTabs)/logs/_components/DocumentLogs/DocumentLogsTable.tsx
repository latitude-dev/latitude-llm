...
function EvaluationsColumn({
  ...
  return (
    <div className='flex justify-center items-center gap-2 shrink-0'>
      ...
      {pendingResults > 0 && <Badge variant='muted'>{pendingResults} pending</Badge>}
    </div>
  )
}
...
export const DocumentLogsTable = forwardRef<HTMLTableElement, Props>(function DocumentLogsTable(
  {
    documentLogs,
    pagination,
    evaluationResults,
    ...
    setSelectedLog,
    isLoading,
    selectableState: { headerState, selectedCount, isSelected, toggleRow, toggleAll },
    limitedView,
    limitedCursor,
    setLimitedCursor,
    onSelectedSpan,
  ...
  const queryParams = typeof window !== 'undefined' ? window.location.search : undefined
        ...
        {documentLogs.map((documentLog) => {
          ...
          return (
            <Fragment key={documentLog.uuid}>
              <TableRow
                onClick={() =>
                  setSelectedLog(selectedLog?.uuid === documentLog.uuid ? undefined : documentLog)
                }
                className={cn('cursor-pointer border-b-[0.5px] h-12 max-h-12 border-border', {
              ...
              >
                <TableCell
                  preventDefault
                  align='left'
                  onClick={() => toggleRow(documentLog.id, !isSelected(documentLog.id))}
                >
                  <Checkbox
                    fullWidth={false}
                    disabled={!!error}
                ...
                <TableCell>
                  <span className='flex flex-row gap-2 items-center truncate'>
                    ...
                    >
                      <Text.H6 noWrap>
                        {documentLog.commit.version ? `v${documentLog.commit.version}` : 'Draft'}
                      </Text.H6>
                    </Badge>
                ...
                <TableCell>
                  <Text.H5 color={cellColor}>{capitalize(documentLog.source || '-')}</Text.H5>
                </TableCell>
                <TableCell>
                  <EvaluationsColumn
                    color={cellColor}
                ...
                <TableCell>
                  <Text.H5 noWrap color={cellColor}>
                    {typeof documentLog.tokens === 'number' ? documentLog.tokens : '-'}
                  </Text.H5>
                </TableCell>
                <TableCell>
                  <Text.H5 noWrap color={cellColor}>
              ...
              {tracingEnabled && selectedLog?.uuid === documentLog.uuid && (
                <TableRow hoverable={false}>
                  ...
                  >
                    <DocumentLogTraces documentLog={documentLog} onSelectedSpan={onSelectedSpan} />
                  </TableCell>
                </TableRow>
              )}
            </Fragment>
