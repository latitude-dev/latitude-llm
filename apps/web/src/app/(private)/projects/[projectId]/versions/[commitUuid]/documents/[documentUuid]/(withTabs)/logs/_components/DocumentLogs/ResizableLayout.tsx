...
export function ResizableLayout({
  ...
  return (
    <div ref={containerRef} className='flex flex-row h-full w-full gap-4'>
      {/* Left pane - grows to fill remaining space when right pane is hidden */}
      <div
        className={
          shouldShowRightPane
      ...
      {/* Resizable right pane - only render when should be shown */}
      {shouldShowRightPane && (
        <ResizableBox
          ...
          onResize={(_e, data) => {
            // Update width during resize for smooth experience
            setRightPaneWidth(data.size.width)
          }}
          onResizeStop={(_e, data) => {
            // Final callback when resize ends
            setRightPaneWidth(data.size.width)
          }}
          resizeHandles={['w']} // Only allow resizing from the west (left) side
          handle={SplitHandle(true)} // Use SplitHandle with visible handle
