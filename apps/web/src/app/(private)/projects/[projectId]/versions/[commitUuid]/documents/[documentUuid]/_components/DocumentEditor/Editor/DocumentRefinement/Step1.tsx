...
export function Step1({
  ...
  // biome-ignore lint: FIXME: Do not use useEffect to set state.
  useEffect(() => {
    if (selectableState.selectedRowIds.length) {
      setSelectedEvaluation(evaluations.find((e) => e.uuid === selectableState.selectedRowIds[0]))
    } else {
  ...
  if (isLoading) {
    return <TableSkeleton rows={7} cols={['Name', 'Description', 'Average score']} />
  }
          ...
          {evaluations.map((evaluation) => (
            <TableRow
              ...
              className={cn(
                ...
                {
                  'bg-secondary hover:bg-secondary/50': selectableState.isSelected(evaluation.uuid),
                },
              )}
            >
              <TableCell align='left'>
                <Checkbox fullWidth={false} checked={selectableState.isSelected(evaluation.uuid)} />
              </TableCell>
              <TableCell>
...
function AverageScoreBadgeV2<T extends EvaluationType, M extends EvaluationMetric<T>>({
  project,
  commit,
  document,
  evaluation,
