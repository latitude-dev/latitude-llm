...
vi.mock(
  '@latitude-data/core/services/billing/createCustomerPortalSession',
  () => ({
...
describe('GET /api/pricings/[slug]', () => {
  ...
  describe('customer portal', () => {
    it('should redirect to customer portal for legacy paid plan with stripe customer', async () => {
      const portalUrl = 'https://billing.stripe.com/portal/session/123'
      const workspaceWithStripe = {
        ...mockWorkspace,
      ...
      mocks.getDataFromSession.mockResolvedValue({
        user: mockUser,
        workspace: workspaceWithStripe,
      })
      mocks.createCustomerPortalSession.mockResolvedValue(
        Result.ok({ url: portalUrl }),
      )
      ...
      expect(response.status).toBe(307)
      expect(response.headers.get('location')).toBe(portalUrl)
      expect(mocks.createCustomerPortalSession).toHaveBeenCalledWith({
        stripeCustomerId: 'cus_123',
        returnUrl: expect.stringContaining('checkout=customer_portal'),
        workspaceId: workspaceWithStripe.id,
        userEmail: mockUser.email,
      })
    })

    it('should redirect to customer portal for ProV2 plan with stripe customer', async () => {
      const portalUrl = 'https://billing.stripe.com/portal/session/456'
      const workspaceWithStripe = {
        ...mockWorkspace,
    ...
    it('should redirect to customer portal for TeamV1 plan with stripe customer', async () => {
      const portalUrl = 'https://billing.stripe.com/portal/session/789'
      const workspaceWithStripe = {
        ...mockWorkspace,
  ...
  describe('plan not available', () => {
    ...
    it('should return 400 when legacy plan user has no stripe customer id', async () => {
      mocks.getDataFromSession.mockResolvedValue({
        user: mockUser,
        workspace: {
          ...mockWorkspace,
