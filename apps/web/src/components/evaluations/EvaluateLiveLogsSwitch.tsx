...
import type { EvaluationMetric, EvaluationType, EvaluationV2 } from '@latitude-data/constants'
...
>({ evaluation, disabled }: { evaluation: EvaluationV2<T, M>; disabled?: boolean }) {
  ...
  const isDisabled =
    disabled || isUpdatingEvaluation || !metricSpecification?.supportsLiveEvaluation
  ...
  if (!metricSpecification?.supportsLiveEvaluation) {
    return (
      <Tooltip
        ...
        side='top'
      >
        {typeSpecification?.name} / {metricSpecification?.name} evaluations do not support live
        evaluation
      </Tooltip>
    )
  }
