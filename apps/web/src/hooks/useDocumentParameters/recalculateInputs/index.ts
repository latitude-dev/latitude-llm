...
import type { Inputs, InputSource } from '@latitude-data/core/browser'
...
export function recalculateInputs<S extends InputSource>({
  ...
  const config = (prompt.config.parameters || {}) as Record<string, { type?: string }>
  ...
  const firstChangedInput = Object.entries(inputs).find(([key]) => !prompt.parameters.has(key))?.[1]

  return Object.fromEntries(
    Array.from(prompt.parameters).map((param) => {
      const input = inputs[param] ?? fallbackInputs?.[param] ?? firstChangedInput
