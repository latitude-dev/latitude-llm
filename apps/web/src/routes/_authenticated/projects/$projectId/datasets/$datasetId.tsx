...
const rowColumns: InfiniteTableColumn<DatasetRowRecord>[] = [
  ...
  },
  {
    ...
    header: "Created",
    sortKey: "createdAt",
    render: (r) => <Text.H6 color="foregroundMuted">{relativeTime(r.createdAt)}</Text.H6>,
  },
  {
    key: "input",
    header: "Input",
    render: (r) => (
      <span className="max-w-48 truncate block">
        <Text.Mono>{formatCellValue(r.input)}</Text.Mono>
      </span>
    ),
  },
  {
    ...
    render: (r) => (
      <span className="max-w-48 truncate block">
        <Text.Mono>{formatCellValue(r.output)}</Text.Mono>
      </span>
    ),
  },
]
