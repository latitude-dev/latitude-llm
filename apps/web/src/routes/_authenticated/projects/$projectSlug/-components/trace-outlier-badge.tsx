...
function formatValue({ value, metric }: { value: number; metric: CohortMetric }): string {
  switch (metric) {
    case "durationNs":
...
function CohortBaseline({
  ...
  // `getMetricPercentileThreshold` so they return null when the cohort
  // is too small (matching `computeLevel`'s behavior).
  const value = level === "p50" ? baseline.p50 : getMetricPercentileThreshold(baseline, level)
  return (
...
export function TraceOutlierBadge({
  ...
  const threshold = getMetricPercentileThreshold(data.baselines[metric], level)
