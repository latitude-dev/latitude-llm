...
import { SpansTab } from "../../-components/trace-detail-drawer/tabs/spans-tab.tsx"
import { TraceTab } from "../../-components/trace-detail-drawer/tabs/trace-tab.tsx"
import { SessionConversationTab } from "./session-conversation-tab.tsx"
...
export type SessionTraceDetailDrawerProps = {
   ...
   * Set false for nested contexts so the parent route URL stays clean.
   */
  ...
  /** Active search query — drives highlights + scroll-to-first inside the Conversation tab. */
  readonly searchQuery?: string
}

export function SessionTraceDetailDrawer({ urlSyncedTabs = true, ...props }: SessionTraceDetailDrawerProps) {
  if (urlSyncedTabs) {
    return <SessionTraceDetailDrawerWithUrlTabs {...props} />
  }
  return <SessionTraceDetailDrawerWithLocalTabs {...props} />
}

function SessionTraceDetailDrawerWithUrlTabs(props: Omit<SessionTraceDetailDrawerProps, "urlSyncedTabs">) {
  ...
  const defaultTab: TabId = props.searchQuery && props.searchQuery.length > 0 ? "conversation" : "trace"
  const [activeTab, setActiveTab] = useParamState("traceDetailTab", defaultTab, {
    validate: isTraceDetailTab,
  })
  const [selectedSpanId, setSelectedSpanId] = useParamState("spanId", "")
  return (
    <SessionTraceDetailDrawerShell
      {...(rest as Omit<
        SessionTraceDetailDrawerProps,
        "urlSyncedTabs" | "initialTab" | "closeLabel" | "drawerStoreKey"
      >)}
      activeTab={activeTab}
...
function SessionTraceDetailDrawerWithLocalTabs(props: Omit<SessionTraceDetailDrawerProps, "urlSyncedTabs">) {
...
function SessionTraceDetailDrawerShell({
  traceId,
  trace,
  projectId,
  filters,
  ...
  searchQuery,
}: Omit<SessionTraceDetailDrawerProps, "urlSyncedTabs" | "initialTab" | "closeLabel" | "drawerStoreKey"> &
