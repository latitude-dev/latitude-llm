import type { PersonalityKind, ReportV3, WrappedReportRecord } from "@domain/spans"
...
interface WrappedReportV3Props {
  readonly record: WrappedReportRecord
  readonly isMember: boolean
}

// ─── Delta utilities (forked from V2 — each report version renders standalone) ─
...
function HeadlineNumbers({
  ...
  readonly totals: ReportV3["totals"]
  readonly lastReport: ReportV3["lastReport"]
}) {
  return (
...
export function WrappedReportV3({ record, isMember }: WrappedReportV3Props) {
  // The dispatcher only routes here when reportVersion === 3; cast is safe.
  const report = record.report as ReportV3
  ...
  return (
    <main className="min-h-screen" style={{ backgroundColor: CREAM }}>
      <div className="mx-auto flex max-w-3xl flex-col gap-12 px-4 py-12 sm:py-20">
        ...
        <SkillsSection skills={report.skills} isMember={isMember} />
