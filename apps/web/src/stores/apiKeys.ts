...
export default function useApiKeys(opts?: SWRConfiguration) {
  ...
  const { execute: create, isPending: isCreating } = useLatitudeAction(createApiKeyAction, {
  ...
  const { execute: destroy, isPending: isDestroying } = useLatitudeAction(destroyApiKeyAction, {
    onSuccess: async ({ data: apiKey }) => {
      toast({
        title: 'Success',
  ...
  const { execute: update, isPending: isUpdating } = useLatitudeAction(updateApiKeyAction, {
    onSuccess: async ({ data: updatedApiKey }) => {
      toast({
        title: 'Success',
        description: `API key "${updatedApiKey.name}" updated successfully`,
      })
      mutate(data.map((item) => (item.id === updatedApiKey.id ? updatedApiKey : item)))
    },
  })
