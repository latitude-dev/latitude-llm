...
| `adaptiveDurationMs` vs `staticDurationMs` | Per-build wall-clock. **Read it against `rootSearchKs`**, not on its own: the deep root re-search deliberately costs a couple of plain adaptive builds, so a re-searched run legitimately lands well above static | `rootSearchKs:0` runs within **~25%** of static; a re-searched run up to ~**3–4×** static and comfortably under `TAXONOMY_CLUSTERING_WORKER_TIMEOUT_MS` (300s) | A `rootSearchKs:0` run ≳ 2× static → perf regression. A `durationMs` sitting at ~300,000 with `fallbackReason:buildError` → the build is being killed by the worker deadline (failed builds now report the time they burned, so a deadline kill is visible as a duration AT the deadline rather than as a zero) |
| `rootSearchKs` | How many K the deep root re-search swept. Zero exactly when `bestRootSeparation` fell below `TAXONOMY_ADAPTIVE_ROOT_SEARCH_MIN_SEPARATION` (no structure to search for) or no root candidate was scored at all — that predicate and nothing else | `TAXONOMY_ADAPTIVE_ROOT_SEARCH_WIDTH` on any project with structure; **0** is legitimate on a project whose root sits under the floor, so read it with `bestRootSeparation` rather than on its own | `rootSearchKs:0` **together with** a `bestRootSeparation` above the floor → the search is being skipped for a reason the floor does not explain. `rootSearchKs` non-zero with `durationMs` near the deadline on the biggest projects → narrow `TAXONOMY_ADAPTIVE_ROOT_SEARCH_WIDTH`, which with the restart budget is what bounds this path |
| `bestRootSeparation` | Best separation any ROOT candidate reached, accepted or not | Above the 0.45 root gate, or well below 0.25 on genuinely unimodal projects | A high value is **not** a stability signal on its own: the two projects whose `rootChildCount` swung worst reached 1.3 and 3.7–42. Read the swing off `rootChildCount` |
...
`adaptiveDurationMs` is within ~25% of static on `rootSearchKs:0` runs (a
re-searched run is expected to be several times static — judge it against the
worker deadline instead), `rootSearchKs` is **non-zero** (the pilot's root separation
sits at 0.41–0.67, well above the floor, so a zero there would be unexplained), and
`peakRssBytes` stays
