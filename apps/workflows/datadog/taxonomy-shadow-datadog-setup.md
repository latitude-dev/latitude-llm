...
| `rootSearchKs` / `rootSearchMembers` | How many K the deep root re-search swept, and how many members each of its restarts saw | `rootSearchKs` = `TAXONOMY_ADAPTIVE_ROOT_SEARCH_WIDTH` on any project with structure, **0** only on unimodal ones; `rootSearchMembers` = `min(observationsSampled, TAXONOMY_ADAPTIVE_ROOT_SEARCH_MEMBER_CAP)` | `rootSearchKs:0` on a project whose `rootChildCount` moves run to run → its root separation is under the floor and the deep search is being skipped. `rootSearchMembers` = `observationsSampled` on the biggest projects with a `durationMs` near the deadline → lower the member cap, which is the bound the deadline is held by |
...
worker deadline instead), `rootSearchKs` is **non-zero**, and `peakRssBytes` stays
