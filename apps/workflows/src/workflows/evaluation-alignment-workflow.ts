...
import {
  ALIGNMENT_FULL_REOPTIMIZE_RATE_LIMIT_MS,
    ...
    switch (payload.reason) {
      ...
      case "debounced-full-realignment":
        if (pendingFullRealignmentAtMs === null) {
          pendingFullRealignmentAtMs = Date.now() + ALIGNMENT_FULL_REOPTIMIZE_RATE_LIMIT_MS
          scheduleRevision += 1
        }
        return
      case "debounced-metric-refresh":
        if (pendingMetricRefreshAtMs === null) {
          pendingMetricRefreshAtMs = Date.now() + ALIGNMENT_METRIC_RECOMPUTE_RATE_LIMIT_MS
          scheduleRevision += 1
        }
