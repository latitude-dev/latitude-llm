...
- live evaluations: `packages/domain/evaluations/src/use-cases/live/run-live-evaluation.ts` — every scan records the baseline `eval-scan` credit after execution; scripts with `llm()` or `classify()` authorize one `llm-call` (the larger estimate). Each LLM generation, Jev classification, and query embed is then metered under a `live-eval` scope
