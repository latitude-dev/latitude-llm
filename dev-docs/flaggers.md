...
User/input-centric strategies (`classifiesAssistantResponseOnly: false` — today `frustration`, `jailbreaking`, and `nsfw`) are dropped when nested-sample provenance is present (`isUserCentricReflagInapplicable`): first-level flagger-generated sessions (`flagger:classify` / `flagger:draft` tags), and taxonomy dogfood traces that carry both a taxonomy tag (`taxonomy:propose-themes`, `taxonomy:name-cluster`, `taxonomy:facet-extract`) and the matching `taxonomy.*` root span name. Tags alone are free-form ingest data, so taxonomy suppression requires the span-name match. In those cases the "user" message is scaffolding that embeds other conversations, which looks like a true positive for user-centric categories. Assistant-response-centric strategies still run so reflag / taxonomy self-observability can catch bad model outputs.
