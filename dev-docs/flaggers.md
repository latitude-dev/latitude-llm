...
User/input-centric strategies (`classifiesAssistantResponseOnly: false` — today `frustration`, `jailbreaking`, and `nsfw`) are dropped when session tags mark the user-role text as nested conversation samples (`isUserCentricReflagInapplicable`): first-level flagger-generated sessions (`flagger:classify` / `flagger:draft`), signal-details naming (`issue:details`), and taxonomy dogfood traces (`taxonomy:propose-themes`, `taxonomy:name-cluster`, `taxonomy:facet-extract`). In those cases the "user" message is scaffolding that embeds other conversations (or occurrence feedback quoting them), which looks like a true positive for user-centric categories. Assistant-response-centric strategies still run so reflag / taxonomy / signal-discovery self-observability can catch bad model outputs.
