...
- Entry points are resolved **once, at process start**. Upgrading the package under a running Hermes changes nothing until the process restarts, and the gateway is both long-lived and supervised (`KeepAlive`), so it never restarts on its own — the cron ticker runs inside it too, so scheduled agents inherit the same stale plugin. The failure mode is the expensive one: the old emitter keeps exporting `200`s, so the traces are merely the wrong *shape* and `LATITUDE_DEBUG` looks healthy. `hermes.plugin.version` (and the `service.version` resource attribute) is the only account of what is *running*, since the installed version on disk is exactly what a stale process contradicts.
- The official installer's venv has **no `pip`**, so every user-facing install/upgrade instruction for that path has to go through `~/.hermes/bin/uv pip --python <venv python>`. `importlib.metadata.version()` is the portable way to read an installed version, since `pip show` is unavailable there.
