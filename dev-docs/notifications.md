...
| `apps/web/src/routes/api/notifications/$nid/incident-trend.png.ts` | Public `GET /api/notifications/<notificationId>/incident-trend/png` route that renders the per-notification trend PNG via `satori` + `@resvg/resvg-js`. Row loaded via the admin client (RLS bypass). |
