...
**Avoid snapshotting human-facing project labels in notification payloads.** Use the row-level `projectId` and resolve names lazily so renames flow through automatically. The old `wrappedReportPayload.projectName` was removed for this reason — it could go stale and bloated every row with redundant data.

Incident payloads carry `sourceType`/`sourceId` (the issue id today) plus an optional **`projectSlug`** snapshotted at request time. Slugs are routing keys, not display names: they keep bell rows and email CTAs linkable when the client projects collection has not synced yet or the server-side project row is missing at email send, while `notification.projectId` + `useProjectsCollection` / `ctx.project` still win when present so slug renames keep working for fresh data. Snapshotting **derived** data like trends, breach numbers, or per-bucket thresholds is fine and encouraged — those are point-in-time facts about the event, not live entity attributes.
