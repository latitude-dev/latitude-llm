...
The builder emits one batch per finished run (plus standalone batches for late `subagent` spans and out-of-run compactions). `Transport` serializes them into one POST each, sequentially, with up to three attempts on `429` / `5xx` / network errors (jittered backoff, `Retry-After` honoured) and no retry on any other `4xx`. A span accepted once is never resent: `traces_mv` / `sessions_mv` add per insert. `gateway_stop` flushes for up to 4 s. Content gating, the per-attribute budget (`maxContentChars`, middle truncation) and attribute redaction all happen in `otlp.ts` at encode time.
