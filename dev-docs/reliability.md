...
- BullMQ is transport, not lifecycle storage; durable progress, dedupe semantics, ownership, and visible progress stay in Postgres/domain state even when BullMQ provides dedupe, debounce, throttle, or latest-throttle coalescing primitives
- topic publication may request logical coalescing keyed by entity identity so BullMQ can reuse delayed jobs or job ids without leaking transport detail to callers
- use `debounceMs` when repeated publishes should reschedule the fire time and preserve the newest payload; use `throttleMs` when the first publish should schedule a fixed fire time and later publishes in the window should be dropped; use `latestThrottleMs` when the first publish should schedule a fixed fire time but later publishes in the window should replace the pending payload without extending the window
- the `TracesIngested` debounced trace-end runtime is the canonical queue-backed debounce example: each new ingest batch fan-outs one publish per deduped trace id, and each publish reschedules the same logical delayed `trace-end:run` task for that trace; the worker composition root is `runTraceEndJob` in `apps/workers/src/workers/trace-end.ts`, with domain rules split across `@domain/spans`, `@domain/evaluations`, and `@domain/annotation-queues` (see `./spans.md`)
...
For the initial reliability events, `TracesIngested` publishes directly through `createEventsPublisher(queuePublisher)` because it comes from a high-volume append-only flow whose upstream write is already durable before publication.
...
- each project starts with default system-created manual queues such as `Jailbreaking`, `Refusal`, `Frustration`, `Forgetting`, `Laziness`, `NSFW`, and `Trashing`; deterministic telemetry signals (`tool-call-errors`, `output-schema-validation`, `empty-response`) skip the queue path and publish annotation scores directly so `issues:discovery` can cluster them
...
- system-created manual queues are populated asynchronously from debounced `TracesIngested`: the `domain-events` dispatcher publishes one `trace-end:run` per deduped trace id, that runtime samples system queues first, and it starts one `system-queue-flagger` workflow per selected queue; the workflow performs deterministic routing or a limited-context flagger pass and, when it confirms a match, writes the queue item and pending-review draft annotation with full context
