...
- each project starts with default system-created manual queues such as `Jailbreaking`, `Refusal`, `Frustration`, `Forgetting`, `Laziness`, `NSFW`, and `Thrashing`; deterministic telemetry signals (`tool-call-errors`, `output-schema-validation`, `empty-response`) skip the queue path and publish annotation scores directly so `issues:discovery` can cluster them
