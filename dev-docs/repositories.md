...
| `**findOptionalByXxx**`                                                            | Same as `findByXxx`, but absence is a normal product state (e.g. an organization that never bought an override). Use `findOptional*` to make optionality obvious to callers.  | `Effect<Entity \| null, E, R>`                 | Usually `RepositoryError` only                                                                                |
...
- **BillingUsageEventRepository** — `insert` (idempotency-violation tagged), `insertMany`, `findOptionalByKey` (for idempotency lookup).
- **BillingUsagePeriodRepository** — `upsert`, `appendCreditsForBillingPeriod` (atomic credits + overage recompute), `findOptionalByPeriod`, `findOptionalCurrent`, `advanceReportedOverageCredits`.
- **StripeSubscriptionLookup** — `findOptionalActiveByOrganizationId` (free-plan orgs have no active subscription).
