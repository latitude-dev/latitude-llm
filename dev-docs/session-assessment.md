...
The ClickHouse adapter performs bounded bulk reads for session details, spans, latest conversation-analysis generations, moments, labels, and screening decisions. Generation facts are read sequentially in bounded trace batches. Selected content keeps one budget across the complete session batch and is loaded in trace-scoped chunks. The adapter excludes sessions ending after the cutoff, excludes facts indexed after the cutoff, and filters moments and labels to the latest analyzed generation. A newer failed or skipped analysis does not fall back to stale moments.
