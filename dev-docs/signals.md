...
The database default is `[]`. Signal creation paths initialize the field explicitly, and rows that predate the column receive the empty list from the schema migration. During an upgrade, that known historical state is the selector for the score-evidence backfill; there is no score-evidence version column.
...
The upgrade backfill selects every promoted, non-deleted, system-discovered signal with `scoreEvidence = []` and at least one published occurrence in the previous 30 days. Resolved, ignored, muted, regressed, assigned, and prioritized signals remain eligible for this classification pass. Each target is processed sequentially. The same 200-score strict-majority rule provides a deterministic classification when possible; otherwise one model call classifies only the signal's canonical name and description. The model does not receive raw score occurrences. An all-false model response remains `[]` as a valid diagnostic classification.

The backfill is bundled in the workers image so self-hosted installations can run it after upgrading from a release without score evidence. From the image's `/app` working directory, run `node --enable-source-maps apps/workers/dist/scripts/backfill-signal-score-evidence.cjs` for a read-only routing preview, then repeat it once with `--execute`. A Postgres advisory lock rejects concurrent execution, and each write is conditional on the signal still being promoted, system-created, non-deleted, and empty. The command logs one progress line per signal with totals, elapsed time, and ETA.
