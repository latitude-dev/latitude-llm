...
The real generation runs in the `issues:nameOnPromotion` task, which the `SignalPromoted` handler publishes *instead of* the two announcements — it generates first and publishes them afterwards. It reaches the same use-case after `promoted_at` is stamped, so the guard does not block it. The ordering is load-bearing rather than tidy: agent dispatch builds its Cursor prompt from the signal's name and description, so announcing first would hand an agent one raw feedback sentence to open a pull request against, and Slack renders once at send time and would freeze the placeholder into a posted message. (The in-app renderer and the email are safe either way — the payload carries no name, so they resolve it live.) Generation is a reasoning-model call while those consumers pick up in milliseconds, so a fan-out does not race, it loses. Generation must also never suppress the announcement: on failure the task announces under the placeholder and lets the throttled refresh correct the name, because a signal announced plainly is recoverable and a signal never announced is not. The generation is asked to ignore the signal's current details, since using the placeholder as the stabilization baseline would anchor the first real summary to one member's phrasing.
