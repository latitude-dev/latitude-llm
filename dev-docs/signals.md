...
The database default is `[]`. Signal creation paths initialize the field explicitly, and rows that predate the column received the empty list from the schema migration. An empty list remains a valid diagnostic classification; there is no score-evidence version column.
