...
- **Level.** `signals.priority` holds the shared `ALERT_SEVERITIES` scale (`low` → `urgent`), so a signal's level and a monitor's severity are one vocabulary; the field name stays `priority` because it is public API. The level flows on to the notification payloads as `severity` (see `dev-docs/notifications.md`) and sets the severity of the signal's escalation incidents. **No level means no delivery** — no email, no Slack, no agent dispatch. Manual dispatch from the UI is exempt: a person can dispatch a signal whether or not it is rated.

  Two things decide it, and they answer different questions. **Severity** is intrinsic — how bad this is when it happens. **Volume** is spread — how much of the traffic it touches — and is measured continuously. `priority` is the effective level; `signals.priority_floor` is the level volume is not allowed to go below.

  *Only verified claims floor.* A floor is permanent in the downward direction, so it must not hold a guess. Human triage floors, and a detector that matched a named failure class floors (`pii-leakage` → `urgent`, `nsfw`/`jailbreaking` → `high`). **The severity rubric does not floor.** Graded against human-labelled production signals it is exact on 44% of cases and gives a different answer on 33% of them across repeated runs, and its errors skew high — so flooring on its rating would permanently pin over-ratings that volume otherwise corrects within hours. This costs no alerts: notifications read `priority`, which the rubric still sets at creation, so the delivery decision is unchanged and only the steady-state level is handed to measurement.

  *Severity, at creation.* `createSignalFromScoreUseCase` rates the prose in the same generation call that writes the name and description, inside the transaction that emits `SignalCreated`, and writes the result to `priority`. Detector floors override it upward, and those go to `priority_floor` too. Ties resolve **upward** — an over-rated signal is noise, an under-rated one is filtered out silently. The field is **required** in the generation schema: an optional field is not a neutral choice, because the model treats null as a legitimate answer and takes it (over the eval fixtures an optional `severity` came back unset 19 times in 20, the required one answered every time). Robustness lives in a retry instead — the same call produces the name and description a signal cannot be created without, so a model that cannot satisfy the wider schema falls back to the narrow one and the signal lands with no level.

  *Deterministic detectors are not rated.* `tool-call-errors`, `output-schema-validation`, `empty-response` and `low-cache-hit-rate` decide by inspection, so there is nothing left for a model to judge — and asking anyway agreed with human triage on none of the production signals these detectors opened, over-rating seven of eight. They start at `low` with **no floor**, which hands the level entirely to volume. `DETERMINISTIC_FLAGGER_SLUGS` in `@domain/flaggers` is the source of truth; `@domain/signals` mirrors it and `apps/workers/src/workers/deterministic-flaggers.test.ts` pins the two together (signals cannot import flaggers — no dependency runs that way, and adding one re-resolves the lockfile against the release-age gate).

  *Volume, continuously.* `recomputeSignalLevelUseCase` re-derives the level from `affectedSessionsPercent` against coarse bands (≥20% `urgent`, ≥5% `high`, ≥1% `medium`, else `low`), one tier higher while the signal sits inside an open escalation, then clamps to the floor. A share rather than a count is what carries across customers of wildly different size: four tool errors is most of a ten-session project and nothing in twenty thousand. It runs on incident open (`escalating: true`, before the incident is created), incident close (`escalating: false`), and the throttled `issues.refresh` job — deliberately ahead of that job's billing gate, since it is a ClickHouse read with no model call and a blocked organization should still see accurate levels. Every call site wraps it in `Effect.catchCause` so a ClickHouse failure cannot block the incident lifecycle.

  *Both directions, which is why the floor exists.* A spike that passes has to stop being `high`, or the level ratchets up over a project's lifetime until every signal claims to be urgent and the threshold sorts nothing. But a card number somebody triaged as urgent is urgent at any share, so volume is not allowed to argue with a judgement — it clamps at the floor. Raising is always allowed, which is what makes this safe without provenance tracking (LAT-844): a signal somebody filed as `low` on Monday still reaches them when it explodes on Friday.

  The backfill in `20260806132443_signal-priority-floor` sets `priority_floor = priority` for existing rows. That is sound precisely because the rubric has never run in production — `createSignalFromScoreUseCase` wrote `priority: null` before this change — so every level already stored there was set by a person.

  *Escalation raises the level rather than bypassing the filter.* A rate breaking its seasonal band is evidence the thing matters more than its steady-state share suggests, and expressing that as a level keeps one delivery rule with no exceptions — "below your threshold stays quiet" always holds. This matters because 97.4% of production escalations come from signals with no level at all, so an exemption would have been the one hole in *no level, no delivery*. Sensitivity (`k`, 1–6, default 3) is per project at `/projects/:slug/settings/signals`, read by `checkSignalEscalationUseCase`.

  *Hand-set levels.* `updateSignalTriageUseCase` writes `priority` **and** `priority_floor` together, so choosing a level asserts a severity the recompute cannot undo. Clearing the priority clears the floor, handing the level back to volume. `createSignalUseCase` does the same for a level chosen when a user creates the signal.

  Grade the rubric with `pnpm --filter @app/workers severity:eval`, which reports false-lows (rated `low` when the case is high or urgent — the miss a threshold silently drops) separately from instability across repeats. It runs on synthetic fixtures by default, or `--cases-file <path>` for real triaged signals exported from a notebook; nothing in the harness connects to a database and the fixtures are synthetic because this repo is public.
