...
Signal detail pages and drawers show the unique dimensions from `scoreEvidence` as chips in canonical dimension order. The Signals list renders the same chips in one row per signal and provides a URL-backed multi-select dimension filter. The session Signals tab uses the same one-row dimension chips. Multiple selected dimensions use OR semantics, and multiple evidence roles for the same dimension never duplicate a row or chip. A signal with no scoring roles, or an ignored signal, leaves its Dimensions area empty. Ignored signals can still store a classification in the API, but the UI does not present those roles as live scoring evidence. Signals with no scoring roles do not match dimension filters.

The database default is `[]`. Signal creation paths initialize the field explicitly, and rows that predate the column receive the empty list from the schema migration. During the rollout, that known historical state is the selector for a one-time backfill; there is no score-evidence version column.
...
The rollout backfill selects every promoted, non-deleted, system-discovered signal with `scoreEvidence = []` and at least one published occurrence in the previous 30 days. Resolved, ignored, muted, regressed, assigned, and prioritized signals remain eligible for this classification pass. Each target is processed sequentially. The same 200-score strict-majority rule provides a deterministic classification when possible; otherwise one model call classifies only the signal's canonical name and description. The model does not receive raw score occurrences. An all-false model response remains `[]` as a valid diagnostic classification.

The backfill is a domain use case backed by the Signal and Score repositories. Its production entrypoint is bundled into the workers image but launched as an independent one-time ECS task rather than a BullMQ job, so queue retries cannot re-run the migration. A dedicated ECS task-definition family is the durable launch marker, and a Postgres advisory lock rejects concurrent execution. The task inherits the deployed production worker task definition's environment, secrets, IAM roles, and network configuration. It logs one progress line per signal with totals, elapsed time, and ETA.
...
**From a saved search.** `savedSearchSignalDraft` (`@domain/signals`, browser-safe) maps a saved search onto the signal that tracks it, and is the single source both entry points use — the "Create a signal from this search" toggle in the save-search modal and the signal action on a saved-search row. Its `filterSet` becomes the detector's scope; its query becomes a `match: "all"` rule that translates each query shape the way `search-plan.ts` executes it:

| query | rule |
| --- | --- |
| phrase clause | one `text_match` over the whole conversation per clause — `"literal"` case-sensitive (`normalizeLiteralPhrase` keeps case), `` `phrase` `` case-insensitive (`tokenizePhrase` lower-cases) |
| free text | one `semantic_similarity` condition at `TRACE_SEARCH_MIN_RELEVANCE_SCORE`, the same cosine floor a semantic-only search admits a match at (both compare a max-pooled per-message similarity), so the signal never sits stricter than the search it came from |
| empty | the single `always` condition, so the signal tracks every session in scope |

Every clause becomes a condition, including free text alongside phrases. Search itself drops the semantic floor once a phrase is present and ranks by similarity instead, so a mixed-query signal is marginally stricter than its search — it excludes a phrase match scoring below search's own noise floor on the free text. That is the deliberate trade: a detector has no ranking, so a clause is either a predicate or discarded, and discarding words the user typed is the worse surprise (it cost a demo once — the mapping shipped that way briefly in #4554).

A query with more phrase clauses than `EVALUATION_RULE_MAX_CONDITIONS` keeps the first ones and the signal is broader than the search, which beats refusing to create it; the free-text clause holds its slot rather than losing it to phrase order. Keys the detector could not usefully gate on are dropped rather than imported as dead scope: the date picker's window (`TRACE_TIME_FILTER_FIELDS`), for the same reason a monitored predicate drops it (an absolute past range would silently freeze a detector that keeps running); session-only keys (`moments`, `topics`, `hasLlmActivity`), because the live pre-gate matches the triggering *trace* and the trace query cannot apply them (`isTraceFilterFieldName`); and `score.*` keys, because the signal's own evaluation is what writes scores, so a session reaching the pre-gate has none yet. The scope editor mirrors all three: it renders the shared filter builder in `traces` mode with `excludeFields={SCORE_FILTER_FIELDS}`. Nothing is stored linking the two, so a later edit to the search does not follow into the signal.
