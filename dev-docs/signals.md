...
The rollout backfill selects every promoted, non-deleted, system-discovered signal with `scoreEvidence = []`, at least one published occurrence in the previous month, and a promotion timestamp before the production task definition carrying the score-evidence rollout was registered. The promotion cutoff prevents a valid diagnostic classification created after rollout from being mistaken for historical ungenerated evidence. Resolved, ignored, muted, regressed, assigned, and prioritized signals remain eligible for this classification pass. Each target is processed sequentially. The same 200-score strict-majority rule provides a deterministic classification when possible; otherwise one model call classifies only the signal's canonical name and description. The model does not receive raw score occurrences. An all-false model response remains `[]` as a valid diagnostic classification.
