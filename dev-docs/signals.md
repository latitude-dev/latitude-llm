...
- **Level.** `signals.priority` holds the shared `ALERT_SEVERITIES` scale (`low` → `urgent`), so a signal's level and a monitor's severity are one vocabulary; the field name stays `priority` because it is public API. The level flows on to the notification payloads as `severity` (see `dev-docs/notifications.md`) and sets the severity of the signal's escalation incidents. **No level means no notification** — no email, no Slack. Agent dispatch is untouched by this and keeps its own rules.
  ...
  **Assigned once, at creation, and never re-assigned automatically.** `createSignalFromScoreUseCase` rates the prose in the same generation call that writes the name and description, inside the transaction that emits `SignalCreated`, so the notification producer reads a level rather than a null. After that only `updateSignalTriageUseCase` writes it — including when somebody clears it back to unset — and a refresh never re-rates. Nothing derives a level from measured impact: a volume model was built and removed before merge, because a level that moves on its own is a level nobody can reason about, and the signals/monitors merge will revisit how impact feeds triage.

  Since a new signal is one occurrence old, the rating describes the failure mechanism rather than measured spread, and ties resolve **upward** — an over-rated signal is noise, an under-rated one is filtered out silently. The level is **required** in the generation schema: an optional field is not a neutral choice, because the model treats null as a legitimate answer and takes it (over the eval fixtures an optional `severity` came back unset 19 times in 20, the required one answered every time). Robustness lives in a retry instead — the same call produces the name and description a signal cannot be created without, so a model that cannot satisfy the wider schema falls back to the narrow one and the signal lands with no level.

  *Detector floors.* Where a flagger names the failure class outright, that beats re-inferring it from prose: `pii-leakage` → `urgent`, `nsfw` → `high`. Applied at creation as a maximum against the model's rating, never lowering it. `jailbreaking` is deliberately absent — it fires on an attempt the guardrail may have stopped, and the one production signal a person triaged for it they parked at `low`.

  *Deterministic detectors are not rated.* `tool-call-errors`, `output-schema-validation`, `empty-response` and `low-cache-hit-rate` decide by inspection, so there is nothing left for a model to judge — and asking anyway agreed with human triage on none of the production signals these detectors opened, over-rating seven of eight. They are created at `low`. `DETERMINISTIC_FLAGGER_SLUGS` in `@domain/flaggers` is the source of truth; `@domain/signals` mirrors it and `apps/workers/src/workers/deterministic-flaggers.test.ts` pins the two together (signals cannot import flaggers — no dependency runs that way, and adding one re-resolves the lockfile against the release-age gate).

  *Escalation does not touch the level.* A signal whose rate breaks its seasonal band keeps whatever level it has; what changes is that its notification ignores the channel's minimum severity entirely (`isSignalEscalation` in `@domain/integrations`). The two are different claims — how bad the pattern is, versus the rate having just moved — and a severity threshold was not set to answer the second. 97% of production escalations come from signals nobody has triaged, so honouring the threshold there would silence nearly all of them. The Slack message says it was sent on volume, so the exemption is visible to whoever receives it instead of looking like the filter leaking.

  *What the rating is worth.* Two people rating the same single occurrence, blind and against this same rubric, agree on the exact tier **60% of the time** (within one tier 92%, Cohen's kappa 0.43). That is the denominator for any measurement of the rubric — 40% exact reads as poor against 100% and near-optimal against 60%, and the task is genuinely subjective. The model matches human agreement on tolerance (93% within one tier) and produced no false-lows, but disagrees with *itself* on 47% of cases against 10% for people, and no configuration fixed that: `temperature: 0` (verified reaching the provider), a different model, majority-of-3, and reframing the question all failed. Treat instability as the ceiling on prompt work here.

  *What the rating is worth.* Two people rating the same single occurrence, blind and against this same rubric, agree on the exact tier **60% of the time** (within one tier 92%, Cohen's kappa 0.43). That is the denominator for any measurement of it: 40% exact reads as poor against 100% and near-optimal against 60%, and the task is genuinely subjective. The model matches human agreement on tolerance (93% within one tier) and produced no false-lows, but disagrees with *itself* on 47% of cases against 10% for people — and nothing fixed that: `temperature: 0` (verified reaching the provider, not just the resolved config), a different model, majority-of-3, and reframing the question all failed. Instability, not knowledge, is the ceiling on prompt work here.

  Grade the rubric with `pnpm --filter @app/workers severity:eval --cases-file <path>`, which reports false-lows (rated `low` when the case is high or urgent — the miss a threshold silently drops) separately from instability across repeats. A cases file is required: nothing in the harness connects to a database, and the file holds real customer feedback so it stays outside this public repo.
