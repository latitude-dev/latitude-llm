...
| query | rule |
| --- | --- |
| any phrase clause | one `text_match` over the whole conversation per clause — `"literal"` case-sensitive (`normalizeLiteralPhrase` keeps case), `` `phrase` `` case-insensitive (`tokenizePhrase` lower-cases). Free text in the same query is **not** translated: the lexical filter is search's precision gate and similarity only ranks the hits, so a semantic condition would make the signal stricter than the search |
| free text only | one `semantic_similarity` condition at `TRACE_SEARCH_MIN_RELEVANCE_SCORE`, the same cosine floor a semantic-only search applies (both compare a max-pooled per-message similarity), so no session the search lists goes untracked |
| empty | the single `always` condition, so the signal tracks every session in scope |

A query with more phrase clauses than `EVALUATION_RULE_MAX_CONDITIONS` keeps the first ones: the signal is then broader than the search, which beats refusing to create it. Keys the detector could not usefully gate on are dropped rather than imported as dead scope: the date picker's window (`TRACE_TIME_FILTER_FIELDS`), for the same reason a monitored predicate drops it (an absolute past range would silently freeze a detector that keeps running); session-only keys (`moments`, `topics`, `hasLlmActivity`), because the live pre-gate matches the triggering *trace* and the trace query cannot apply them (`isTraceFilterFieldName`); and `score.*` keys, because the signal's own evaluation is what writes scores, so a session reaching the pre-gate has none yet. The scope editor mirrors all three: it renders the shared filter builder in `traces` mode with `excludeFields={SCORE_FILTER_FIELDS}`. Nothing is stored linking the two, so a later edit to the search does not follow into the signal.
