...
- **Level.** `signals.priority` holds the shared `ALERT_SEVERITIES` scale (`low` → `urgent`), so a signal's level and a monitor's severity are one vocabulary; the field name stays `priority` because it is public API. With the `derivedSignalSeverity` flag on, `createSignalFromScoreUseCase` fills it from the same generation call that writes the name and description, inside the transaction that emits `SignalCreated` — so the notification producer reads a level rather than a null. It only ever fills an unset value: `updateSignalTriageUseCase` remains the sole writer afterwards, including when someone clears the level back to unset. Since a new signal is one occurrence old, the derived value rates the failure mechanism, not measured impact, and ties resolve downward. The level flows on to the notification payloads as `severity` (see `dev-docs/notifications.md`) and sets the severity of the signal's escalation incidents.
