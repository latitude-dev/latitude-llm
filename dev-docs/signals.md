...
Deliberate human intent promotes without evidence. A signal created by a person (`origin = 'user'`) is born promoted. A discovered signal that somebody tracks by attaching an evaluation is promoted too — that one arrives with enforcement, as a clause in the re-evaluation pass, since nothing reads `promoted_at` before then and an unpromoted signal is not reachable to track once it does. Everything else has to accumulate evidence.
