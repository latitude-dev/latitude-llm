...
- **Level.** `signals.priority` holds the shared `ALERT_SEVERITIES` scale (`low` → `urgent`), so a signal's level and a monitor's severity are one vocabulary; the field name stays `priority` because it is public API. `createSignalFromScoreUseCase` fills it from the same generation call that writes the name and description, inside the transaction that emits `SignalCreated` — so the notification producer reads a level rather than a null. Only at creation: `updateSignalTriageUseCase` remains the sole writer afterwards, including when someone clears the level back to unset, and a refresh never re-rates. Since a new signal is one occurrence old, the derived value rates the failure mechanism, not measured impact, and ties resolve **upward** — an over-rated signal is noise, an under-rated one is filtered out silently. The level is **required** in the generation schema. An optional field is not a neutral choice — the model treats null as a legitimate answer and takes it: over the eval fixtures an optional `severity` came back unset 19 times in 20, while the required one answered every time. Robustness lives in a retry instead: the same call produces the name and description a signal cannot be created without, so a model that cannot satisfy the wider schema falls back to the narrow one and the signal lands with no level. Grade the rubric with `pnpm --filter @app/workers severity:eval`, which reports false-lows (rated `low` when the case is high or urgent — the miss a threshold silently drops) separately from instability across repeats. The level flows on to the notification payloads as `severity` (see `dev-docs/notifications.md`) and sets the severity of the signal's escalation incidents.
