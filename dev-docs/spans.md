...
**It fails closed.** A throw or a deadline overrun fails the effect and nothing is inserted. The worker treats that `RedactionError` as terminal, so the same payload is not walked again. Transient ClickHouse or object-store failures retry (`attempts: 10`, 1s exponential backoff). Before insert, identities already in ClickHouse are skipped: `spans` is a `ReplacingMergeTree` and would collapse a re-insert, but `traces_mv`/`sessions_mv` are `AggregatingMergeTree` and add per inserted block, so a post-insert retry would permanently double counts, tokens, cost, and duration. `TracesIngested` still publishes after a skipped insert so a publish failure can recover without rewriting the rollups. This is a deliberate divergence from lmnr and langfuse, which fail open because their synchronous export path made dropping telemetry the only alternative. Ours is already acknowledged, and for the deterministic tier "failure" means a code bug or a deadline overrun, so failing open would reduce to silently writing plaintext PII for a customer who explicitly asked us not to — permanently, since redaction is not retroactive and there is no delete path.
