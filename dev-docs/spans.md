...
- backtick text (`` `...` ``) is an ordered token phrase. It is tokenized with the same `splitByNonAlpha` shape as the ClickHouse text index. ClickHouse 26.2 does not expose `hasPhrase`, so the repository composes `hasAllTokens(search_text, tokens)` as the indexed prefilter with `hasSubstr(tokens(search_text, 'splitByNonAlpha'), tokens)` as the adjacency/order check.
