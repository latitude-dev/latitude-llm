...
A leaf centroid sits closer to its own members than a root does, so the same number is a harsher gate online than in reassignment. Measured on production the practical difference is small, but mind the timepoint: the figures below re-score the `centroid_online` rows of one 7-day window (9,829 rows whose assigned cluster was still active) against the tree as it stood at measurement time, and those rows were admitted by the *old* 0.65 root gate. So they answer "what would this window be rejected at today", not "how does the 0.75 gate behave" — under 0.75 a depth-3 assignment must by construction have cleared 0.75 at the root. On that basis the median cluster's centroid sits at 0.86 cosine (depth 3) to its root, only 0.2% of depth-3 rows fall below 0.75 at the root — reaching depth 3 already required clearing the root and each descent gate — and rejections concentrate at depth 0 (23.6%), where the two tests coincide because there the root *is* the assigned cluster. Separating a loose root/admission gate from a strict leaf-fit floor is a real design question, deferred with per-cluster thresholds.

**Where 0.75 comes from.** The previous 0.65 was too permissive nearly everywhere: across production clusters with ≥50 members, the per-cluster p10 of member-to-centroid similarity — the similarity at the 10th percentile of a cluster's own members, i.e. the level a tenth of its genuine members sit below — has median 0.813 and q05 0.671, so 0.65 sat below the p10 of ~95% of clusters. 0.75 is deliberately *below* that median, because those floors span 0.671 (q05) to 0.977 (q95) and no flat value fits them all. Matching the median cluster (0.81) rejects 27.7% of online assignments and costs 2 of 11 well-measured projects more than half their assignments; 0.75 rejects 12.1% and costs no project that much. Bounding the collateral of being flat is what the value is for — per-cluster thresholds are the real fix, and that spread is the evidence for them.
