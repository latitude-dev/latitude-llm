...
Taxonomy organizes live-retained moment-level **topic observations** produced by [conversation intelligence](./conversation-intelligence.md) into a **single tree of clusters** per project. There is no separate category model: every tree level is the same kind of node, and **depth is clustering density** — depth-0 roots form at a coarse calibrated link threshold, and each level below re-clusters its parent's members at a tighter density derived from that parent's own similarity distribution. "Categories" in the product UI are simply depth-0 nodes.
