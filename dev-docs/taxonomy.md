...
Taxonomy organizes a representative sample of moment-level **topic observations** produced by [conversation intelligence](./conversation-intelligence.md) into a **single tree of clusters** per project. There is no separate category model: every tree level is the same kind of node, and **depth is clustering density** — depth-0 roots form at a coarse calibrated link threshold, and each level below re-clusters its parent's members at a tighter density derived from that parent's own similarity distribution. "Categories" in the product UI are simply depth-0 nodes.
...
- `taxonomy_observations` — a bounded representative sample of moment topic projections: embedding, `assigned_cluster_id` (empty = noise), assignment method (`centroid_online` / `gardening_birth` / `gardening_reassign` / `noise`), confidence, `analysis_hash`, `reassignment_run_id`. `ReplacingMergeTree(indexed_at)` is ordered for gardening and read paths by `(org, project, assigned_cluster_id, start_time, observation_id)`, with bloom indexes on `session_id`, `analysis_hash`, and `observation_id`. The deprecated observation `dimension` column is not part of this table; clusters and run records may still carry the singleton `topic` dimension while the broader taxonomy naming cleanup completes.

### Representative sampling

Taxonomy observations are deliberately sampled. Conversation intelligence stores every semantic moment and moment label, but only a bounded sample enters clustering so gardening remains non-paginated and predictable for high-volume projects. The analyzer accepts taxonomy observations until the project-level sample cap is reached, and accepts noise observations only until the noise sample cap is reached. Once a cap is full, additional moments still appear in session moment intelligence and labels, but they do not create taxonomy observation rows or live centroid increments. Gardening reads at most the configured noise sample cap, so birth and reassignment work stays bounded.
...
- **Sibling merges** (`mergeNearDuplicateClustersUseCase`): merges are **sibling-only** (cross-parent merges would give a node two ancestries). Nomination is **density-aware per level**: two siblings more similar than the density that created their level are one cluster at that density — roots nominate at the calibrated root link, children at their parent's `split_link_threshold`. Name-overlap (Jaccard or token-subset) also nominates above a sane floor. Candidate nomination is bounded with a nearest-neighbour cap per cluster and a per-parent candidate cap; the LLM judge (Bedrock Haiku, sizes in the prompt, "a small fragment of the same domain merges into the dominant topic") decides the bounded nominated set, and failures are logged and conservatively reject. Approved pairs assemble via **complete-linkage components** — best pairs first, a component only grows while every cross-pair clears the floor — so one weak transitive edge can no longer drop a whole component's high-confidence merges. The survivor absorbs the loser's sampled observations and **adopts its subtree** (children re-parent, path prefixes rewrite, from fresh locked reads). A loser is marked merged only after the bounded observation rewrite completes; if the rewrite hits the hard list cap, the merge is skipped rather than partially stranding rows on an inactive cluster.
...
- **Representative clustering sample**: taxonomy formation does not need every semantic moment. Complete moment and label data remains in conversation intelligence tables; taxonomy uses a bounded observation/noise sample to keep gardening deterministic and performant for projects with millions of sessions per month.
...
- **Merge redirect read support**: sampled rows are currently rewritten during safe merges; read-time redirect resolution for `merged_into_cluster_id` would make old rows and interrupted compactions transparent.
