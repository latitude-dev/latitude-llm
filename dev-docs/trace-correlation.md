...
`child_env()` returns the current environment plus the four variables, anchored on the tool span currently executing. The header is published under **both** `TRACEPARENT` and `LATITUDE_TRACEPARENT`: reads prefer the scoped name, so publishing only the bare one would let a scoped header this process inherited outrank the span just opened, attaching the child to its grandparent. `current_traceparent()` returns just the header. Both read a `ContextVar` set for the duration of a tool call, so they are correct under Hermes's per-turn worker threads.

Setting `LATITUDE_HERMES_EXPORT_TRACEPARENT=1` additionally scopes those variables onto `os.environ` around each tool call, so *any* subprocess inherits them with no code change. It is **off by default**: `os.environ` is process-wide while Hermes runs each turn on its own thread, and the ownership lock cannot fix that: it keeps a concurrent call from corrupting the owner's saved values, but the environment still holds the owner's header, so a subprocess launched by the skipped call inherits the **wrong** parent span rather than none. Safe only when tool calls do not overlap. `child_env()` is the path that is always correct.
