...
| `LATITUDE_TRACEPARENT` | Same format, higher precedence. Lets a repo that already sets `TRACEPARENT` for an unrelated pipeline opt a harness in or out without disturbing it. |
| `LATITUDE_SESSION_ID` | The Latitude session both harnesses report, so they group in the session view as well as the trace. |
...
A trace id that is minted per turn is itself a unique salt for the span ids inside it. An inherited one is shared by every turn of the session, so the turn coordinates have to move into the salt. In the Claude Code emitter this matters concretely: a call with no message id falls back to `noid:<index>`, which repeats every turn, so turn 2's first call would otherwise reuse turn 1's span id.
