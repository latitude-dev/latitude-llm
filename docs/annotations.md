...
- human-created draft finalization is driven by persisted due-work scans plus the `score-draft-finalization` background task, not by browser-local timers or BullMQ delayed jobs
- system-created queue drafts still use the queue CUID as `source_id`
- they are created by a separate validation/annotation task that runs after the queue-flagging event handler has flagged the trace for that system queue
...
6. when the human-editable draft becomes due, the `score-draft-finalization` worker clears `draftedAt`
