...
<AccordionGroup>
  <Accordion title="Refusal">
    ...
    Not flagged: a correct refusal, where the request is unsafe, unsupported, missing context the agent needs, or an external action / tool / unsupported modality that none of the agent's declared tools can fulfill. Native model work such as summarizing pasted text is still flagged when refused.
  </Accordion>
