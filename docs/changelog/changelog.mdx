...
## Implement Agent Toolbar for New Header Editor
- Added a toolbar for the new header editor.
- Made the blocks editor take full height as Monaco, with only the editor content scrollable.

## Make DB Read Replicas Optional
- Introduced a feature to make database read replicas optional.

## Prepare Editor Header for New Arrangement of Elements
- Moved elements and added a feature flag to set up everything without breaking the current header in Documents or Evaluations.

## Delete Integration Triggers
- Deletes the deployed trigger from Pipedream when removing the trigger in Latitude.

## Style Steps According to Design and Add Step Names to Auto Complete
- Styled step blocks and added step names to auto-complete.

## Pipedream Triggers
- Added integration triggers for Pipedream, including webhook handlers and UI components.

## Bump @ai-sdk/google-vertex from 2.1.12 to 2.2.27
- Updated the dependency to include new features and fixes.

## Bump cron-parser from 5.0.4 to 5.3.0
- Updated the cron-parser dependency to support new features and improvements.

## Implement Blocks Editor Update Cycle
- Connected the dots for the blocks editor update cycle, including text to AST to blocks to Lexical Editor and back.

## Fix Segment ID Column Type
- Fixed the segment ID column type in the database.
