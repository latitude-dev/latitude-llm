...
- **LLM Evaluations V2**: Introduced the base for LLM Evaluations V2, setting the foundation for further enhancements. [PR #1131](https://github.com/latitude-dev/latitude-llm/pull/1131)
- **SMTP Support for Mail Transport**: Added SMTP support for mail transport, enhancing communication capabilities. [PR #1122](https://github.com/latitude-dev/latitude-llm/pull/1122)
- **SSO with Google**: Implemented Single Sign-On (SSO) with Google, a significant feature for user authentication. [PR #1121](https://github.com/latitude-dev/latitude-llm/pull/1121)

## Bug Fixes

- **SDK Import Issue**: Fixed an issue with the SDK failing to import methods from constants, which is crucial for functionality. [PR #1124](https://github.com/latitude-dev/latitude-llm/pull/1124)
- **Chart Display**: Improved chart display by fixing overflow issues, enhancing visual representation.
- **Dataset Preview Error Details**: Enhanced error details for dataset preview generation, improving debugging and user experience.

## Codebase Improvements

- **Remove Dataset V1 Leftovers**: Cleaned up remnants of dataset v1, which could streamline the codebase.
- **Rollbacked Migration**: Added a rollbacked migration, which might be significant for database management and version control.
