...
Telemetry is kept for `config.telemetryRetentionDays` (default `3650`, ten years), stamped on each row as it is ingested. Nothing is metered: `LAT_BILLING_ENABLED` stays off unless you are running Latitude Cloud. Changing the value applies to newly ingested data — see [widening retention on existing data](/deployment/single-host#retention).
