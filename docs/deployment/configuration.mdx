...
| `LAT_JEV_BASE_URL`, `LAT_JEV_MODEL`, `LAT_JEV_TIMEOUT_MS` | Optional Jev provider endpoint, model, and request timeout in milliseconds. The endpoint and model default to `https://api.typesafe.ai` and `jev-latest`; the timeout defaults to `2000` for shadow calls and `5000` for pre-classifier calls. |
