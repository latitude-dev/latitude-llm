...
| `LAT_JEV_FLAGGER_SHADOW_ENABLED` | Enables the Jev flagger shadow pilot globally (default `false`). It also requires `LAT_JEV_API_KEY` and the `jevFlaggerShadow` organization feature flag. The shadow call is advisory and does not change flagger results or billing. |
| `LAT_JEV_FLAGGER_PRECLASSIFIER_ENABLED` | Enables the Jev session pre-classifier globally (default `false`). It also requires `LAT_JEV_API_KEY` and the `jevFlaggerPreclassifier` organization feature flag. |
| `LAT_JEV_API_KEY` | API key shared by the optional Jev flagger shadow and pre-classifier paths. Keep this in a secret. |
| `LAT_JEV_BASE_URL`, `LAT_JEV_MODEL`, `LAT_JEV_TIMEOUT_MS` | Optional Jev provider endpoint, model, and request timeout in milliseconds. The endpoint and model default to `https://api.typesafe.ai` and `jev-latest`; the timeout defaults to `2000` for shadow calls and `12000` for pre-classifier (`decideMany`) calls; set `LAT_JEV_TIMEOUT_MS` only when you intentionally override the provider budget. |
