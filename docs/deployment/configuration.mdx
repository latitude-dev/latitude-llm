...
| `LAT_TELEMETRY_RETENTION_DAYS` | How long spans, traces, and search rows are kept, in days (default `3650`, max `3650`). Stamped on each row at ingest, so a change applies to new data only. Ignored when `LAT_BILLING_ENABLED=true`, where the organization's plan sets retention. |
| `LAT_BILLING_ENABLED` | Enforce Latitude Cloud's credit plans and plan retention (default `false`). Self-hosted deployments leave it off: ingestion is never capped and retention comes from `LAT_TELEMETRY_RETENTION_DAYS`. |
