...
| `LAT_ORCAROUTER_*` | OrcaRouter `orcarouter` | Generation, embeddings |
...
| `LAT_JEV_API_KEY` | API key for the optional Jev flagger pre-classifier. Keep this in a secret. |
| `LAT_JEV_BASE_URL`, `LAT_JEV_MODEL`, `LAT_JEV_TIMEOUT_MS` | Optional Jev provider endpoint, model, and request timeout in milliseconds. The endpoint and model default to `https://api.typesafe.ai` and `jev-latest`; the pre-classifier (`decideMany`) timeout defaults to `12000`; set `LAT_JEV_TIMEOUT_MS` only when you intentionally override the provider budget. |
