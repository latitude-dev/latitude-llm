...
Reranking is configured globally through `LAT_AI_RERANKING_PROVIDER` and `LAT_AI_RERANKING_MODEL`. It defaults to `voyage` / `rerank-2.5`, and refines issue-discovery candidate matching. Reranking is optional — when unavailable, discovery falls back to embedding-order matching.
