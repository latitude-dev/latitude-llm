...
A self-hosted deployment keeps telemetry for `LAT_TELEMETRY_RETENTION_DAYS` (default `3650`, ten years). Nothing is metered and no ingest request is ever rejected for billing: `LAT_BILLING_ENABLED` stays `false` unless you are running Latitude Cloud.
