...
A self-hosted deployment keeps telemetry for `LAT_TELEMETRY_RETENTION_DAYS` (default `3650`, ten years). With `LAT_BILLING_ENABLED` off, no ingest request is ever rejected for billing and no plan shortens retention. Usage is still counted per organization, so the credit figures on the billing page stay populated; nothing enforces them.

Retention is stamped on each row as it is ingested, so lowering or raising the value applies to new data only. To widen the window on data you have already ingested, update the stamp in ClickHouse:

```sql
ALTER TABLE spans UPDATE retention_days = 3650 WHERE retention_days < 3650;
ALTER TABLE traces UPDATE retention_days = 3650 WHERE retention_days < 3650;
ALTER TABLE sessions UPDATE retention_days = 3650 WHERE retention_days < 3650;
ALTER TABLE trace_search_documents UPDATE retention_days = 3650 WHERE retention_days < 3650;
ALTER TABLE trace_message_occurrences UPDATE retention_days = 3650 WHERE retention_days < 3650;
```

ClickHouse deletes a row `retention_days + 30` days after its start time, so the extra month is your window to notice and correct a retention setting before anything is removed.
