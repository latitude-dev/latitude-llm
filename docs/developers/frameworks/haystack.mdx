---
title: Haystack
description: Connect your Haystack-based application to Latitude Telemetry to observe pipelines per feature and run evaluations.
---
...
<Steps>
  ...
  <Step title="Initialize Latitude Telemetry">
    ...
    You must enable the **Haystack** instrumentor so Telemetry can trace it.
    ...
    telemetry = Telemetry(
        ...
        TelemetryOptions(
            instrumentors=[Instrumentors.Haystack],  # This enables automatic tracing for Haystack
        ),
    )
    ```

    <Info>
      The Telemetry instance should only be created once. Any Haystack pipeline
      runs after this will be automatically traced.
    </Info>

  </Step>

  <Step title="Wrap your Haystack-powered feature">
    Wrap the code that runs Haystack pipelines using <code>telemetry.capture</code>.
    ...
    def generate_support_reply(input: str) -> str:
        # Your regular Haystack pipeline code here
        pipeline = Pipeline()
