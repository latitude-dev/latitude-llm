...
<Steps>
  ...
  <Step title="Initialize Latitude Telemetry">
    ...
    You must enable the **Watsonx** instrumentor so Telemetry can trace it.
    ...
    telemetry = Telemetry(
        ...
        TelemetryOptions(
            instrumentors=[Instrumentors.Watsonx],  # This enables automatic tracing for watsonx.ai
        ),
    )
    ```

    <Info>
      The Telemetry instance should only be created once. Any watsonx.ai client
  ...
  <Step title="Wrap your watsonx.ai-powered feature">
    Wrap the code that calls watsonx.ai using <code>telemetry.capture</code>.
