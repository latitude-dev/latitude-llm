...
- issue-linked live monitor failures claim `scores.issue_id` during the canonical score write so the failed score is immutable immediately; other evaluation-originated failed scores that stay unowned may still flow through the centralized `issues:discovery` task, which resolves the linked issue before similarity search starts
