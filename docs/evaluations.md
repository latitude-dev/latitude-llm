...
- annotation-driven automatic realignment flows through two rate-limited BullMQ tasks: `evaluations:automaticRefreshAlignment` (1h rate-limit) starts `refresh-evaluation-alignment`, and on an incremental MCC drop that workflow publishes `evaluations:automaticOptimization` (8h rate-limit) which starts `optimize-evaluation`. Workflows never sleep — the queue owns both windows. Rate-limit (not debounce) semantics: the first publish schedules the fire time; subsequent publishes within the window are dropped by BullMQ, so a constant annotation stream cannot starve the refresh. Worst-case latency is bounded (1h for refresh, 8h for optimize) and fires are capped at once per window per evaluation
...
- after creation, rate-limited automatic realignment still runs as new annotations arrive for each linked evaluation
...
- rate-limited incremental metric recomputation at most once per hour per evaluation; fires at most 1h after the first annotation
- rate-limited full realignment at most once per eight hours per evaluation; fires at most 8h after the first MCC-drop escalation
...
- rate-limited automatic refresh runs through `refresh-evaluation-alignment` (started by the 1h-rate-limited `evaluations:automaticRefreshAlignment` queue task) and escalates into `optimize-evaluation` (started by the 8h-rate-limited `evaluations:automaticOptimization` queue task) when the incremental evaluator returns `full-reoptimization`; manual background refresh reuses the `evaluation-alignment` workflow path until it is migrated onto `optimize-evaluation` in a follow-up change
...
There is no minimum negative-example count for initial issue-linked generation. A monitor may be created from a single positive occurrence with zero negatives, and its alignment may be weak at first. As users add more annotations, the rate-limited realignment flow should improve that monitor over time.
