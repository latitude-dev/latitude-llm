...
Conditions, Jev classifiers, and LLM judges compile down to the same kind of script, so a custom script can express anything they can, plus whatever else you write.
...
### classify(instructions, options)

Asks Jev to choose between named options and returns the probability for every option. The current session is the state Jev evaluates. Each option value is a description or `null` when the name is clear enough.

```js
const probabilities = await classify('How should this support session be routed?', {
  billing: 'Payments, invoices, or refunds',
  technical: 'Bugs, outages, or integrations',
  account: 'Access, profile, or account settings',
})

return Score(probabilities.billing, 'Probability that billing should handle this session')
```

The probabilities sum to `1`. Your script decides which probability to score, how to combine probabilities, and where to set thresholds.
...
<Note>
Call `classify`, `llm`, and `semanticSimilarity` by name in your source. Latitude reads those names to allocate the right resources and to make the functions available, so building the call dynamically won't work.
</Note>

### What isn't available

The sandbox is deliberately small. There is no network access, no `fetch`, no timers, no `Date.now` or `Math.random`, and no Node or browser APIs. The only import allowed is `zod`. A script's verdict depends only on the session and its declared AI calls.
...
| Calls `classify()` | about 120 seconds |
...
Classifier, LLM, and embedding calls cost money, so use the sampling control in the [Scope step](../signals/create#scope) to check a slice of traffic rather than every session.

## Editing and detaching

If you build a detector with conditions, a Jev classifier, or an LLM judge, the Custom script tab shows the exact script Latitude compiled from your settings, read-only. Choose "Edit as custom script" to take it over by hand. This clears the settings form and switches the detector to your script.
...
- [Detection methods](./detection-methods): the builder methods for defining a detector
