...
description: "How an evaluation decides whether a session matches a signal: conditions, a Jev classifier, an LLM judge, or a custom script."
---

When you create a signal, you define how Latitude decides whether a session belongs to it. That check is the signal's evaluation, and you can build it four ways.
...
| [Jev classifier](#jev-classifier) | Jev assigns probabilities to options you define | One classification call per checked session | Routing, categories, and focused semantic decisions |
...
All four produce the same kind of detector, and a signal has one active detector at a time. You can switch methods while you build or edit a signal.
...
A Jev classifier asks one focused question about the session and assigns a probability to every option you define. Choose which option means the signal is present. Its probability becomes the signal score.

For example, ask "How should this support session be routed?" with options for billing, technical support, and account management. If billing is the signal match option and Jev returns `0.82` for billing, the evaluation returns a score of `0.82`.

Use a classifier when the answer fits a closed set of distinct outcomes. Use the preview to check the option descriptions and sampling rate against real sessions.
...
- If the behavior is a focused choice between known outcomes, use a Jev classifier.
- If the behavior needs a more open-ended judgment, use an LLM judge.
