...
<Tabs>
  <Tab title="Keyring (recommended)">
    Store each key once in your operating system's secret store (Keychain on MacOS, Secret Service on Linux, Credential Manager on Windows):
    ...
    Each command prompts for the corresponding API key. The CLI checks the `lat_sandbox_` prefix before saving a sandbox key, so a live key cannot be stored in the sandbox slot by mistake. Manage either credential with:

    ```bash
    latitude auth status --env sandbox
    latitude auth logout --env sandbox
    ```
  </Tab>
  <Tab title="Environment variable">
    ...
    Without an environment selector, `LATITUDE_API_KEY` takes precedence over the legacy keyring entry. This keeps existing scripts and single-key setups working.
  </Tab>
</Tabs>

Set a project once, then switch environments with one variable. Explicit environment selection always uses the matching saved credential, even when `LATITUDE_API_KEY` is inherited from the shell or a `.env` file:

```bash
export LATITUDE_PROJECT_SLUG=my-app

LATITUDE_SANDBOX=false latitude traces list   # saved production key
LATITUDE_SANDBOX=true latitude traces list    # saved sandbox key
```

If the selected credential is missing, the command stops and prints the matching `auth login --env ...` command. It never falls back to the other environment. An explicit `--project-slug` overrides `LATITUDE_PROJECT_SLUG`.
...
| Variable | Purpose |
| --- | --- |
| `LATITUDE_API_KEY` | API key used when `LATITUDE_SANDBOX` is not set (alternative to the legacy keyring entry). |
| `LATITUDE_PROJECT_SLUG` | Default value for `--project-slug`. |
| `LATITUDE_SANDBOX` | `true` selects the saved sandbox key; `false` selects the saved production key. |
