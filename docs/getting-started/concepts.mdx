...
└─ Refine
   ...
   └─ Datasets and regression tests that lock a fix in CI
```
...
8. **Datasets** built from real traces feed regression tests in CI, so a fix keeps passing the failures it fixed; if the failure returns in production anyway, the resolved signal reopens as regressed.
