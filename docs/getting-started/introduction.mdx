...
- **Evaluate**: Generate monitors from discovered issues. Evaluations run continuously on live traffic, catching regressions before users notice.
...
- **Simulate**: Before shipping changes, run your agent against test scenarios locally or in CI. Reuse the same evaluations that monitor production.
...
| **Evaluation**    | An automated monitor that tracks behaviour across traces and produces scores               |
| **Issue**         | A named failure pattern discovered by grouping similar failed scores                       |
| **Annotation**    | A verdict on a trace from a human, a flagger, or the API, attached as a score              |
| **Flagger**       | A built-in automatic annotator for a known failure category like _jailbreaking_ or _frustration_ |
| **Search**        | A hybrid keyword + semantic search over trace conversations, with optional saved filters   |
| **Saved Search**  | A named bookmark for a search query plus filters, with assignee and review progress        |
