...
<Warning>
  The MCP reaches your **live** organizations only. An OAuth token binds to an organization you're a member of, and [sandboxes](/observability/features/sandbox) aren't organizations you join — so they never appear in the consent screen. To let an agent investigate a sandbox trace, give it the [CLI](/getting-started/cli) with that sandbox's `lat_sandbox_` key instead.
</Warning>
