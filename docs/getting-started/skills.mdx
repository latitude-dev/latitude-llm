...
Once traces are flowing, head to the [telemetry guides](/telemetry/start-tracing) for provider-specific details, the [CLI](/getting-started/cli) and [MCP server](/getting-started/mcp) pages to keep driving Latitude from your terminal or agent, or [Artifacts](/more/artifacts) to turn that data into reports and dashboards without opening the web app.
