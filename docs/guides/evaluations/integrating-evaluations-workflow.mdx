...
- **Choose Evaluation Types**: Select the right mix of [LLM-as-Judge](/guides/evaluations/llm_as_judge_evaluations), [Programmatic Rules](/guides/evaluations/programmatic_rules), and [Manual Evaluations](/guides/evaluations/manual_evaluations) to cover your criteria.
...
- **Draft Evaluations**: Run batch evaluations on your golden dataset _before_ merging changes from a draft version.
- **Peer Review**: Include evaluation results (especially for failing cases) as part of the review process for prompt changes.

## 4. Pre-Deployment Checks (CI/CD Integration)

- **Automated Batch Evaluations**: Trigger batch evaluations against the golden dataset as part of your CI/CD pipeline before deploying a new prompt version.
- **Quality Gates**: Fail the deployment if key evaluation metrics drop below your acceptance criteria.
- **API/SDK Integration**: Use the Latitude [API](/guides/api/api-access) or [SDKs](/guides/sdk/typescript) to programmatically trigger evaluations and check results within your pipeline.

## 5. Continuous Monitoring in Production

- **Live Evaluations**: Enable [live evaluations](/guides/evaluations/running-evaluations#running-evaluations-continuously-live-mode--ongoing) for critical metrics (e.g., format validation, safety checks, basic relevance) to monitor real-time performance.
- **Regular Batch Runs**: Schedule periodic batch evaluations on recent production logs to track trends and catch subtle drifts in quality.
- **Alerting**: Set up alerts (potentially using webhooks or monitoring tools integrated with Latitude) for significant drops in evaluation scores.
