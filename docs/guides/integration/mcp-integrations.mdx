---
title: Third-Party Integrations (MCP)
description: Connect Latitude to other tools and services using Model Coordination Protocol (MCP) servers.
---

# Third-Party Integrations (MCP Servers)

Latitude supports extending its capabilities by integrating with external tools and services through servers implementing the **Model Coordination Protocol (MCP)**. This allows you to connect Latitude prompts to systems like Slack, Notion, databases, or custom internal tools.

## What is MCP?

MCP (Model Coordination Protocol) is a standardized way for AI platforms like Latitude to interact with external tools or "skills". An MCP server acts as a bridge:
...
1.  **Choose a Framework**: Use any web server framework (e.g., Node.js/Express, Python/Flask, Go/Gin).
