...
3.  **Automatic Validation**: Latitude automatically validates the model's response against the schema. If the response doesn't match, an error may be flagged or retries might occur, depending on the provider and settings.
