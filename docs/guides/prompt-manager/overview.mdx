...
Make your prompts dynamic by using variables:

- **Input Parameters**: Define placeholders like `{{ topic }}` in your prompt. These are filled in via the Playground or API calls.
- **Internal Variables**: Use `<var>` tags to define variables within your prompt logic.
...
- **Comments (Coming Soon)**: Add comments directly within the editor for feedback and discussion.

## Video Tutorial: Prompt Editor Tour

Watch this 3-minute tour to see the Prompt Editor in action:

<iframe
  width='100%'
  height='500px'
  src='https://www.youtube.com/embed/EXAMPLE_ID'
  title='Prompt Editor Tour'
  frameborder='0'
