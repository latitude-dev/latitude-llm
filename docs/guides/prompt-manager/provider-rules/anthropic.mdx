...
### Rule 1: System messages must be followed by at least another message
...
Instead, add another message wrapping it in a `<user>` or `<assistant>` tag:
...
### Rule 2: No system messages are allowed after an assistant or user message
