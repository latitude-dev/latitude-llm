...
- [Using Roles in Your Prompts](#using-roles-in-your-prompts)
...
User messages can include images. You can use the `<image>` tag to include an image in the user role. The image tag must contain either a path to the image or its base64 encoded content. Example:

```
<user>
    <image>https://example.com/image.jpg</image>
    Here is the image I'd like to ask about.
</user>
```

Note: Only models with multimodal capabilities – like `gpt-4o` – can process images in prompts.
