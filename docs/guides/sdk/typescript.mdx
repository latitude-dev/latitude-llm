...
You can create, retrieve, update, and list prompts using the SDK.
...
async function createNewPrompt() {
  try {
    const newPrompt = await latitude.prompts.create({
      name: 'Marketing Email Generator',
      // Use PromptL syntax for content
      content: `
        ---
        provider: openai
        model: gpt-4o
        temperature: 0.8
        ---
        <system>You are an expert marketing copywriter.</system>
        <user>Write a short, engaging marketing email for our new product: {{productName}} targeting {{audience}}.</user>
      `,
      // Optional: Link to a project if needed (usually inferred from API key)
