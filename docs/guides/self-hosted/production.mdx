...
- **Storage Configuration**:
  - `DRIVE_DISK`: Choose between `local` or `s3` for file storage
    ...
    - AWS S3 with IAM Roles:
      Credentials are resolved through the AWS SDK default credential chain.
      Use IAM roles attached to your AWS services (ECS, EC2, Lambda).
      Ensure the AWS resource has proper IAM role permissions for S3 access (GetObject, PutObject, DeleteObject).
