...
The first time you start the services, the database will be initialized but you
will need to manually run the database migrations in order to set up all tables
and relations appropiately. To do so, first install dependencies in your host machine:
...
```bash
cd packages/core && DATABASE_URL=postgresql://<DB_USERNAME>:<DB_PASSWORD>@localhost:5432/<DB_NAME> pnpm db:migrate
```

This will create the necessary tables and relations in the database. This
process needs only be done once.
...
4. For a more robust production environment, make sure to:
