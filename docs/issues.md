...
Issue discovery uses the existing Temporal-backed workflow abstraction in `apps/workflows`, while queue tasks in `@domain/queue`, `@platform/queue-bullmq`, and `apps/workers` continue to dispatch the upstream single-step triggers, including the rate-limited `issues:refresh` task.
...
- `issues:refresh` as a rate-limited single-step task for one issue whose name/description rate-limit window has elapsed
...
- rate-limited issue refresh relies on the `issues:refresh` queue task with logical `dedupeKey` + `rateLimitMs`, not on implicit BullMQ delayed/repeat jobs or persisted due-work scans — first publish schedules fire; subsequent publishes within the window are dropped
- `ScoreAssignedToIssue` is the trigger for later existing-issue detail regeneration; the dispatcher publishes `issues:refresh` keyed by the canonical issue id with the configured eight-hour rate-limit window (at most one refresh per issue per 8h, fires at most 8h after the first assignment)
...
- `issues:refresh` runs after the configured rate-limit window elapses for an existing issue
...
- rerank limit: `100`
- issue details regeneration rate-limit: `8 hours` (at most once per issue, fires at most 8h after the first assignment)
...
- once created, automatic rate-limited realignment continues as new annotations arrive: each new annotation writes `ScoreAssignedToIssue`, which the `domain-events` dispatcher routes to `issues:refresh` (rate-limited at 8h), which in turn publishes `evaluations:automaticRefreshAlignment` (rate-limited at 1h, one per active linked evaluation) to kick off `refresh-evaluation-alignment`; that workflow escalates into `optimize-evaluation` via `evaluations:automaticOptimization` (rate-limited at 8h) when the incremental MCC drop exceeds tolerance. All windows are first-publish-wins so a continuous annotation stream cannot push the fire time forward indefinitely
