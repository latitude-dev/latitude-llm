...
2. **From a saved search**: open the saved-search dropdown on the Sessions or Traces page. Each saved search offers **Create monitor** (or **View monitor** if one already watches it), which opens the same form with that search pre-selected.

<Frame caption="The saved-search dropdown: each saved search offers a Create monitor action that opens the form with that search pre-selected.">
  <img src="/images/monitors/create-monitor-from-saved-search.png" alt="The saved-search dropdown with a Create monitor action on a saved-search row" />
</Frame>
