...
The page opens with four KPIs for the selected time range: **Total spend** on billable LLM calls, **Avg per day** over completed days, **Avg per trace** across traces that contain at least one billable call, and the **Top spend model** with its share of spend. The **Cost over time** chart below shows spend per bucket, stacked by model on the **Total** tab, while the **Average** and **p95** tabs plot per-trace cost, so you can separate a traffic increase from an expensive tail.
