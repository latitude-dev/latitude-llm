...
- identify token-heavy behaviors in one product area
- investigate high TTFT for one model/provider cohort
- combine outlier badges with search to discover behavioral causes

For example, filter to `p95` duration traces, then search for _agent loops between tools_ to find looping behavior behind slow traces.
...
- [Search](../../search/overview): Discover behaviors inside outlier cohorts
