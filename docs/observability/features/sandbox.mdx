...
```bash
# `LATITUDE_API_KEY` beats the keyring, so this switches environment per command
...
- Your **live** key reads only live data. Because a sandbox mirrors your project slugs, the same query that works in the sandbox returns an empty page under a live key — not an error. If your sandbox traces look invisible from the API, check the key prefix first.
- Only the **trace-level** surface returns data (traces, sessions, and spans). The monitoring endpoints — searches, signals, monitors, evaluations, annotations — answer empty, because those features don't run on sandbox traces.
