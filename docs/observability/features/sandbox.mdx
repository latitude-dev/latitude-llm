...
The same `lat_sandbox_` key also **reads** the sandbox over the [API](https://api.latitude.so/docs) and the [CLI](/getting-started/cli). Save the production and sandbox keys once, then select the sandbox for each command:

```bash
latitude auth login --env production
latitude auth login --env sandbox

export LATITUDE_PROJECT_SLUG=my-app

LATITUDE_SANDBOX=true latitude traces list --format json
LATITUDE_SANDBOX=true latitude traces get --trace-id <traceId> --format json
```

`LATITUDE_SANDBOX=true` ignores an inherited `LATITUDE_API_KEY` and uses only the saved sandbox credential. If that credential is missing, the command fails before making a request. Use `LATITUDE_SANDBOX=false` for production. An explicit `--project-slug` overrides `LATITUDE_PROJECT_SLUG` when a command needs a different project.
