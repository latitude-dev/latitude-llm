...
```bash
# `LATITUDE_API_KEY` beats the keyring, so this points the shell at the sandbox
export LATITUDE_API_KEY=lat_sandbox_...

latitude traces list --project-slug my-app --format json
latitude traces get --project-slug my-app --trace-id <traceId> --format json
```
