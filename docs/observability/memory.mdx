...
Everything the Memory page reads is also exposed to your own tools and coding agents over the REST API, the MCP server, the SDKs, and the CLI: list stores, read a store's snapshot and a record's history, fetch a change's diff, and read a session's or trace's memory footprint. See the [API reference](../more/api-reference).
