...
- issue name/description regeneration for existing issues runs through the rate-limited `issues:refresh` task keyed by the canonical issue id (at most once per 8h per issue; first publish wins)
...
- unchanged scripts can refresh alignment incrementally before a full re-optimization run; annotation-driven automatic refresh flows through the rate-limited `evaluations:automaticRefreshAlignment` queue task (1h, first-publish-wins) into `refresh-evaluation-alignment`, which on a `full-reoptimization` outcome publishes `evaluations:automaticOptimization` (8h, first-publish-wins) to start `optimize-evaluation`; the rate-limit (not debounce) semantics bound worst-case latency at 1h / 8h and cap fires at once per window per evaluation even under a constant annotation stream; manual refresh still runs through the `evaluation-alignment` workflow until it is migrated onto `optimize-evaluation` in a follow-up change
