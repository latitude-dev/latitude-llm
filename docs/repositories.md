...
- **TraceRepository** / **SessionRepository** — `listByProjectId`, `listByTraceIds` (trace only), `countByProjectId`, `aggregateMetricsByProjectId` (zero-filled when no rows), `histogramByProjectId` (trace only), `distinctFilterValues`; **TraceRepository** `findByTraceId` → `NotFoundError` if missing.
