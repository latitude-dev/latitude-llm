...
<Frame>
  <img
    ...
    alt="The Traces page with a search query active: the search bar, summary metrics, and matching results"
  />
</Frame>
...
<Info>
  Semantic search is an **exploration** tool. It returns a ranked sample of the most relevant traces, not every match, so it cannot be saved, counted, or alerted on. To revisit or monitor a behaviour, capture it with filters or exact text and save that.
</Info>
...
<Frame>
  <img
    ...
    alt="Search results on the Traces page with the filters panel open alongside the query"
  />
</Frame>
...
When filters, an exact-text search, or both become useful enough to revisit, click **Save search** and give it a clear name, such as _Failed payments_ or _Tool loops in production_.

A saved search captures your **filters and exact-text query**. It does not save a semantic, natural-language query: semantic search returns a ranked sample for exploration, not every match, so it is not something you can save or monitor. To keep a behaviour you found by meaning, express it with filters or exact text (a tag, a metadata field, an error string) and save that.
