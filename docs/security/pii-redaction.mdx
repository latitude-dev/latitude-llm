...
| Crypto wallet addresses | Off | Bitcoin addresses passing the address checksum, and Ethereum addresses |

The last two are off by default because they also match things that are not personal data: version strings such as `1.2.3.4` look like IPv4, and a 40-character hex string is both an Ethereum address and a commit hash. Every category can be toggled individually.
...
- **A 16-digit numeric identifier beginning with 4 or 5 has roughly a one-in-ten chance** of passing the card checks and being redacted. That is inherent to Luhn plus an issuer prefix.
