...
Your telemetry sends real user and agent interactions into Latitude as traces. These traces are the raw material for signals.

### 2. Verdict sources produce scores

Latitude uses several sources to decide whether a trace represents good or bad behavior:
...
As new annotations, flagger matches, and scores arrive, Latitude can realign the evaluation so it stays calibrated to the latest input.
...
A signal can be in multiple states at once. For example, a newly discovered signal can also be escalating.
