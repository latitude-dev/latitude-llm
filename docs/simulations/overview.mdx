...
3. Runs **evaluations** against those traces
...
Simulations reuse the same evaluations that monitor production traffic. This means the quality bar in testing matches the quality bar in production.
...
5. The CLI runs configured **evaluations** against each trace
...
The key insight behind simulations: **they reuse your production evaluations**. The same monitor that tracks an issue in production also checks for that issue in your simulation.

This ensures:

- Test quality standards match production quality standards
- New evaluations generated from production signals automatically become test checks
- There's no separate "test evaluation" system to maintain

## Related

- [Evaluations](../evaluations/overview): The evaluations that power simulations
