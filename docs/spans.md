...
- debounced downstream tasks (`live-evaluations:enqueue`, `live-annotation-queues:curate`, `system-annotation-queues:fanOut`) published directly from the `SpanIngested` handler with a debounce window defined by a named constant whose initial default is `90 seconds`
...
- when the debounce window elapses, each downstream task executes directly — there is no intermediate event or worker hop
