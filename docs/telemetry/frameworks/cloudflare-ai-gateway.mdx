...
<Steps>
  ...
  <Step title="Add a Latitude exporter">
    Add an OTLP exporter with the following URL and headers:
    ...
    `<PROJECT_SLUG>` with your project's slug. AI Gateway supports both OTLP/JSON and
    OTLP/protobuf; either works.

    <Note>
      To keep the API key out of the exporter config, store it in Cloudflare
      [Secrets Store](https://developers.cloudflare.com/secrets-store/) and reference it from the
      exporter's authorization field.
    </Note>
  </Step>
