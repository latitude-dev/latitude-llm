...
Think's codemode `execute` tool appears as a normal AI SDK tool span. If you
also want the tools called from inside codemode to appear as child spans under
that `execute` span, wrap both the codemode tool set and the outer execute tool
with `createCodemodeTelemetry()`.

```ts
import { createExecuteTool } from "@cloudflare/think/tools/execute"
import { createCodemodeTelemetry } from "@latitude-data/telemetry/codemode"
    ...
    return {
      execute: codemode.wrapExecuteTool(
        createExecuteTool(this, {
          tools: codemode.traceToolSet({ getWeather, estimateTripBudget }),
        }),
      ),
    }
  }
