...
For a comparison of voice observability across providers, see [Voice agents](/telemetry/voice-agents). If you need full **STT → LLM → TTS** tracing, consider [LiveKit Agents](/telemetry/frameworks/livekit) or a self-hosted [Vercel AI SDK v7](/telemetry/frameworks/vercel-ai-sdk-v7) pipeline instead.
...
<Warning>
  **STT and TTS are not traced.** ElevenLabs handles speech recognition and
  synthesis on its own infrastructure. Latitude only sees the LLM calls that flow
  through your Custom LLM proxy — not the audio pipeline before or after.
</Warning>
