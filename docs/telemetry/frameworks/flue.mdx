...
<Check>
  You'll keep building Flue workflows exactly as you do today. Latitude only adds
  an OpenTelemetry export path for the spans Flue already emits.
</Check>
...
const redactLogMessage = (message: string) => message.replace(/[\w.-]+@[\w.-]+/g, "[REDACTED_EMAIL]")
const redactLogAttributes = (attributes: Record<string, unknown> | undefined) =>
  Object.fromEntries(Object.entries(attributes ?? {}).filter(([key]) => !key.toLowerCase().includes("secret")))
