...
LiveKit is Latitude's **turnkey voice-agent integration**: STT, LLM, and TTS all run in one pipeline with native tracing. See the [Voice agents](/telemetry/voice-agents) guide for how this compares to [Vercel AI SDK v7](/telemetry/frameworks/vercel-ai-sdk-v7) and [ElevenLabs](/telemetry/frameworks/elevenlabs).
...
LiveKit also emits spans for speech-to-text, text-to-speech, and voice-activity detection. To include the full **STT → LLM → TTS** pipeline in every trace, disable the smart filter:

<Note>
  By default the span processor forwards only LLM-relevant spans. Pass
  `disable_smart_filter=True` (Python) or `disableSmartFilter: true` (TypeScript)
  to export STT, TTS, and VAD spans as well:
