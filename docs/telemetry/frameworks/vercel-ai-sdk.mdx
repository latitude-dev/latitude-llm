...
<Steps>
  ...
  <Step title="Initialize and use">
    Initialize Latitude **without** an `instrumentations` array. The Vercel AI SDK emits its own OpenTelemetry spans. Pass `latitude.getAiSdkTelemetry()` to each AI SDK call.
    ...
    <Note>
      `latitude.getAiSdkTelemetry()` passes a tracer from the provider Latitude actually exports
      from, so it always routes spans correctly. When Latitude owns the global provider or attaches
      its processor to an existing one, a plain `experimental_telemetry: { isEnabled: true }` works
      too — but `getAiSdkTelemetry()` is also what keeps traces flowing in the fallback case where
      Latitude logged that it could not attach to an already-registered provider. To wire the tracer
      yourself instead, use `experimental_telemetry: { isEnabled: true, tracer: latitude.getTracer("vercelai") }`.
    </Note>
  </Step>
