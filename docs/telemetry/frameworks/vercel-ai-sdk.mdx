...
<Steps>
  ...
  <Step title="Initialize and use">
    Initialize Latitude **without** an `instrumentations` array. The Vercel AI SDK emits its own OpenTelemetry spans. Pass `latitude.getAiSdkTracer()` as the AI SDK tracer on each call.
