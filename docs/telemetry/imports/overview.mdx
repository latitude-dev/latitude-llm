...
keywords: ["import", "imports", "backfill", "migrate", "migrating", "migration", "historical traces", "langfuse", "langsmith", "braintrust"]
---
...
Latitude can import the traces you already have in another observability platform, a
one-time backfill so you do not start from an empty project when you migrate. For the
whole migration path, live tracing included, see [Migrate to Latitude](./migrate). Imports run from **Project settings →
