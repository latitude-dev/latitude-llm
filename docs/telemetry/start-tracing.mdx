...
<Note>
  ...
  instead: it covers this setup plus porting your context, backfilling your existing traces
  with a one-time [import](./imports/overview), and cutting over.
</Note>
...
<Note>
  The coding agent that receives the prompt performs the setup. It is not necessarily what
  Latitude will trace. For example, pasting the application prompt into Hermes instruments the
  application in the current repository; it does not trace Hermes's own conversations. To trace
  Hermes itself, use the [Hermes telemetry guide](./hermes).
</Note>
...
<Steps>
  <Step title="Ask your agent to install tracing">
    ...
    ```text
    Install the `latitude-setup` skill from `github.com/latitude-dev/skills` and use it to add Latitude tracing to the application in this repository, following best practices.
    ```
...
- Already have traces elsewhere? [Import them as a one-time backfill](./imports/overview) so you do not start from an empty project
