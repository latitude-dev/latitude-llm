...
System queues evaluate each incoming trace after it completes. A per-queue **sampling** check runs first — only sampled-in traces are evaluated further. Then:

- **Tool Call Errors** and **Resource Outliers** use **deterministic rules** (no LLM needed)
- The remaining queues use a lightweight LLM-based classifier with limited conversation context
