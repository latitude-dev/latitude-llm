...
description: Connect your Amazon Bedrock-powered application to Latitude Telemetry for observability in Latitude V2.
---
...
- Every Amazon Bedrock call (e.g. `invokeModel`) will appear as a trace in Latitude.
- Traces are grouped inside a Latitude **project** and identified by a `path`.
- You can inspect inputs/outputs, measure latency, score, and debug Amazon Bedrock-powered features from the Latitude dashboard.
...
Each Amazon Bedrock call appears as a child span under the capture, giving you a full view of what happened.
