---
title: Ollama
description: Connect your Ollama-powered application to Latitude Telemetry for observability in Latitude V2.
---
...
- Every Ollama call (e.g. `chat`, `generate`) will appear as a trace in Latitude.
- Traces are grouped inside a Latitude **project** and identified by a `path`.
- You can inspect inputs/outputs, measure latency, score, and debug Ollama-powered features from the Latitude dashboard.
...
<Steps>
  ...
  <Step title="Wrap your Ollama-powered feature">
    Initialize Latitude Telemetry and wrap the code that calls Ollama using <code>telemetry.capture</code>.
...
Each Ollama call appears as a child span under the capture, giving you a full view of what happened.
