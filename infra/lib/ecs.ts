...
function createTaskDefinition(
  ...
  const containerDefinitions = pulumi
    .all([
      ...
      secrets["resend-api-key"].arn,
      secrets["resend-from"].arn,
    ...
    .apply(
      ([
        ...
        const baseEnvironment: { name: string; value: string }[] = [
          ...
          { name: "LAT_EMAIL_PROVIDER", value: "mailgun" },
        ...
        const baseSecrets: { name: string; valueFrom: string }[] = [
          ...
          { name: "LAT_RESEND_API_KEY", valueFrom: resendApiKeyArn },
          { name: "LAT_RESEND_FROM", valueFrom: resendFromArn },
