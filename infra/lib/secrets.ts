...
export function createApplicationSecrets(baseName: string, environment: string): SecretsOutput {
  ...
  const jevApiKey = createSingleSecret(
    baseName,
    "jev-api-key",
    "Jev API key for flagger preclassifier — replace placeholder-change-me in Secrets Manager",
    process.env.LAT_JEV_API_KEY ?? "placeholder-change-me",
    environment,
    immutableSecretResourceOptions,
  )
  secrets["jev-api-key"] = jevApiKey.secret
  secretVersions["jev-api-key"] = jevApiKey.secretVersion
