...
export class InitCommand extends BaseCommand {
  ...
  async execute(options: InitOptions): Promise<void> {
    try {
      ...
      // Get API key and create client (check if one already exists)
      const apiKey = await this.getOrPromptForApiKey()
      this.client = this.createLatitudeClient(apiKey)

      // Only detect module format for npm projects
      if (this.isNpmProject) {
        await this.detectModuleFormat()
        await this.detectTypeScriptProject()
      }
      ...
      await this.configManager.setProjectId(projectId)
  ...
  private async getOrPromptForApiKey(): Promise<string> {
    ...
    const existingApiKey = await this.configManager.getApiKey()

    if (existingApiKey) {
      console.log('Using existing API key from configuration')
  ...
  private async useExistingProject(): Promise<number> {
    ...
    // Create lock file
    await this.createLockFile(projectId, promptsRoot)
   ...
   */
  private async detectTypeScriptProject(): Promise<void> {
    try {
      ...
      try {
        ...
        // tsconfig.json not found, ask user
        const { isTypeScript } = await inquirer.prompt([
          {
            type: 'confirm',
            name: 'isTypeScript',
        ...
        this.isTypeScriptProject = isTypeScript
        console.log(
          `Project identified as ${isTypeScript ? 'TypeScript' : 'JavaScript'} based`,
        )
      }
    } catch (error: any) {
      console.warn(
        `Warning: Could not detect TypeScript status: ${error.message}`,
      )
  ...
  private async createLockFile(
    ...
    try {
      ...
      const lockFile: LatitudeLockFile = {
        ...
        typescript: this.isTypeScriptProject,
      }
