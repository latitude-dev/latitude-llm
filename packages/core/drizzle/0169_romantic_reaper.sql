CREATE TABLE IF NOT EXISTS "latitude"."spans" (
	...
	CONSTRAINT "spans_metadata_uuid_unique" UNIQUE("metadata_uuid")
);
...
--> statement-breakpoint
DO $$ BEGIN
 ALTER TABLE "latitude"."spans" ADD CONSTRAINT "spans_parent_id_spans_id_fk" FOREIGN KEY ("parent_id") REFERENCES "latitude"."spans"("id") ON DELETE cascade ON UPDATE no action;
EXCEPTION
 WHEN duplicate_object THEN null;
END $$;
--> statement-breakpoint
...
CREATE INDEX IF NOT EXISTS "spans_metadata_uuid_idx" ON "latitude"."spans" USING btree ("metadata_uuid");--> statement-breakpoint
