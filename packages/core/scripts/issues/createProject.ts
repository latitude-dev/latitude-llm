...
async function main() {
  ...
  if (existingProject.length > 0) {
    console.log('📁 Project already exists, fetching existing data...')
...
Maintain high standards for accuracy and objectivity in all research activities.`,
  ...
  console.log(
    `📊 Found ${existingTitles.size} existing issues, will avoid duplicate titles...`,
  )

  console.log('📊 Creating issues and histograms...')
  await createIssuesAndHistograms(projectData, existingTitles)
...
- Updated ${i === 1 ? 'statistical' : i === 2 ? 'interaction' : 'documentation'} standards`,
    ...
    for (const change of changes) {
      ...
      const existingDoc = projectData.documents.find(
        (doc) => doc.path === change.path,
      )
...
async function createIssuesAndHistograms(
  ...
  const documentUuids = projectData.documents.map(
    (doc: any) => doc.documentUuid,
  )
  ...
  while (createdCount < issueCount && attempts < maxAttempts) {
    ...
    if (status === 'resolved') {
      await database
        .update(issues)
        .set({
          resolvedAt: faker.date.between({ from: lastSeenAt, to: today }),
        })
...
function generateHistogramData(
  ...
  for (
    ...
    const multiplier =
      Math.random() < 0.1 ? faker.number.int({ min: 3, max: 8 }) : 1
