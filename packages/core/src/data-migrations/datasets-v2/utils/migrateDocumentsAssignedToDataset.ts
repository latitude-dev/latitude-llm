...
async function getDatasetV2Metadata({
  ...
  const prevMappedColumns = Object.keys(info.mappedInputs) ?? []
  const mappedInputs = prevMappedColumns.reduce(
    (acc, key) => {
      const columnIdentifier = columns[key]
      if (!columnIdentifier) return acc

      acc[key] = columnIdentifier
      return acc
    },
    {} as Record<string, string>,
  )

  const rows = info.rowIndex
    ? await rowsRepo.findByDatasetWithOffsetAndLimit({
        datasetId: dataset.id,
        offset: info.rowIndex,
        limit: 1,
      })
    : []
