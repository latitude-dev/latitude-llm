...
import { and, desc, eq, exists, gte, isNotNull, isNull, notExists, sql } from 'drizzle-orm'
import {
...
export const requestDocumentSuggestionsJob = async (_: Job<RequestDocumentSuggestionsJobData>) => {
  const mergedCommits = database.$with('merged_commits').as(
    database
      ...
      .where(
        and(isNull(projects.deletedAt), isNull(commits.deletedAt), isNotNull(commits.mergedAt)),
      ),
  )
    ...
    database
      ...
      .where(
        and(
          ...
          eq(evaluationResultsV2.evaluationUuid, liveEvaluations.evaluationUuid),
    ...
    database
      ...
      .where(
        and(
          ...
          eq(documentSuggestions.evaluationUuid, liveEvaluations.evaluationUuid),
          gte(
  ...
  for (const candidate of candidatesV2) {
    await documentSuggestionsQueue.add('generateDocumentSuggestionJob', candidate, {
      attempts: 1,
      deduplication: {
