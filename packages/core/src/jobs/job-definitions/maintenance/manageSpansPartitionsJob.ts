...
export type ManageSpansPartitionsJobData = Record<string, never>
...
/**
 * Daily job that manages weekly partitions for spans:
 ...
 */
export async function manageSpansPartitionsJob(
  _: Job<ManageSpansPartitionsJobData>,
) {
...
async function checkDefaultPartition() {
  ...
  if (hasRows) {
    captureException(
      new Error(
        'DEFAULT partition for spans contains rows. ' +
          'This indicates rows with started_at outside any defined partition range.',
      ),
    )
  }
}
