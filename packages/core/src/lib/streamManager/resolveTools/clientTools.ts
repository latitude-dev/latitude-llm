...
function buildDefinition(streamManager: StreamManager) {
  return ([name, toolDefinition]: ToolTuple) => {
    ...
    if (toolHandler) {
      definition.execute = instrumentToolHandler({
        context: streamManager.$context,
        toolName: name,
        toolHandler: decorateToolHandler({
          context: streamManager.$context,
...
/**
 * Decorates a tool handler with some extra props to pass to the handler
 * function (e.g. telemetry context or tool definition)
 */
function decorateToolHandler({
  toolDefinition,
  context,
  toolHandler,
}: {
  ...
  return (args: any, toolCall: ToolExecutionOptions) =>
    toolHandler({
      toolDefinition,
      toolCall,
      context,
...
/**
 * Instruments a tool handler execution with OTL telemetry
 */
function instrumentToolHandler({
  ...
  context: TelemetryContext
  toolName: string
  toolHandler: (args: any, toolCall: ToolExecutionOptions) => Promise<any>
}) {
  return async (args: any, toolCall: ToolExecutionOptions) => {
    ...
    try {
      const result = await toolHandler(args, toolCall)

      $span?.end({ result: { isError: false, value: result } })
      return result
    } catch (error) {
