...
export class ExperimentsRepository extends Repository<Experiment> {
  ...
  private experimentDtoPresenter = (
    ...
    const completedEvals = row.passedEvals + row.failedEvals + row.evalErrors
    ...
    return {
      ...
      results: {
        ...
        passed: row.passedEvals,
        failed: row.failedEvals,
        errors: row.evalErrors + row.evaluationUuids.length * row.logErrors,
