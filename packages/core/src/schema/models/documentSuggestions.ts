import { bigint, bigserial, foreignKey, index, text, uuid } from 'drizzle-orm/pg-core'
...
export const documentSuggestions = latitudeSchema.table(
  'document_suggestions',
  {
    ...
    evaluationId: bigint('evaluation_id', { mode: 'number' }).references(() => evaluations.id, {
      onDelete: 'cascade',
  ...
  (table) => ({
    workspaceIdIdx: index('document_suggestions_workspace_id_idx').on(table.workspaceId),
    documentVersionsFk: foreignKey({
      columns: [table.commitId, table.documentUuid],
      foreignColumns: [documentVersions.commitId, documentVersions.documentUuid],
      name: 'document_suggestions_document_versions_fk',
    }).onDelete('cascade'),
    commitIdIdx: index('document_suggestions_commit_id_idx').on(table.commitId),
    documentUuidIdx: index('document_suggestions_document_uuid_idx').on(table.documentUuid),
    evaluationIdIdx: index('document_suggestions_evaluation_id_idx').on(table.evaluationId),
    createdAtIdx: index('document_suggestions_created_at_idx').on(table.createdAt),
  }),
)
