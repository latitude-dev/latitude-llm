import { bigint, bigserial, index, jsonb, timestamp, uuid, varchar } from 'drizzle-orm/pg-core'

import { latitudeSchema } from '../db-schema'
import { timestamps } from '../schemaHelpers'
import { commits } from './commits'
import type { ExperimentMetadata } from '@latitude-data/constants'
...
export const experiments = latitudeSchema.table(
  'experiments',
  {
    ...
    uuid: uuid('uuid').notNull().default(sql`gen_random_uuid()`).unique(),
    ...
    evaluationUuids: uuid('evaluation_uuids').array().notNull().default(sql`'{}'::uuid[]`),
  ...
  (table) => ({
    ...
    documentUuidIdx: index('experiments_document_uuid_idx').on(table.documentUuid),
