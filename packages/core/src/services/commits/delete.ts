...
export async function deleteCommitDraft(commit: Commit, transaction = new Transaction()) {
  ...
  return transaction.call<Commit>(async (tx) => {
    try {
      const projectCommits = await unsafelyFindCommitsByProjectId(commit.projectId, tx)
      if (projectCommits.length === 1) {
        return Result.error(new BadRequestError('Cannot delete the only version in a project'))
      }
      ...
      if (error instanceof DatabaseError && error.code === databaseErrorCodes.foreignKeyViolation) {
