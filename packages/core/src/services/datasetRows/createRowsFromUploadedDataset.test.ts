...
describe('createRowsFromUploadedDataset', () => {
  beforeAll(async () => {
    ...
    const { file } = await createTestCsvFile({ disk: testDrive })
  ...
  afterAll(async () => {
    await testDrive.deleteAll(`workspaces/${workspace.id}`)
  })

  it('should create rows from an uploaded dataset', async () => {
    const event = {
      type: 'datasetV2Created' as DatasetV2CreatedEvent['type'],
      data: {
    ...
    const rows = await repo.findByDataset(dataset.id)
