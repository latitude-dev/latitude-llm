import type { DocumentTriggerType, DocumentVersion } from '@latitude-data/constants'
import type { Commit, DocumentTrigger, Project, Workspace } from '../../browser'
...
export async function createDocumentTrigger<T extends DocumentTriggerType = DocumentTriggerType>(
  {
    workspace,
    project,
    commit,
  ...
  if (commit.mergedAt) {
    return Result.error(new BadRequestError('Cannot create document trigger in a merged commit'))
  }
