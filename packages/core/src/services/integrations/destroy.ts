...
export async function destroyIntegration(
  ...
  return transaction.call(async (trx) => {
    ...
    if (integration.type === IntegrationType.HostedMCP && integration.mcpServerId) {
      // Get the MCP server
      const mcpServerRepo = new McpServerRepository(integration.workspaceId, trx)
      mcpServer = await mcpServerRepo.find(integration.mcpServerId).then((r) => r.unwrap())
    }
    ...
    await destroyPipedreamAccountFromIntegration(integration).then((r) => r.unwrap())
