...
export async function listIntegrationReferences(
  ...
  const documentToolsResult = await toolIntegrationsScope.getAllActive()
  if (!Result.isOk(documentToolsResult)) return documentToolsResult
  const documentTools = documentToolsResult.unwrap()
