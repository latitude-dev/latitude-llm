...
export async function unignoreIssueEvaluations(
  { issue }: { issue: Issue },
  ...
  await db
    ...
    .set({
      ignoredAt: null,
      configuration: sql`${evaluationVersions.configuration} || jsonb_build_object('trigger', jsonb_build_object('mode', ${sql.raw(`'${EvaluationTriggerMode.EveryInteraction}'`)}))`,
      updatedAt: now,
    })
