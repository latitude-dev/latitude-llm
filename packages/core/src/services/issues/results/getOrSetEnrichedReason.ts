 ...
 * @param db - Optional database instance to use for updates (useful when inside a transaction)
...
async function generalizeReason({
  ...
  const response = await runCopilot<
    z.ZodObject<{ reasoning: z.ZodString; generalized_annotation: z.ZodString }>
  >({
