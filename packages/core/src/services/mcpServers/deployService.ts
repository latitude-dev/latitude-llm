    ...
    return Transaction.call(async (tx) => {
      const k8sApp = await tx
        ...
        .values({
          workspaceId,
          authorId,
          name: appName, // Store the original name for display purposes
          uniqueName: uniqueAppName, // Store the unique name with hash for k8s resources
          status: 'deploying',
          // TODO: encrypt environment variables as they could be sensitive
