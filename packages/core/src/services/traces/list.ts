...
import type { Project, SearchFilter, Span } from '../../browser'
...
export async function listTraces({
  ...
  let where
  try {
    where = buildWhereClause({ filters, project })
  } catch (e) {
...
function buildWhereClause({
  ...
  const conditions = [eq(traces.projectId, project.id)]
