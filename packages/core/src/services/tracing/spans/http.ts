...
async function process(
  ...
  const requestMethod = extractingqm.value.toUpperCase()
  ...
  const requestHeaders = toCamelCase(extractingqh.value)
  ...
  const responseHeaders = toCamelCase(extractingsh.value)
...
function extractRequestMethod(
  ...
  if (method) return Result.ok(method)
...
function extractResponseStatus(
  ...
  const status = Number(attributes[ATTR_HTTP_RESPONSE_STATUS_CODE] || '')
