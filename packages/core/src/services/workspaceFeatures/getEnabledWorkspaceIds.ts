...
export async function getEnabledWorkspaceIdsForFeature(featureId: number, db = database) {
  const enabledWorkspaces = await db
    .select({
      workspaceId: workspaceFeatures.workspaceId,
    })
    .from(workspaceFeatures)
    .where(and(eq(workspaceFeatures.featureId, featureId), eq(workspaceFeatures.enabled, true)))
