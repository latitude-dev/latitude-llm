 ...
 * fields (value, passed, feedback, anchor) and replaces the flat `traceId`
 * with the `trace` discriminated union.
 *
 * Stripped from the accepted body:
 *   - `id` — the public API is creation-only; every submission writes a new
 *     annotation. Updates are not exposed through the public surface.
