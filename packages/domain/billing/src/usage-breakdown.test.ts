...
describe("billingUsageCategoryFor", () => {
  it("classifies trace and eval-scan rows by action regardless of label", () => {
    ...
    expect(billingUsageCategoryFor({ action: "eval-scan", meteringLabel: null })).toBe("signals")
  ...
  it("classifies AI rows by their metering label", () => {
    ...
    expect(billingUsageCategoryFor({ action: "semantic-query", meteringLabel: "live-eval" })).toBe("signals")
    expect(billingUsageCategoryFor({ action: "llm-call", meteringLabel: "eval-optimize" })).toBe("signals")
    ...
    expect(billingUsageCategoryFor({ action: "llm-call", meteringLabel: "session-analysis" })).toBe("behaviors")
...
describe("summarizeBillingUsageBreakdown", () => {
  it("folds ledger rows into one row per project and category, largest first", () => {
    ...
    expect(rows).toEqual([
      ...
      { projectId: PROJECT_A, category: "signals", credits: 85 },
