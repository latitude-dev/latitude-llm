...
import type { Report, ReportV3 } from "@domain/spans"
...
/**
 * V3 of the Claude Code Wrapped email. Forks V2 and adds a "Skills used"
 * section mirroring the report's public skill numbers (distinct + total). Skill
 * *names* stay off the email — only counts, matching the public web view.
 *
 * Frozen-in-amber when V4 ships — old V3 reports keep rendering with this
 * template via the version dispatch in `../index.tsx`.
 */

interface EmailTemplateV3Props {
  readonly userName: string
  /**
   * Typed as `Report` to satisfy the shared template interface in the
   * dispatcher. The V3 renderer only receives this component when
   * `reportVersion === 3`, so the cast to `ReportV3` below is safe.
   */
...
function AnchorBlock({ anchor }: { anchor: ReportV3["loc"]["writtenAnchor"] }) {
...
function HeroSection({ userName, report }: { userName: string; report: ReportV3 }) {
  return (
...
function HeadlineNumbersGrid({ report }: { report: ReportV3 }) {
...
function TokenSection({ report }: { report: ReportV3 }) {
  const { totals, lastReport } = report
...
function LocSection({ report }: { report: ReportV3 }) {
...
function SkillsSection({ report }: { report: ReportV3 }) {
  const { skills } = report
  ...
  return (
    <Section style={sectionStyle}>
      ...
      >
        <tbody>
          <tr>
            <StatCard label="Skills used" value={formatCompact(skills.distinctUsed)} />
            <StatCard label="Skill uses" value={formatCompact(skills.totalUses)} />
          </tr>
        </tbody>
      </table>
    </Section>
...
function PersonalityRevealSection({
  ...
  personality: ReportV3["personality"]
  imageBaseUrl: string
}) {
...
export function ClaudeCodeWrappedEmailV3({ userName, report: reportBase, webAppUrl, reportId }: EmailTemplateV3Props) {
  // Safe: dispatcher only routes here when reportVersion === 3.
  const report = reportBase as ReportV3
...
ClaudeCodeWrappedEmailV3.PreviewProps = {
  userName: "Alex",
  webAppUrl: "http://localhost:3000",
  reportId: "ccwv3p".padEnd(24, "x").slice(0, 24) as WrappedReportId,
  report: {
    ...
    skills: {
      ...
      totalUses: 34,
      top: [
        { name: "code-review", count: 12 },
        { name: "create-pr", count: 7 },
