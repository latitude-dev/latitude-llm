...
- classify(instructions, options) — async; classifies the session with Jev. \`options\` is an object whose keys are option names and values are descriptions or null. Returns an object mapping every option name to its probability. Use ordinary code to select, combine, or threshold those probabilities.
...
- When you call classify(), provide at least two distinct options and use the returned probabilities in the final Score.`
