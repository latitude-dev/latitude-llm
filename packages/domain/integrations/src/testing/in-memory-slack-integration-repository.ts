 ...
 * the live adapter's invariant of a single active row per organization
 * (`revoked_at IS NULL`); the same `teamId` may be active in several
 * orgs. `seed` lets tests inject rows representing prior state.
 *
