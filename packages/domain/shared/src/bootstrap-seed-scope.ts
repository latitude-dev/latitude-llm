...
const BOOTSTRAP_CUID_OVERRIDES: Readonly<Record<string, string>> = {
  ...
  // Named issues (8)
  "issue:warranty-fab": SEED_SIGNAL_ID,
  "issue:combination": SEED_COMBINATION_SIGNAL_ID,
  "issue:logistics": SEED_GENERATE_SIGNAL_ID,
  "issue:returns": SEED_RETURNS_SIGNAL_ID,
  "issue:billing": SEED_BILLING_SIGNAL_ID,
  "issue:access": SEED_ACCESS_SIGNAL_ID,
  "issue:installation": SEED_INSTALLATION_SIGNAL_ID,
  "issue:flagger": SEED_FLAGGER_SIGNAL_ID,

  // Long-tail issues (128) — populated below in the spread
  ...Object.fromEntries(SEED_EXTRA_SIGNAL_IDS.map((id, i) => [`issue:extra:${i}`, id])),
...
/**
 * Legacy generator the score seeders used pre-refactor: deterministic
 * but **not** project-scoped. Two demo projects calling it would produce
 * the same primary-key value (`scores.id`) and clobber each other's rows
 * via `onConflictDoUpdate`. The seeders now look up score ids via
 * `scope.cuid("score:<prefix>:<index>")`; the bootstrap caller routes
 * those keys through this function so `pnpm seed` keeps producing the
 * same literal ids, while the demo path falls through to derivation.
 */
const legacySeedScoreId = (prefix: string, index: number): string =>
  `${prefix}${index.toString().padStart(3, "0")}${"x".repeat(24 - prefix.length - 3)}`

const SCORE_FIXTURE_KEY_PATTERN = /^score:([a-z0-9-]+):(\d+)$/

const resolveBootstrapCuid = (key: string): string | undefined => {
  const literal = BOOTSTRAP_CUID_OVERRIDES[key]
  if (literal !== undefined) return literal

  // Fixture-position keys for score rows (e.g. "score:i1:0", "score:al:5",
  // "score:io:12"). Pre-refactor these were generated by `seedScoreId` in
  // each seeder; the bootstrap path routes them back to the same legacy
  // string so `scores.id` stays identical to what `pnpm seed` produced.
  const scoreMatch = SCORE_FIXTURE_KEY_PATTERN.exec(key)
  if (scoreMatch) {
    const [, prefix, indexRaw] = scoreMatch
    if (prefix !== undefined && indexRaw !== undefined) {
      return legacySeedScoreId(prefix, Number(indexRaw))
    }
  }
...
const BOOTSTRAP_UUID_OVERRIDES: Readonly<Record<string, string>> = {
  "issue:warranty-fab:uuid": SEED_SIGNAL_UUID,
  "issue:combination:uuid": SEED_COMBINATION_SIGNAL_UUID,
  "issue:logistics:uuid": SEED_GENERATE_SIGNAL_UUID,
  "issue:returns:uuid": SEED_RETURNS_SIGNAL_UUID,
  "issue:billing:uuid": SEED_BILLING_SIGNAL_UUID,
  "issue:access:uuid": SEED_ACCESS_SIGNAL_UUID,
  "issue:installation:uuid": SEED_INSTALLATION_SIGNAL_UUID,
  "issue:flagger:uuid": SEED_FLAGGER_SIGNAL_UUID,

  ...Object.fromEntries(SEED_EXTRA_SIGNAL_UUIDS.map((uuid, i) => [`issue:extra:${i}:uuid`, uuid])),
}
