...
describe("monitor-metric-units", () => {
  it("maps each metric to its display unit", () => {
    ...
    expect(metricUnit({ kind: "avg", field: "duration" })).toBe("ms")
    expect(metricUnit({ kind: "p95", field: "cost" })).toBe("$")
  ...
  it("converts display values to the stored unit the firing path compares", () => {
    ...
    expect(metricValueToStored(500, { kind: "p95", field: "duration" })).toBe(500_000_000)
  ...
  it("formats a stored value with its unit", () => {
    ...
    expect(formatMetricValue(500_000_000, { kind: "p95", field: "duration" })).toBe("500ms")
