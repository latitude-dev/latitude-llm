...
describe("applySignalLifecycleCommandUseCase", () => {
  ...
  it("mutes and unmutes without soft-deleting evaluations or emitting escalation events", async () => {
    const now = new Date("2026-04-10T12:00:00.000Z")
    ...
    expect(muted.items[0]?.changed).toBe(true)
    expect(muted.items[0]?.mutedAt?.toISOString()).toBe(now.toISOString())
    expect(softDeleteBySignalIdCalls).toEqual([])
    expect(events).toEqual([])

    const unmuted = await Effect.runPromise(
      applySignalLifecycleCommandUseCase({
        projectId,
        signalIds: [signal.id],
        command: "unmute",
    ...
    expect(unmuted.items[0]?.changed).toBe(true)
    expect(unmuted.items[0]?.mutedAt).toBeNull()
  })
})
