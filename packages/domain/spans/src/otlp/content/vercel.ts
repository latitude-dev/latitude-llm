...
function parseInputFromTopLevel(attrs: readonly OtlpKeyValue[]): {
  ...
  if (typeof system === "string" && system) {
    systemInstructions = JSON.stringify([{ type: "text", content: system }])
  }
...
function parseInputFromCallLevel(attrs: readonly OtlpKeyValue[]): {
  ...
  const system = result.system ? JSON.stringify(result.system) : ""
  return { messages: result.messages as GenAIMessage[], system }
}
...
function parseToolDefinitions(attrs: readonly OtlpKeyValue[]): string {
  const kv = attrs.find((a) => a.key === "ai.prompt.tools")
  if (!kv?.value?.arrayValue?.values) return ""

  const tools = kv.value.arrayValue.values
    .map((v) => {
      if (!v.stringValue) return undefined
      return parseJsonSafe(v.stringValue)
    })
