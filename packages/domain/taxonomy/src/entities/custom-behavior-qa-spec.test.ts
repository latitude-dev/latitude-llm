...
// non-empty, and none of the excluded taxonomy-output fields (topics, moments).
describe("custom-behavior QA cohort spec", () => {
  ...
  it("every cohort filterSet is valid, non-empty, and free of excluded fields", () => {
    for (const cohort of CUSTOM_BEHAVIOR_QA_COHORT_LIST) {
      ...
      for (const field of CUSTOM_BEHAVIOR_EXCLUDED_FILTER_FIELDS) {
        expect(cohort.filterSet[field]).toBeUndefined()
      }
    }
  })
