...
const displayableClusters = (clusters: readonly TaxonomyCluster[]): readonly TaxonomyCluster[] =>
  clusters.filter((cluster) => isDisplayableTaxonomyName(cluster.name))
...
export const listClustersUseCase = (input: ListTaxonomyClustersInput) =>
  Effect.gen(function* () {
    ...
    if (search) {
      ...
      const items = displayableClusters(
    ...
    return toPage({ ...result, items: displayableClusters(result.items), limit, offset })
