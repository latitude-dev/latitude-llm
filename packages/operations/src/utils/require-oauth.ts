...
/**
 * Asserts that the request was authenticated through the OAuth flow
 * (`Bearer loa_*` token), not through an organization-scoped API key. Yields the
 * authenticated user id so the operation can pass it to use-cases that need a
 ...
 * attribution).
 *
 * Fails with `ForbiddenError` (→ 403) for API-key callers. API keys are
 ...
 */
export const requireOAuthUserId = (auth: AuthContext): Effect.Effect<UserId, ForbiddenError> =>
  auth.method === "oauth"
    ? Effect.succeed(auth.userId)
    : Effect.fail(
        new ForbiddenError({
          message:
            "This endpoint requires OAuth authentication; API-key callers can't act on behalf of a specific user.",
        }),
      )
