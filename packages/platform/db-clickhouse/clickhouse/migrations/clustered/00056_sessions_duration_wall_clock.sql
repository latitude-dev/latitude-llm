...
-- Session duration_ns must not depend on root spans. Incomplete trees (parent
-- never ingested, every surviving span has a non-empty parent_span_id) made the
-- root-sum rollup evaluate to 0 while min_start_time / max_end_time were still
-- correct — list UIs rendered "-" next to a detail drawer that showed ~3.9s from
-- the same spans. Match traces: wall-clock = max(end) - min(start) as an ALIAS
-- over those mergeable extrema (same formula as 00007 before 00016 switched to
-- sum-of-roots). Existing sessions need no backfill; extrema were already stored.
...
ALTER TABLE sessions ON CLUSTER default
    ...
    ADD COLUMN duration_ns Int64 ALIAS reinterpretAsInt64(max_end_time) - reinterpretAsInt64(min_start_time)
        AFTER max_start_time;
...
-- Restore the 00055 sessions_mv (sum-of-root-span durations) and the stored
-- SimpleAggregateFunction(sum) duration_ns column from 00016.
