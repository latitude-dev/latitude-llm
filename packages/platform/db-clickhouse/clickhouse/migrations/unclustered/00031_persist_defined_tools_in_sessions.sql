...
-- Persist defined tools (tool_names on chat spans) on the sessions rollup, so
-- sessions can be filtered by tools that were offered to the model but never
-- called — 00025 only rolled up *called* tools (execute_tool spans).
--
-- SimpleAggregateFunction(groupUniqArrayArray, ...) follows the `tags`
-- pattern (00007): the -Array combinator consumes the array column row-wise,
-- so no arrayJoin is needed (which would multiply sibling aggregates).
--
-- 00033 backfills rows written before this MV rebuild. Spans ingested in the
-- DROP→CREATE VIEW window miss the rollup (same exposure 00025 accepted);
-- the backfill repairs `defined_tools` for that window because uniq-array
-- unions merge idempotently.
