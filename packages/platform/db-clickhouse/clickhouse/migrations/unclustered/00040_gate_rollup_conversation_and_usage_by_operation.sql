...
--
-- The trace-detail conversation (getTrace.allMessages) is rolled up from
-- `output_messages` / `last_input_messages` (the latest-ending span with a
-- non-empty output) and `input_messages` / `system_instructions` (the first
-- span). The Vercel AI SDK wrapper span ends after its leaves and carries a
-- lossy operation summary (v6: final text only; v7: final text + orphan
-- tool_calls, no tool results), so the un-gated `output_messages != ''`
-- condition picks the wrapper and the conversation comes out incomplete/
-- malformed. The v7 wrapper also reports aggregate usage on top of its leaves'
-- per-call usage, so `sum(tokens_*)` / `sum(cost_*)` double-counts.
--
-- Fix: pick conversation only from real model-call leaves
-- (`chat` / `text_completion` / `generate_content`) and sum usage only from
-- billable leaves (those three + `embeddings` / `reranker`). The reclassified
-- Vercel wrappers (now `invoke_agent`, see resolvers/operation.ts) and every
-- other family's inert wrappers fall outside both gates. The usage gate is an
-- allowlist by design: unknown/custom operations simply don't sum.
--
-- This only redefines the two materialized views (DROP VIEW + CREATE) — the
-- target tables and their AggregateFunction column types are unchanged (the
-- gate lives in the existing `UInt8` condition slot), so no ALTER / table
-- rebuild is needed and existing aggregate states stay valid. It is therefore
-- new-data-only: spans aggregated before this migration keep their prior
-- rolled-up values; only spans ingested after it use the gated definition.
