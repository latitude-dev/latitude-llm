...
echo "Checking if database '${CLICKHOUSE_DB}' exists..."
if ! goose -dir "$PKG_DIR/clickhouse/migrations" clickhouse "${DBSTRING%/*}/default" status 2>&1 | grep -q "${CLICKHOUSE_DB}"; then
	echo ""
	echo "⚠️  Warning: Database '${CLICKHOUSE_DB}' may not exist."
	echo ""
	echo "For ClickHouse Cloud, you need to create the database manually:"
	echo "1. Log into https://console.clickhouse.cloud"
	echo "2. Select your service: fv1tbv7wm1"
	echo "3. Click 'SQL Console' and run: CREATE DATABASE ${CLICKHOUSE_DB};"
	echo ""
fi
