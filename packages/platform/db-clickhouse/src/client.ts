...
export const createClickhouseClientEffect = (
  ...
  return Effect.all({
    ...
    username: config.username ? Effect.succeed(config.username) : parseEnv(process.env.CLICKHOUSE_USER, "string"),
    password: config.password ? Effect.succeed(config.password) : parseEnv(process.env.CLICKHOUSE_PASSWORD, "string"),
    database: config.database ? Effect.succeed(config.database) : parseEnv(process.env.CLICKHOUSE_DB, "string"),
  }).pipe(
