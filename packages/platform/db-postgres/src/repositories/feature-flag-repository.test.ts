...
describe("FeatureFlagRepositoryLive", () => {
  ...
  it("returns false for a flag with no DB row at all", async () => {
    const enabled = await runWithLive(
      Effect.gen(function* () {
        ...
        return yield* repo.isEnabledForOrganization("sso")
      }),
    )
  ...
  it("enables, lists, and disables a feature flag for an organization", async () => {
    const result = await runWithLive(
      Effect.gen(function* () {
        ...
        const enabled = yield* repo.isEnabledForOrganization("sso")
        const list = yield* repo.listEnabledForOrganization()
        yield* repo.disableForOrganization("sso")
        const disabled = yield* repo.isEnabledForOrganization("sso")
    ...
    expect(result.list.map((featureFlag) => featureFlag.identifier)).toEqual(["sso"])
  ...
  it("isolates enabled flags by organization", async () => {
    await runWithLive(
      Effect.gen(function* () {
        const repo = yield* FeatureFlagRepository
        yield* repo.enableForOrganization({ identifier: "sso", enabledByAdminUserId: ADMIN_USER_ID })
      }),
    )
  ...
  it("treats globally enabled flags as enabled for any organization", async () => {
    await pg.db.insert(featureFlags).values({ identifier: "sso", enabledForAll: true })
    ...
    expect(result.enabled).toBe(true)
    expect(result.list.map((flag) => flag.identifier)).toEqual(["sso"])
  })
