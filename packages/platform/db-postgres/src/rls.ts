 ...
 */
export const setUserContext = (client: PoolClient, userId: string): Effect.Effect<void, RLSError> => {
  return Effect.tryPromise({
 ...
 */
const releaseClient = (client: PoolClient): Effect.Effect<void, never> => Effect.sync(() => client.release())
