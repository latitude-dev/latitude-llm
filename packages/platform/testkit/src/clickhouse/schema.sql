...
(
    ...
    `threshold` Float64 CODEC(Gorilla(8), ZSTD(1)),
    `probability` Nullable(Float64) CODEC(Gorilla(8), ZSTD(1)),
    ...
    `selection_probability` Nullable(Float64) CODEC(Gorilla(8), ZSTD(1)),
...
CREATE TABLE flagger_jev_preclassifier_observations
(
    observation_id        FixedString(64)                 CODEC(ZSTD(1)),
    organization_id       LowCardinality(FixedString(24)) CODEC(ZSTD(1)),
    project_id            LowCardinality(FixedString(24)) CODEC(ZSTD(1)),
...
CREATE TABLE flagger_screening_decisions
(
    ...
    `inclusion_probability` Nullable(Float64) CODEC(Gorilla(8), ZSTD(1)),
...
(
    ...
    `flagger_slug` Nullable(String) CODEC(ZSTD(1)),
    ...
    `flagger_bundle_key` Nullable(String) CODEC(ZSTD(1)),
...
(
    ...
    `max_start_time` DateTime64(9, 'UTC'),
    `duration_ns` Int64,
...
(
    ...
    `session_id` String DEFAULT '' CODEC(ZSTD(1)),
    `user_id` String DEFAULT '' CODEC(ZSTD(1)),
    `user_email` String DEFAULT '' CODEC(ZSTD(1)),
    `trace_id` FixedString(32) CODEC(ZSTD(1)),
    ...
    `tool_names` Array(LowCardinality(String)) MATERIALIZED arrayMap(x -> JSONExtractString(x, 'name'), JSONExtractArrayRaw(tool_definitions)) CODEC(ZSTD(1)),
    `ingested_at` DateTime64(3, 'UTC') DEFAULT now64(3) CODEC(Delta(8), LZ4),
    `retention_days` UInt16 DEFAULT 90 CODEC(T64, ZSTD(1)),
    INDEX idx_trace_id trace_id TYPE bloom_filter(0.01) GRANULARITY 1,
    INDEX idx_session_id session_id TYPE bloom_filter(0.01) GRANULARITY 2,
