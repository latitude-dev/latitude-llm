...
- `client.agent_score.get` — read today's published Agent Score and its five dimensions for a project.
- `client.agent_score.history` — list a project's published Agent Scores over a date range.
- `client.agent_score.causes` — inspect the ranked evidence explaining a project's current Agent Score.
