...
- `listTraces` / trace-ref filters now reject `gtePercentile` on every field except `duration`, `ttft`, and `cost` (returns `400`). Previously only `startTime`/`endTime` were rejected at the API boundary; other fields could still reach the filter builder and fail as 500.
