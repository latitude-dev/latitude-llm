// This file was auto-generated by Fern from our API Definition.

import type * as Latitude from "../index.js";

/**
 * Metric measured over the monitor target.
 */
export type MonitorMetric =
    | Latitude.MonitorMetric.Count
    | Latitude.MonitorMetric.ErrorRate
    | Latitude.MonitorMetric.CacheHitRate
    | Latitude.MonitorMetric.Avg
    | Latitude.MonitorMetric.Sum
    | Latitude.MonitorMetric.Min
    | Latitude.MonitorMetric.Max
    | Latitude.MonitorMetric.Median;

export namespace MonitorMetric {
    export interface Count {
        kind: "count";
    }

    export interface ErrorRate {
        kind: "errorRate";
    }

    export interface CacheHitRate {
        kind: "cacheHitRate";
    }

    export interface Avg {
        kind: "avg";
        /** Numeric field to aggregate. */
        field: MonitorMetricAvg.Field;
    }

    export namespace MonitorMetricAvg {
        /** Numeric field to aggregate. */
        export const Field = {
            Duration: "duration",
            Cost: "cost",
            Tokens: "tokens",
        } as const;
        export type Field = (typeof Field)[keyof typeof Field];
    }

    export interface Sum {
        kind: "sum";
        /** Numeric field to aggregate. */
        field: MonitorMetricSum.Field;
    }

    export namespace MonitorMetricSum {
        /** Numeric field to aggregate. */
        export const Field = {
            Duration: "duration",
            Cost: "cost",
            Tokens: "tokens",
        } as const;
        export type Field = (typeof Field)[keyof typeof Field];
    }

    export interface Min {
        kind: "min";
        /** Numeric field to aggregate. */
        field: MonitorMetricMin.Field;
    }

    export namespace MonitorMetricMin {
        /** Numeric field to aggregate. */
        export const Field = {
            Duration: "duration",
            Cost: "cost",
            Tokens: "tokens",
        } as const;
        export type Field = (typeof Field)[keyof typeof Field];
    }

    export interface Max {
        kind: "max";
        /** Numeric field to aggregate. */
        field: MonitorMetricMax.Field;
    }

    export namespace MonitorMetricMax {
        /** Numeric field to aggregate. */
        export const Field = {
            Duration: "duration",
            Cost: "cost",
            Tokens: "tokens",
        } as const;
        export type Field = (typeof Field)[keyof typeof Field];
    }

    export interface Median {
        kind: "median";
        /** Numeric field to aggregate. */
        field: MonitorMetricMedian.Field;
    }

    export namespace MonitorMetricMedian {
        /** Numeric field to aggregate. */
        export const Field = {
            Duration: "duration",
            Cost: "cost",
            Tokens: "tokens",
        } as const;
        export type Field = (typeof Field)[keyof typeof Field];
    }
}
