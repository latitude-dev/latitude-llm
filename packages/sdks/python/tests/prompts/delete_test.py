...
from latitude_sdk import DeletePromptOptions
...
class TestDeletePrompt(TestCase):
    async def test_success_global_options(self):
        ...
        endpoint_mock = self.gateway_mock.delete(endpoint).mock(
            return_value=httpx.Response(200, json=fixtures.DELETE_PROMPT_RESPONSE)
        )
        ...
        self.assertEqual(result, fixtures.DELETE_PROMPT_RESULT)
