...
import { mockRequest, mockStreamResponse } from '$sdk/tests/common/helpers/run'
...
let projectId = 123
let sdk: Latitude<SdkApiVersion>
...
describe('/run', () => {
  ...
  describe('v1', () => {
    beforeAll(() => {
      sdk = new Latitude<'v1'>(latitudeApiKey)
    })
    it(
      'sends auth header',
    ...
    it('it use project id defined in class', async () => {
      sdk = new Latitude<'v2'>(latitudeApiKey, {
        projectId: 345,
        apiVersion: 'v1',
      })
      const { mockUrl } = mockRequest({
  ...
  describe('v2', () => {
    beforeAll(() => {
      sdk = new Latitude<'v2'>(latitudeApiKey, { apiVersion: 'v2' })
    })
