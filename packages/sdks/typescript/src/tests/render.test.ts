...
`,
  ...
  describe('chain', () => {
    ...
    it('automatically runs tool handlers when the response contains a tool call request', async () => {
      const getWeatherTool = vi.fn(async (args: Record<string, unknown>) => {
        return {
          role: 'tool',
          content: [
            {
              type: 'tool-result',
              text: `The weather in ${args.location as string} is sunny and 25°C`,
      ...
      const searchTool = vi.fn(async (args: Record<string, unknown>) => {
        return {
          role: 'tool',
          content: [
            {
              type: 'tool-result',
              result: `Search results for "${args.query as string}": Found 5 relevant articles`,
              toolCallId: 'search_1',
              toolName: 'search',
            },
          ],
