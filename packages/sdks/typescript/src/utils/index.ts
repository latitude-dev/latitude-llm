    ...
    switch (handler) {
      case HandlerType.GetDocument: {
        ...
        return this.projects
          .project(p.projectId)
          .versions.version(p.versionUuid ?? HEAD_COMMIT)
      ...
      case HandlerType.GetAllDocuments: {
        ...
        return this.projects
          .project(p.projectId)
          .versions.version(p.versionUuid ?? HEAD_COMMIT).documents.root
      }
      case HandlerType.GetOrCreateDocument: {
        const p = params as GetOrCreateDocumentUrlParams
        return this.projects
          .project(p.projectId)
          .versions.version(p.versionUuid ?? HEAD_COMMIT).documents.getOrCreate
      }
      ...
      case HandlerType.RunDocument: {
        ...
        return this.projects
          .project(p.projectId)
          .versions.version(p.versionUuid ?? HEAD_COMMIT).documents.run
      }
      ...
      case HandlerType.PushCommit:
        return this.projects
          .project((params as PushCommitUrlParams).projectId)
