...
Optional env vars:
  ...
  LATITUDE_PRIME_INTELLECT_TELEMETRY_ENABLED / LATITUDE_TELEMETRY_ENABLED - master switch
"""
...
SCOPE_NAME = "latitude-telemetry-prime-intellect"
PKG_VERSION = "0.1.0"
...
def _load_config() -> Dict[str, Any]:
    ...
    enabled_flag = _env("LATITUDE_PRIME_INTELLECT_TELEMETRY_ENABLED") or _env("LATITUDE_TELEMETRY_ENABLED")
    enabled = enabled_flag.lower() not in {"0", "false", "no"} if enabled_flag else True
    no_content = _env("LATITUDE_PRIME_INTELLECT_NO_CONTENT") or _env("LATITUDE_NO_CONTENT")
...
def _debug(message: str) -> None:
    if _config().get("debug"):
        logger.info("Latitude Prime Intellect telemetry: %s", message)
