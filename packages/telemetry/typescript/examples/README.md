...
| Vercel AI SDK | `test_vercel_ai.ts` | `ai`, `@ai-sdk/openai` | `latitude.getAiSdkTelemetry()` |
| LangChain | `test_langchain.ts` | `langchain`, `@langchain/openai` | `"langchain"` |
| LlamaIndex | `test_llamaindex.ts` | `llamaindex` | `"llamaindex"` |

The Vercel AI SDK example uses the stable `experimental_telemetry` hook with `latitude.getAiSdkTelemetry()`. It does not require a Latitude auto-instrumentation entry.
