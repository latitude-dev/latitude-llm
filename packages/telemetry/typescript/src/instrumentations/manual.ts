...
export class ManualInstrumentation implements BaseInstrumentation {
  ...
  baggage(ctx: otel.Context | TraceContext) {
    ...
    if (!(ATTR_LATITUDE_SEGMENT_ID in baggage) || !(ATTR_LATITUDE_SEGMENTS in baggage)) {
      return undefined
    }
  ...
  private segment<T extends SegmentType>(
    ...
    const span = this.span(ctx, SEGMENT_SPECIFICATIONS[type].name, SpanType.Segment, {
