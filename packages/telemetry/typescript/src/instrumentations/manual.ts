...
import { ATTRIBUTES, HEAD_COMMIT, LogSources, SPAN_SPECIFICATIONS, SpanType, VALUES } from "../constants/index.ts"
import { toKebabCase, toSnakeCase } from "../utils/index.ts"
...
export class ManualInstrumentation implements BaseInstrumentation {
  ...
  private error(span: otel.Span, error: Error, options?: ErrorOptions) {
    ...
    span.setStatus(
      error.message ? { code: otel.SpanStatusCode.ERROR, message: error.message } : { code: otel.SpanStatusCode.ERROR },
    )
    span.end()
  }

  private span<T extends SpanType>(ctx: otel.Context, name: string, type: T, options?: StartSpanOptions) {
    if (!this.enabled) {
      return {
        context: ctx,
  ...
  private attribifyConfiguration(direction: "input" | "output", configuration: Record<string, unknown>) {
    const prefix = direction === "input" ? ATTRIBUTES.LATITUDE.request._root : ATTRIBUTES.LATITUDE.response._root
    ...
    for (const key in configuration) {
      ...
      attributes[`${prefix}.${field}`] = value as otel.AttributeValue
    }
  ...
  completion(ctx: otel.Context, options: StartCompletionSpanOptions) {
    ...
    try {
      const translateOptions = {
        to: Provider.GenAI as const,
        direction: "input" as const,
        ...(this.options.provider ? { from: this.options.provider } : {}),
      }
      const translated = translate(input, translateOptions)
    ...
    return {
      ...span,
        ...
        try {
          ...
          const translated = translate(output, translateOptions)
        ...
        span.end({
          attributes: {
            ...
            [ATTRIBUTES.OPENTELEMETRY.GEN_AI.response.finishReasons]: [finishReason],
            ...(end.attributes || {}),
          },
        })
      },
  ...
  chat(
    ctx: otel.Context,
    { documentLogUuid, previousTraceId, source, name, versionUuid, promptUuid, ...rest }: ChatSpanOptions,
  ) {
    const attributes = {
      [ATTRIBUTES.LATITUDE.documentLogUuid]: documentLogUuid,
  ...
  external(
    ctx: otel.Context,
    { promptUuid, documentLogUuid, source, versionUuid, externalId, name, ...rest }: ExternalSpanOptions,
  ) {
    const attributes = {
      [ATTRIBUTES.LATITUDE.documentUuid]: promptUuid,
      [ATTRIBUTES.LATITUDE.documentLogUuid]: documentLogUuid,
  ...
  unresolvedExternal(
    ctx: otel.Context,
    { path, projectId, versionUuid, conversationUuid, name, ...rest }: CaptureOptions,
  ) {
    const attributes = {
