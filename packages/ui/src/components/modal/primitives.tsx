...
const DialogContent = forwardRef<ElementRef<typeof DialogPrimitive.Content>, DialogContentProps>(
  ({ className: _cn, children, forceMount, dismissible = false, height = "content", ...props }, ref) => {
    ...
    return (
      <DialogPortal>
        {forceMount ? <DialogOverlay forceMount /> : <DialogOverlay />}
...
const DialogTitle = forwardRef<ElementRef<typeof DialogPrimitive.Title>, DialogTitleProps>(
  ({ children, ...props }, ref) => (
    <DialogPrimitive.Title ref={ref} {...props}>
      <Text.H4 weight="medium">{children}</Text.H4>
    </DialogPrimitive.Title>
  ),
)
DialogTitle.displayName = DialogPrimitive.Title.displayName

type DialogDescriptionProps = Omit<ComponentPropsWithoutRef<typeof DialogPrimitive.Description>, "className">
