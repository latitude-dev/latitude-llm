...
function Badge({
  ...
  return (
    <div
      className={cn('min-w-0', badgeVariants({ variant, shape, size }), className, {
        'opacity-50': disabled,
      ...
      {...props}
    >
      {iconProps && iconProps.placement === 'start' ? <Icon {...iconProps} size='xsmall' /> : null}
      <span
      ...
      {iconProps && iconProps.placement === 'end' ? <Icon {...iconProps} size='xsmall' /> : null}
    </div>
  )
}
