...
export function DocumentChangeSkeleton({
  ...
  return (
    <li className='w-full flex flex-row items-center gap-x-1 px-2 min-h-8'>
      <Icon name={icon as IconName} className='flex-shrink-0 w-4 h-4 text-gray-400 animate-pulse' />
      <div className='flex-grow h-5'>
        <Skeleton className={'h-full bg-muted rounded-full'} style={{ width: `${width}%` }} />
      </div>
    </li>
  )
}
