...
export type TabSelectOption<V = unknown> = {
  label: string
  value: V
  icon?: ReactNode | IconName
}

export function TabSelect<V = unknown>({
  options,
  value,
  onChange,
  label,
  ...
  return (
    <div className={cn('flex flex-col gap-y-2 w-full', { 'mt-[4px]': fancy }, className)}>
      <div
                  ...
                  <span className='flex flex-row items-center gap-x-2'>
                    {typeof option.icon === 'string' ? (
                      <Icon
                        ...
                        color={selected === option.value ? 'foreground' : 'foregroundMuted'}
                        className='shrink-0'
                      />
                    ) : (
                      option.icon
                    )}
                    <Text.H5M color={selected === option.value ? 'foreground' : 'foregroundMuted'}>
