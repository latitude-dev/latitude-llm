...
- **Wrap-up**
  ...
  - [x] CI drift jobs for `apps/api/openapi.json` and `apps/api/mcp.json`. New `.github/workflows/api-manifests.yml` runs on PRs to `development`: regenerates each manifest and `git diff --exit-code`s it. Any route/schema change that lands without a corresponding manifest update fails the check.
