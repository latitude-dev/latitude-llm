...
- **Cursors move out of the `destinations` row into `destination_source_cursors`** — one row per `(destination_id, source)` holding the per-source `watermark` (+ tie-breaker id), `last_run_at`, and `consecutive_empty_runs`. Today these are inline span-only columns on `destinations` (`cursor_ingested_at`/`cursor_span_id`/`last_run_at`/`consecutive_empty_runs`); the migration extracts them. Enabling/disabling a source per destination becomes a row, not a schema change.
- **The sweep stays a single, source-agnostic timer.** One every-minute `destinations:sweep` selects due **cursor rows** (join `destination_source_cursors` → active, non-sandbox `destinations`; per-row due predicate `last_run_at + intervalMs·2^consecutive_empty_runs ≤ now`) and fans out. The minute tick is a heartbeat; per-source cadence is data in the row, not a separate timer. `destinations:prune` likewise stays one nightly GC.
...
- **Read/cursor/scheduling generalization** (DestinationSource read port, `destination_source_cursors`, per-source `runSync`, single sweep over cursor rows) is useful for *any* second source — even another append-only one (scores) — and carries no delete/mutable assumptions. **Decision (2026-06-16): do this before QA** so the engine is in its multi-source shape and QA validates the lasting structure once, rather than QA-now-plus-re-QA. Spans remain the sole wired source.
...
| 6 | **P2-5** (LAT-680), **P3-1** (LAT-681), **P3-2** (LAT-682) | ⬜ tests · backfill · throughput (P3-2 is operational load validation) |
