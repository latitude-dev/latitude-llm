...
**Implementation sequencing:** extract `SourceContract` from *two real cases* (spans + the first mutable source), not from spans + a hypothetical — the tombstone and upsert/event-projection seams are unknowable at N=1. Land it as the first PR of "export issues/behaviours" (Phase 4), gated on the source's soft-delete migration. Not before the Phase 1/2 QA pass, which validates the shipped engine as-is.
