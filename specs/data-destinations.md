...
- [ ] **P1-7**: Queue topic `destinations` (`sweep`, `runSync`, nightly `pruneSyncRuns`) + `apps/workers/src/workers/destinations.ts` — repeatable sweep registration (stable key), due-selection by `last_run_at` with idle backoff, excluding sandbox orgs (via `listDue`), fan-out with dedupeKey; separate nightly `pruneSyncRuns` job for audit-row retention; runSync wiring of layers + deliverer registry. Error policy: throw on retryable (BullMQ backoff), ack on non-retryable; final-failure hook increments `consecutive_failures`.
- [ ] **P1-8**: `destinations` feature flag registered (off). Local-dev-only seed/CLI path to create a destination for smoke testing against a real PostHog project — dev scaffolding to exercise Phase 1 before the UI exists, **not** a product creation surface (creation is UI-only; see the feature-flag decision).
...
- [ ] **P2-3**: Web-private server functions (`apps/web`) for CRUD + test-connection, org-scoped authz at the boundary. No public API/MCP creation surface — creation is UI-only by decision (see the feature-flag decision above).
