...
| `task` | calls | input | output | cache read | reasoning | in Latitude? |
| --- | --- | --- | --- | --- | --- | --- |
| *(main loop)* | 205 | 668,303 | 81,784 | 26,727,680 | 31,529 | **yes** |
| `background_review` | 31 | 76,666 | 18,367 | 4,865,408 | 4,054 | **yes** |
| `approval` | 38 | 15,771 | 1,457 | 0 | 0 | no |
| `compression` | 2 | 37,509 | 16,499 | 0 | 0 | no |
| `title_generation` | 1 | 312 | 73 | 0 | 0 | no |
| **all** | **277** | 798,561 | 118,180 | 31,593,088 | 35,583 | 236 of 277 |

**Latitude = main loop + `background_review`, to the token:**

| Metric | Latitude | main + `background_review` |
| --- | --- | --- |
| `chat` spans / calls | 236 | 205 + 31 = **236** |
| input | 744,969 | 668,303 + 76,666 = **744,969** |
| output incl. reasoning | 64,568 + 35,583 = 100,151 | 81,784 + 18,367 = **100,151** |
| reasoning | 35,583 | 31,529 + 4,054 = **35,583** |
| cache read | 31,593,088 | 26,727,680 + 4,865,408 = **31,593,088** |

Five independent quantities match exactly. **The plugin's usage mapping is correct and nothing is double-counted or lost on the paths it sees.** The audit also confirms the per-span arithmetic verified earlier (raw `input 1356 / output 634 / cache_read 166528 / reasoning 549 / total 168518` → `tokensInput 1356`, `tokensOutput 85` = 634 − 549, `tokensCacheRead 166528`) and the absence of duplication (root `hermes.llm_calls: 48` / `hermes.tool_calls: 75` on trace `be969da7…` against exactly 48 `llm_request` and 75 `tool_call:*` spans, each with a distinct prompt size and monotonically growing cache read).

**Why `/usage` says 205 and we say 236.** `background_review` forks run a **full conversation loop on their own `Agent` instance in a separate thread** (`run_agent.py:8840` — the fork copies the turn's Context, which is how its usage still reaches `session_model_usage`). Being a real conversation loop it fires `pre_api_request` / `post_api_request`, so the plugin records those 31 calls; being a *separate agent object* it has its own `session_*` counters, so `/usage` — which reads the main agent's counters — never sees them. `agent.log` shows exactly this: **236 `API call #N` lines**, a maximum counter of **205**, and **7 restarts of the numbering** (one per background review, each fork counting from #1). 205 + 31 = 236, and `sessions.api_call_count` = 205 confirms the persisted main-loop figure agrees with `/usage`.

Two earlier hypotheses are therefore **disproved** and must not be chased: the CLI's agent-rebuild-on-route-change (`init_agent` zeroing the counters) did **not** happen here — the DB's main-loop total equals `/usage` exactly — and the eight retry-loop `break` paths that skip the accounting line did not fire either. They remain real mechanisms, just not this session's explanation; the counter restarts in the log are fork boundaries, not rebuilds.

**What is genuinely missing: 41 auxiliary calls** — `approval` (38, the smart-approval aux LLM), `compression` (2, context compaction) and `title_generation` (1) — worth 53,592 input and 18,029 output tokens. They go through `agent/auxiliary_client.py`, which fires **no** hooks at all (there is no `invoke_hook` anywhere in it), so no hook-based emitter can see them. Compaction is the most valuable of the three to observe: two calls consumed 37,509 input and 16,499 output, more per call than any main-loop turn. This is [F22](#6-findings) / [M11](#milestone-11--auxiliary-llm-accounting-f22).

**Cost is confirmed as F21.** The DB records `billing_mode = subscription_included`, `cost_status = included`, `estimated_cost_usd = 0.0` for this session; Latitude priced it at ≈ $225.26. The log also shows the real route is `provider=openai-codex`, which Latitude's `resolveProviderName` normalises to `openai` — correct for model pricing, but it erases the distinction between the subscription route and a metered API key, which is exactly the distinction the cost depends on. So the plugin should carry the raw route (`hermes.provider.raw`, `hermes.billing.mode`) even though the normalised provider stays `openai`.

**One presentational difference remains, and it is ours to document, not to fix.** Hermes's "Output tokens" is inclusive of reasoning; Latitude's `tokensOutput` excludes it with `tokensReasoning` beside it. `gen_ai.usage.output_tokens` must keep carrying the inclusive figure — the resolver subtracts reasoning itself, and sending the exclusive value would double-subtract.
---
...
**F21 — Cost is silently list-price on subscription routes.** Confirmed from `state.db`: this session is `billing_mode = subscription_included`, `cost_status = included`, `estimated_cost_usd = 0.0`, while Latitude priced it at ≈ $225.26 from the catalog. Neither number is wrong, but the span says nothing about which regime it is in. Compounding it, the real route is `provider=openai-codex` (per Hermes's own log) and Latitude's `resolveProviderName` normalises that to `openai` — right for model pricing, wrong for telling a subscription apart from a metered key.

**F22 — 41 auxiliary LLM calls per session are invisible.** `approval` (38 calls — the smart-approval aux LLM), `compression` (2 — context compaction) and `title_generation` (1) went through `agent/auxiliary_client.py`, which fires **no** hooks (no `invoke_hook` anywhere in it), so no hook-based emitter can see them. They are worth 53,592 input and 18,029 output tokens here — ~7% of the session's non-cache tokens — and compaction is the most interesting of the three to observe (two calls at 37,509 in / 16,499 out, more per call than any user turn). Hermes does record them, keyed by `task`, in `session_model_usage`.

**F23 — Background-review runs are indistinguishable from user turns.** `background_review` forks a full conversation loop onto its own `Agent` in a separate thread, so it fires the api hooks and lands in Latitude as its own `interaction` trace inside the user's session — with a `user_prompt` that is a generated review instruction, not something the user typed. Nothing on the span says so, so a chunk of the session's traces (7 reviews / 31 calls here) read as real user turns.
...
Lowest priority of the plan, and the only milestone that reads Hermes's SQLite state. If it slips, [M10-3](#milestone-10--docs-version-qa-repo-hygiene) must still document the gap precisely.

- [ ] **M11-1**: Mark fork-origin runs: record `hermes.thread.name` and `hermes.thread.is_main` on every span (no interpretation, zero cost), and set `interaction.kind = "background"` plus a `background` tag when a run opens off the main thread **and** the session already has a main-thread run — the conservative test, so gateway worker threads are not mislabelled.
- [ ] **M11-2**: At `on_session_finalize`, read `session_model_usage` for the session from `<hermes_home>/state.db` opened **read-only** (`file:…?mode=ro`, guarded, never a write, never a lock), and compute the remainder against the usage this process already exported for that session.
- [ ] **M11-3**: Emit the remainder as one span per unexported `task`: `gen_ai.operation.name = chat`, name `aux:<task>`, `gen_ai.request.model` from the row, `gen_ai.usage.*` from the row's buckets, `hermes.aux.task`, `hermes.aux.api_calls`, `hermes.aux.source = "session_model_usage"`, `interaction.kind = "auxiliary"`, in their own trace carrying the session id. Reconcile on **totals**, not on a hardcoded task allowlist, so a task that starts firing hooks later cannot be double-counted.
- [ ] **M11-4**: Handle the background flusher: the DB write is queued, so read at finalize, retry briefly, and if a task's row is still short of what we exported (negative remainder) emit nothing for it rather than a negative span.
- [ ] **M11-5**: Off by default? **No** — on by default (`LATITUDE_HERMES_AUX_USAGE=0` to disable), because a session's cost is wrong without it. Skip silently when `state.db` is absent or unreadable.
- [ ] **M11-6**: Tests against a fixture SQLite file: remainder computed per task; already-exported task yields no span; negative remainder suppressed; missing DB is a no-op; the emitted span rolls up into the session's token totals.
...
4. **Cost on subscription-included routes.** Latitude priced this session at ≈ **$225.26** of catalog value although the user paid nothing — confirmed emitter-side from `state.db`: `billing_mode = subscription_included`, `cost_status = included`, `estimated_cost_usd = 0.0`. The number is defensible as list-price equivalent, but there is no way for an emitter to say "this route is included": a zero cost is treated as absent. A `gen_ai.usage.cost.status` / billing-mode hint (or accepting an explicit zero as authoritative) would let harnesses report reality. Related: `resolveProviderName` folds `openai-codex` into `openai`, which is right for pricing lookups but erases the subscription-vs-metered distinction that the cost depends on — worth keeping the raw route name alongside the normalised one.
