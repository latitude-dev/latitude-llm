...
**Settling (2) versus (3) is a read-only audit on the VM**: `packages/telemetry/hermes/scripts/usage-audit.sh [session-id] [--profile NAME]`. Hermes persists the answer in two places the agent's own counters cannot reach, both in `<hermes_home>/state.db`:

- **`sessions`** carries cumulative `api_call_count` and every token bucket keyed by session id, so it survives an agent rebuild. `sessions.api_call_count` ≫ `/usage`'s figure proves (3).
- **`session_model_usage`** is keyed `(session_id, model, billing_provider, billing_base_url, billing_mode, task)`. **`task = ''` is the main loop; any other `task` is an auxiliary call** — compression, vision, `web_extract`, `session_search`, MoA slots, background-review forks — recorded through `agent/aux_accounting.py` precisely because the main loop never counts them (`run_agent.py:8848`). It also carries `billing_mode`, `cost_status` and both cost figures per route, which settles (4) at the same time.

Third source: Hermes logs `API call #%d: model=… in=… out=… total=…` at INFO from the accumulator itself (`conversation_loop.py` ~4260) into `<hermes_home>/logs/agent.log`, and every line is tagged `[session_id]`. Because the line prints the running counter, **a restart of the numbering is a direct fingerprint of the agent rebuild** — the script counts the lines and reports how many times the sequence went backwards. (Logs rotate at 5 MB × 3, so an old session may have aged out; the DB has no such limit.) After [M3](#milestone-3--error-interruption-and-retry-semantics-f6-f7-f12-f17c) lands, `hermes.retry_count` and real error types make the same question answerable from Latitude alone.
...
| Usage vs `/usage` | run `/usage`, then `scripts/usage-audit.sh <session-id>` on the VM, then compare with `getSession` per [5.1](#51-usage-accounting-latitude-versus-hermes-usage) | input/cache reconcile; output reconciles once reasoning is added back; any remaining call-count gap is explained by the `API call #N` count (reset vs denominator) |
