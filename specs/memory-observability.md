...
Phases 0–4 build the consume side (ingest, classify, ledger, four surfaces); nothing yet helps a customer **emit** memory spans. OTEL's memory convention has no auto-instrumentation and is `Development`-stage, so emission is hand-rolled raw OTEL today — docs and a skill alone would teach attribute-string soup. The linchpin is a typed SDK helper that docs, onboarding, and the skill can all point at; the four steps deliver it and the guidance around it. The emit-side correctness rules to teach all live in [Store identity](#store-identity): `store.id` is load-bearing (absent ⇒ the `""` bucket, [D15](#decisions)); per-user isolation is a store-naming convention, not a separate axis; `gen_ai.memory.records` content is **opt-in** (diffs/blame/token-deltas degrade to counts without it, with a PII tradeoff, [D1](#decisions)); `record.id` splits on `/` for the Memory-page tree.

- [ ] **P5-1**: SDK memory helper — a typed emitter in `@latitude-data/telemetry` (TS) and `latitude-telemetry` (Python): `memory.{create,update,upsert,delete,search,createStore,deleteStore}`. Models the existing manual-`gen_ai`-span helper `packages/telemetry/typescript/src/sdk/codemode.ts` (the only first-party `gen_ai.*` emitter); sets `gen_ai.operation.name` + the `gen_ai.memory.*` attributes, makes `store.id` a required argument, and gates `records` content behind an explicit opt-in. Adds the `gen_ai.memory.*` keys to `constants/attributes.ts` (both SDKs). Does **not** reintroduce a scope key (`latitude.memory.scope` was removed in `94f6efc42`; `store_id` is the sole key, [D4](#decisions)).
...
**Exit gate:** a customer who installs the SDK helper (or follows the docs) emits memory spans that classify on the Spans tab, attribute to the right store, and populate the session/trace summary and the Memory page — with `store.id` always set and content opt-in understood.
