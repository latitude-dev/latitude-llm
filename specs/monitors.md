...
- [x] Drop `assignedUserId` and `createdByUserId` columns via Drizzle migration. Update `saved_searches.ts` schema, `SavedSearch` entity, repository, every use-case, and the OpenAPI `SavedSearchSchema`. `createdByUserId` survives as a transient input to `createSavedSearch` (it still stamps the `SavedSearchCreated` audit event's `actorUserId`) but is no longer stored or returned.
- [x] Delete `assignSavedSearchUseCase` (+ `AssigneeNotOrgMemberError` and the now-unused `@domain/organizations` dep) and the `POST /searches/{slug}/assign` endpoint. Remove the assign-user UI.
- [x] Add `<SearchInput>` (with the syntax-legend popover) to `Layout.Actions` of `apps/web/src/routes/_authenticated/projects/$projectSlug/index.tsx`. It applies to whichever tab is active (Sessions gets `isSearching={hasSearchQuery}` so the "Matching traces" column appears when querying). Both `TracesView` and `SessionsView` already accept a `searchQuery` prop. The query lives in the `query` URL param (full word, matching `filters`).
- [x] Add `<SavedSearchSelector>` (new component, dropdown listing project's saved searches with a filter input, per-row delete + Create/Edit-monitor, and a "Save current search…" footer) immediately left of `<SearchInput>`. Add `<SaveOrUpdateSearchButton>` immediately right with the four-state behaviour. The shared `SaveSearchModal` is owned by the page so the button and the selector footer both trigger it.
- [x] "Create monitor" / "Edit monitor" entry-point inside each `<SavedSearchSelector>` row: a `useMonitors` scan over `alerts[].source.id` decides which. Create opens `MonitorCreateModal` in place on the current page (prefilled via its existing `initialAlert`) and redirects to `monitors?monitorSlug=<newSlug>` on success; Edit deep-links to the existing `monitors?monitorSlug=<slug>`.
- [x] Delete `apps/web/src/routes/_authenticated/projects/$projectSlug/search/index.tsx` outright (no redirect shim — the route is dropped from the generated route tree). **As-built:** the full-page `SavedSearchesList` (its only consumer was this route) is deleted, which cascaded to deleting `MemberSelector`, `useSavedSearchAggregates`/`SavedSearchAggregates`, and the orphaned `countAnnotatedTracesByProject` / `findLastTraceAtByProject` web fns; `save-search-modal` + `search-syntax-legend` stay (now imported from the new components). `SearchInput` was extracted out of the deleted route into `-components/search-input.tsx`.
- [x] Repoint the deep-links that targeted the standalone `/search` route to the new `/` location with the saved search applied: the incidents-table **Source** cell (`monitor-incidents-table.tsx`), the alert form's "Create a new saved search" footer (`saved-search-source-picker.tsx`), the project sidebar (`project-sections.ts` — the "Search" section is dropped; Traces already points at `/`), the command palette (`use-project-search-commands.tsx`), and the trace drawer's "Open session" command (`trace-detail-drawer.tsx`).
- [x] Backend tests: saved-search create/update without the dropped columns; assign endpoint tests removed. **Regen pulled into this PR (not deferred to M9):** `openapi.json` + `mcp.json` re-emitted and the TS SDK regenerated (`6.0.0-alpha.4` → `6.0.0-alpha.5` with a CHANGELOG entry) so the breaking saved-search removal is reflected immediately.
