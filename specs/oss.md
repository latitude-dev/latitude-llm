...
- **Sensible OSS defaults** in `.env.example`: email defaults to **Mailpit**, object storage defaults to the **`fs`** driver (S3 is opt-in and S3-compatible, so MinIO works), Temporal/ClickHouse can be self-hosted **or** point at their respective Clouds.
...
- [ ] **P3-3**: Add a **MinIO** option for S3-compatible object storage (document `fs` vs MinIO vs external S3); ship a **`.env.selfhost.example`** pre-wired to the self-host profile with secrets-generation guidance and AI keys called out.
...
- [ ] **P4-3**: Decide the **stateful-dependency** story (Postgres/ClickHouse/Redis/object storage): bundled subcharts for a turnkey install vs. bring-your-own managed services, with both documented.
...
- **Stateful deps in the Helm chart (P4-3)**: bundle Postgres/ClickHouse/Redis/object-storage subcharts (turnkey but not production-ideal), or bring-your-own managed services (recommended for real prod)?
