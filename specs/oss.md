...
> **⚠️ Out-of-band step (repo settings, not in this PR) — Docker Hub publishing secrets.** `publish-docker-images.yml` needs two repo-level **secrets** to push to the `latitudedata` Docker Hub org: **`DOCKERHUB_USERNAME`** (a Docker Hub account/bot user that is a member of the `latitudedata` org with push rights) and **`DOCKERHUB_TOKEN`** (a Docker Hub **access token** with Read/Write/Delete scope for that org, created at Docker Hub → Account Settings → Personal access tokens). The `latitudedata/<service>` repositories should exist (or allow auto-create on first push). No GHCR change is needed — the existing `GITHUB_TOKEN`-based GHCR push is untouched.
