...
**Status**: pending. PR 3 now has the domain execute-and-persist seam that loads evaluation and trace context, while lifecycle gates, duplicate-prevention, hosted execution, and score persistence are still pending.
...
- `packages/domain/evaluations/src/use-cases/live/run-live-evaluation.ts` now loads one queued live evaluation plus its trace detail and returns a structured `loaded` vs stale-task `skipped` outcome for the later PR 3 steps
