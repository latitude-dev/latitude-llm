...
`SpanDecodingError` is swallowed as a warning: undecodable payloads are dropped without retry. `RedactionError` is also swallowed (logged as error): a deterministic redaction timeout or malformed policy fails closed and is not retried, so one oversized batch cannot burn ~5 minutes of worker CPU. Any other error propagates and fails the job. **The topic configures `attempts: 10` with a 1s exponential backoff, so a transient ClickHouse or object-store failure is retried up to ten times.** Before insert, the worker asks ClickHouse which `(project_id, trace_id, span_id)` rows already exist and writes only the missing ones; `TracesIngested` still publishes so a post-insert publish failure can retry without re-inserting. `spans` is a `ReplacingMergeTree`, but `traces_mv`/`sessions_mv` are `AggregatingMergeTree` and add per inserted block, so skipping existing identities is what keeps rollup counts, tokens, cost, and duration from doubling. Stalled-job recovery can still redeliver a job whose worker died mid-processing, which is a different mechanism from retry, and is covered by the same skip.
...
**The deterministic tier fails closed.** If the redaction pass throws or overruns its deadline, the effect fails and nothing is inserted. The worker treats `RedactionError` as terminal ([§2.2](#22-worker-path)) so the job is not retried: a deadline overrun on an unchanged payload would otherwise re-run the same ~30s synchronous walk up to ten times.
...
5. **Should `span-ingestion` retry before dropping a batch?** *Resolved: yes, for transient storage/ClickHouse failures only.* The topic sets `attempts: 10` with a 1s exponential backoff. Before insert, existing `(project_id, trace_id, span_id)` rows are skipped so a post-insert retry cannot double `traces_mv`/`sessions_mv` rollups. `RedactionError` is terminal: fail-closed still drops the batch, but the same payload is not walked ten times.
...
- **`span-ingestion` retries transient storage/ClickHouse failures.** The topic sets `attempts: 10` with a 1s exponential backoff. Inserts skip identities already in ClickHouse so a post-insert retry cannot double `traces_mv`/`sessions_mv`. `RedactionError` stays terminal. Reflected in [§2.2](#22-worker-path), [§4.6](#46-failure-policy), and [open question 5](#10-open-questions).
