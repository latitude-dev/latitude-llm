...
`SpanDecodingError` is swallowed as a warning (`:105-107`): undecodable payloads are dropped without retry. Any other error propagates and fails the job. **The topic configures `attempts: 10` with a 1s exponential backoff, so a failed ingest job is retried up to ten times before BullMQ drops it.** Stalled-job recovery can still redeliver a job whose worker died mid-processing, which is a different mechanism from retry.
...
**The deterministic tier fails closed.** If the redaction pass throws or overruns its deadline, the effect fails and nothing is inserted. `span-ingestion` now runs with `attempts: 10` ([§2.2](#22-worker-path)), so the job is retried and the batch is dropped only once the attempts are exhausted — nothing is ever inserted unredacted.
...
5. **Should `span-ingestion` retry before dropping a batch?** *Resolved: yes.* It now runs with `attempts: 10` and a 1s exponential backoff, so a transient redaction, ClickHouse, or object-store failure is retried instead of dropping the batch immediately; a deterministic failure still drops once the attempts are spent. `spans` is a `ReplacingMergeTree` so re-inserts dedup there, but the `AggregatingMergeTree` rollups (`traces_mv`/`sessions_mv`) double-count a re-inserted block — making the insert/event boundary idempotent against post-insert retries is tracked as follow-up.
...
- **`span-ingestion` retries.** The topic sets `attempts: 10` with a 1s exponential backoff, so a failed pass is retried up to ten times before it drops (BullMQ's default of one attempt was the earlier behavior). Reflected in [§2.2](#22-worker-path), [§4.6](#46-failure-policy), and [open question 5](#10-open-questions).
