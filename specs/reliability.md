...
- `refresh-evaluation-alignment`: linear workflow invoked by the debounced `evaluations:automaticRefreshAlignment` queue task; runs the incremental metric refresh and either persists the new confusion matrix (strategy `metric-only`) or publishes `evaluations:automaticOptimization` for full re-optimization (strategy `full-reoptimization`); it never sleeps — the 1h debounce is owned by the queue
- `optimize-evaluation`: linear workflow invoked either directly (initial generation from an issue, manual realignment) or by the debounced `evaluations:automaticOptimization` queue task after an MCC drop; runs example collection, GEPA optimization, baseline evaluation, and persistence; the 8h re-optimization rate limit is owned by the queue, not by the workflow
...
- [x] Implement annotation-driven debounced refresh with the one-hour / eight-hour cadence from the proposal through delayed BullMQ queue tasks that kick off single-purpose Temporal workflows: `evaluations:automaticRefreshAlignment` (1h debounce, keyed per evaluation) starts `refresh-evaluation-alignment`, which publishes `evaluations:automaticOptimization` (8h debounce, keyed per evaluation) to start `optimize-evaluation` when the incremental pass reports `full-reoptimization`; workflows themselves do not sleep or manage timers — debounce and rate-limit semantics live in `dedupeKey + debounceMs` on the queue.
