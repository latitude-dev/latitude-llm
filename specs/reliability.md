...
Reliability background work should use the existing repository task stack:

- `@domain/queue` for producer/consumer contracts
- `@platform/queue-bullmq` for BullMQ transport
- `apps/workers` for worker registration, queue consumers, and pollers
- the outbox consumer plus `domain-events` topic for post-commit domain-event fan-out
...
- `evaluation-execution`: executes one evaluation against one trace/session input after live trigger selection
...
Issue discovery and issue creation do not automatically generate evaluations. Instead, the issue table row and issue details modal/page expose a `Generate evaluation` action. Issues may have several linked evaluations, and each time a user triggers that action, the server publishes an `evaluation-alignment` job, returns a `jobId` immediately, and the frontend polls a dedicated status endpoint that reads a Redis key for that job while the initial generation/alignment pipeline runs in the background.
...
- for every queue flagged by either deterministic rules or the flagger model, the flagging handler publishes a separate `annotation-queue-validation` queue message for that `(queueId, traceId)` pair
...
- human-editable draft finalization should be driven by persisted due-work scans and the `score-draft-finalization` queue topic rather than by browser-local timers or BullMQ delayed jobs
...
- eligible finalized scores should enqueue `issue-discovery` after commit rather than running embeddings/search inline in request or annotation-edit paths
- issue name/description regeneration should run through `issue-refresh` once the debounce window has elapsed
- both `issue-discovery` and `issue-refresh` should use persisted Postgres eligibility state for correctness and idempotency rather than BullMQ job history
...
- the server publishes an `evaluation-alignment` job and returns a `jobId`
...
- `Generate evaluation` is available per issue and starts a background `evaluation-alignment` job
...
### (LAT-457) Phase 0 - Reliability Platform Foundations
...
**Exit gate**: reliability background-task/event contracts are explicit enough that later phases can enqueue work without inventing new queue conventions ad hoc; other phases can start without reopening core platform conventions.

### (LAT-458) Phase 1 - Settings Foundations

**Depends on**: Phase 0

**Parallelization notes**: can run in parallel with phases 2 through 7 once Phase 0 lands.
...
**Parallelization notes**: can run in parallel with phases 1 and 3 through 7 once Phase 0 lands.
...
**Parallelization notes**: can run in parallel with phases 4 through 7 once phases 0 and 2 land.
...
**Parallelization notes**: can run in parallel with phases 5 through 7 once phases 0 and 2 land.
...
**Depends on**: Phase 0, Phase 2, Phase 4

**Parallelization notes**: can run in parallel with phases 6 and 7 once phases 0, 2, and 4 land.
...
- [ ] Wire the platform/domain interfaces for text generation and optimizer orchestration so evaluation generation can use them in later phases.
...
**Parallelization notes**: can run in parallel with Phase 7 once phases 0, 2, and 3 land.
...
**Parallelization notes**: should land before score analytics and post-MVP simulation phases start, but it can run in parallel with phases 3 through 6 once phases 0 and 2 land.
...
**Parallelization notes**: can run in parallel with Phase 9 once Phase 8 lands.
...
- [ ] Implement in-product annotation creation on conversations/messages/spans/traces/sessions, including Postgres-backed draft score creation, annotation-side issue intent for automatic discovery, existing-issue linking, inline manual issue creation, due-work-driven draft finalization through `score-draft-finalization`, and post-finalization immutability after the debounce window closes.
...
- [ ] Add integration tests covering UI/API annotation parity, raw/enriched feedback preservation, refresh-safe draft visibility, debounce-based finalization through due-work scans plus `score-draft-finalization`, deletion after finalization, and reliable reopening of message-level and text-range anchors.
...
- [ ] Implement the create-or-match discovery flow behind the `issue-discovery` queue topic, including Postgres issue creation, canonical Postgres score assignment, Weaviate projection sync, recheck-before-work, single-owner invariants around canonical `scores.issue_id`, and one-time ClickHouse publication after the score becomes immutable.
...
- [ ] Implement asynchronous issue name/description generation and eight-hour refresh debounce through the `issue-refresh` queue topic plus persisted due-work scans, with retry-safe coordination between Postgres state and Weaviate projection updates and without relying on BullMQ delayed/repeat features.
...
**Depends on**: Phase 5, Phase 10, Phase 11
...
**Depends on**: Phase 6, Phase 9, Phase 10, Phase 16
