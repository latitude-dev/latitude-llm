...
- [~] Implement feedback embedding with `voyage-4-large` at `2048` dimensions and caching.
  ...
  - [ ] Wire issue-discovery embedding calls to enforce `voyage-4-large` with `2048` dimensions.
  - [ ] Wire embedding caching in the issue-discovery embedding flow.
...
- [~] Implement reranking with `rerank-2.5`, configurable thresholds, and the v1-style search sequence of hybrid retrieval first and rerank second.
  ...
  - [ ] Wire issue-discovery rerank calls to enforce `rerank-2.5`.
  - [ ] Implement configurable discovery thresholds over rerank scores.
  - [ ] Implement and verify the retrieval order: hybrid retrieval first, rerank second.
