...
- **The script returns the score (+ optional feedback); the host derives the verdict.** A normalized `value` (for sort/confidence/display) and optional `feedback`; the host derives the verdict by thresholding `value` via `isScoreMatch` (default 0.5). Generated judges return `Passed`/`Failed`; which verdict means the behavior is *present* (and so stamps `signal_id`) is per-evaluation (`evaluations.membership_on_pass`, see `specs/signals.md`). Under the current convention a behavior-present trace is `Passed` (`value` 1 → `passed = true`, an occurrence) and a clean trace is `Failed` (`value` 0 → `passed = false`, absent); deprecated judges use the inverse.
