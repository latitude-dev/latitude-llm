...
>
> **Status** — design settled; **all four PRs built**. PR1 (promotion computed and observed, #4407) and PR2 (the gate enforced: candidates invisible everywhere, announcements moved to `SignalPromoted`, escalation gated, `isSignalNew` re-anchored, #4465) are **released to production**. PR3 (naming at promotion, #4471) reached production in `v0.3.83`, tagged 2026-08-18. PR4 (consolidation + expiry) is built, and the Q2 calibration ran against production on 2026-08-24 (P4-12): `CONSOLIDATION_MIN_SIMILARITY` stays at `0.70`. P4-13 closed the chained-merge ordering gap with a merge pointer. What remains is P4-14, detection for a ClickHouse mutation that never applies, and M-4, the calibration rerun on a larger population. Decisions in §5 cover the volume-scaled threshold and its constants (D3b, D3c), uniform promotion with no bypasses (D6), placeholder-then-promote naming (D2), the full `promoted_at = created_at` backfill (D13), straight cutover with the constants as the kill switch (D14), and default-deny at the repository (D15). PR2 dropped the re-evaluation pass D16 rode on and extended D13's backfill to the enforcement boundary instead — see P2-1 in §8 for why, and treat that as the current reading of D16.
>
...
- **Scope.** Candidate-to-candidate only. A candidate may never absorb a promoted signal, and two promoted signals are never auto-merged (that is the prohibited v1 merge system). As built this is a property of the read, not of the use-case: the neighbor scan passes `unpromotedOnly`, which inverts the default-deny rule on both sides rather than relaxing it (P4-2).
...
- **Mechanism.** Pick a survivor, reassign the losers' scores (`scores.signal_id`), fold the losers' centroids into the survivor's running weighted sum, soft-delete the loser rows, re-evaluate the survivor's promotion gate. No "merged" state, no redirect, no tombstone that has to be rendered — nothing pointed at a candidate. **Reassignment spans two stores — see §4.8; a Postgres-only merge ships a visibly wrong occurrence count.**
...
- **Name and description.** The survivor keeps its placeholder while it stays a candidate. PR3 made that deliberate: an unpromoted signal is named from its occurrence's own words because that is the better rerank and lexical document for a thin cluster, and the refresh returns early for it. If the merge carries the survivor over the gate, promotion names it from the full merged occurrence set, which is the point at which a merged cluster gets one title.
...
- **At promotion**: `generateSignalDetailsUseCase` runs once over the promoted cluster's occurrences, called with `ignorePreviousDetails: true` so `previousName` and `previousDescription` stay `null`. That option exists because the use-case otherwise derives both from the signal row whenever it is given a `signalId`, which at promotion means reading the placeholder back as its own stabilization baseline and anchoring the summary to one occurrence's phrasing — the exact failure this section removes, and one that fails silently, so it is guarded by a test.

**A candidate is never refreshed.** `ScoreAssignedToSignal` schedules the throttled `signals:refresh` for every assignment, candidates included, so `refreshSignalDetailsUseCase` returns early while `promoted_at` is null. Without that guard a candidate picking up a second score is renamed by a model eight hours later, which spends a call to make matching *worse*: it replaces the occurrence's own feedback — the better rerank and lexical document for a cluster this thin (§4.6 below) — with a generalization drawn from the same one or two members that make the summary ill-posed. The promotion task runs through the same use-case after `promoted_at` is stamped, so it is unaffected.
...
- **`scores`: reconcile, through the outbox.** One `ALTER TABLE scores UPDATE signal_id = <survivor> WHERE signal_id IN (<losers>)` per consolidation pass, bounded by `organization_id`, `project_id` and a `created_at` lower bound. One mutation per pass rather than per loser, and idempotent, so redelivery is safe. CH mutations are asynchronous anyway; a few seconds of stale count on a just-announced signal is acceptable.

  Two things this paragraph originally had wrong. The lower bound is **not** "candidates are young, so use theirs": a replayed or backfilled annotation is older than the signal it was later assigned to, so a bound taken from the losers' `created_at` would skip exactly those rows and leave the survivor's count short — the bug this section exists to prevent. It comes from the oldest row Postgres actually moved, which `ScoreRepository.reassignSignal` returns. And the mutation must set the legacy `issue_id` alongside `signal_id`; it is still dual-written on insert, so moving only one column splits a score's ownership across the two.

  **It must not be a call that follows the commit.** The merge task's own retry cannot repair a crash between the two stores, because the thing that makes the merge safe to retry is what disarms it: a re-run finds the loser already soft-deleted, so the merge no-ops and never reaches the mutation. The loser's ClickHouse rows would then sit under a soft-deleted id forever while the survivor's occurrence count stays permanently short. Write the intent inside the merge transaction instead — a `SignalsConsolidated { survivorId, loserIds }` outbox event, the same transactional-outbox path every other cross-store effect here already takes — and let its consumer run the mutation. At-least-once delivery plus an idempotent mutation is what actually closes the window; P4-5's "naturally idempotent" claim is about the merge, and holds only because the reconciliation no longer rides on it.
...
Only `scores_hourly_buckets` is left holding rows under the loser's id, and those are harmless: the loser is soft-deleted, so no read ever queries that id again. The raw `scores` rows are not optional to move. Skipping that mutation leaves the survivor's occurrence count permanently short, which is the whole reason this section exists.
...
- [x] **P3-4**: The gate and promotion split into two events. `assignScoreToSignalUseCase` stops stamping `promoted_at` and emits `SignalQualifiedForPromotion`; `issues:promoteSignal` generates the summary, stamps the latch and emits `SignalPromoted` in one transaction; `domain-events` fans the notification and the dispatch off *that*, in parallel, under the dedupe keys it always used. `SignalPromoted` is therefore fully formed for every consumer, and a signal is never visible under its placeholder.

  Two traps. The consumer publishes under a **leading throttle**, not a bare dedupe key: a bare key becomes a BullMQ job id, `removeOnFail` retains failed jobs, and a permanently failed promotion would shadow every later publish so the signal could never promote — the marker expires instead, so the next score to re-qualify it retries. And generation failure must not block promotion: `promoteSignalUseCase` catches the *cause* (a provider that throws is a defect, not an `AIError`), promotes under the placeholder, and lets the now-unblocked refresh correct the name.
...
**Known gap**: `promoteSignalUseCase` is covered directly (names then stamps, promotes under the placeholder when generation fails, idempotent when already promoted), and both fan-outs are covered by the dispatcher test. What is not covered is the `promoteSignal` worker handler's own billing wiring, which needs Postgres, Redis and an AI layer in one harness.
...
- [x] **P4-1**: Extract the **gate** (count distinct sessions → compare to threshold → emit `SignalQualifiedForPromotion`) into one shared helper. ~~Three call sites~~ — **two**: creation has no count to do (it tests `1 ≥ threshold` and short-circuits the volume read on `PROMOTION_MIN_SESSIONS`), so it keeps sharing only `promotionThresholdForVolume`. `qualifySignalForPromotion` covers assignment and consolidation, and takes the locked signal plus a threshold the caller resolved outside the transaction.

  Stamping stays out of it. PR3 split qualification from promotion, so only `promoteSignalUseCase` sets `promoted_at`, and it does so after naming the signal from its cluster. A helper that finalized promotion would let the assignment path stamp a signal that still carries its placeholder, which is the state that split exists to prevent. Consolidation therefore re-evaluates the survivor by qualifying it, and the survivor is named from its merged occurrence set on the way through — which is also what §4.4 asks for.

  `SignalQualifiedForPromotion.triggerScoreId` became nullable: a merge has no trigger score. The only consumer ignores the field.
- [x] **P4-2**: ~~`includeUnpromoted`~~ **`unpromotedOnly`** opt-in on `findSimilarByCentroid`, on both sides (the source read and the neighbor scan), plus the fake. Inverting the rule rather than relaxing it is the correction: a plain include would return promoted neighbors with no cheap way to drop them — `findByIds` default-denies candidates and has no opt-in — which would make "a candidate never absorbs a promoted signal" discipline in the use-case instead of a property of the read. (Also corrected: §4.3's claim that discovery uses `findSimilarByCentroid`. It does not; `hybridSearch` is the discovery read, and it is untouched.)
- [x] **P4-3**: `ScoreRepository.reassignSignal({ projectId, fromSignalIds, toSignalId, updatedAt })` — bulk UPDATE, port + PG + fake. Deliberately not `assignSignalIfUnowned`: these scores are already owned. Returns `{ count, earliestCreatedAt }`, which is what P4-6's mutation is bounded by.
- [x] **P4-4**: Consolidation use-case — neighbor search on `CONSOLIDATION_MIN_SIMILARITY`, survivor selection (most distinct sessions, tie-broken by oldest `createdAt`, then lowest id so the choice is total), score reassignment, centroid fold via the existing `mergeCentroids` in `@domain/shared` (previously exported with no callers; this is its first), loser soft-delete, and re-qualification of the survivor through P4-1. No refresh publish: a survivor that stays a candidate keeps its placeholder, and one that crosses the gate is named by promotion.

  The survivor may be a **neighbor rather than the trigger**, so the pass has to tolerate the candidate whose centroid changed being the one absorbed.
- [x] **P4-5**: Locking and idempotency. **Trap**: a pass needs `SIGNAL_UPDATE_LOCK_KEY` for the survivor **and every loser it absorbs**, acquired in ascending id order or two concurrent merges deadlock. Not just a pair: `CONSOLIDATION_MAX_MERGES_PER_PASS` admits up to five losers, and any loser left unlocked can take a concurrent assignment while the merge reassigns its scores and deletes it, stranding that score on a deleted row and silently losing the evidence. The merge itself is one transaction, and a re-run finds the losers soft-deleted, so the task is naturally idempotent — which is exactly why the ClickHouse reconciliation cannot hang off that retry (§4.8). Every participant is re-read with `findByIdForUpdate` under those locks, which both re-validates after the unlocked scan and avoids needing an `includeUnpromoted` on `findByIds`.
- [x] **P4-6**: ClickHouse reconciliation (§4.8) driven by a `SignalsConsolidated` outbox event written in the merge transaction, not by a call after it — the merge's own retry no-ops on the soft-deleted loser and would never reach the mutation. One bounded `ALTER TABLE scores UPDATE` per pass; the `scores_hourly_buckets` skip is deliberate and documented. Test the crash-after-commit path: redelivering the event alone reconciles, and redelivering it twice does not double-apply.

  Two corrections to §4.8 found in the code. The `created_at` lower bound **cannot** come from the losers' own `created_at` — a replayed annotation is older than the signal it was later assigned to, so that bound would silently skip rows and leave the count short, which is the exact bug the section exists to prevent; it comes from the moved rows instead. And the mutation must also set the legacy `issue_id` column, which is still dual-written on insert.
- [x] **P4-7**: Guard tests — a promoted signal is never absorbed as a loser and never selected as survivor (candidate-to-candidate asserted from both directions); the merge cap binds and is reported; N fragments merge into one that qualifies when the union crosses the gate. Plus: a re-run is a no-op, and the reconciliation intent carries the oldest moved score.
- [x] **P4-8**: `CANDIDATE_EXPIRY_IDLE_DAYS` (Q3) and the sweep. Much simpler than the escalation sweep it is modelled on: expiry is a pure `deleted_at` stamp with no event and no cascade (scores stay attached by design), so it is one capped admin `UPDATE` per tick — `SignalRepository.expireIdleCandidates({ idleBefore, now, limit })` under `withPostgres(SignalRepositoryLive, adminPgClient)` with no org id, following `sweepEscalating`'s use of `IncidentRepositoryLive`. No per-signal fan-out. Side benefit: soft-delete frees the slug, so expiry recycles the `PRE-XXXX` space.

  **Needed one migration**, which this spec had missed: `signals_project_lifecycle_idx` is partial on `deleted_at IS NULL AND promoted_at IS NOT NULL`, the exact complement of what the sweep scans, so without `signals_candidate_idle_idx` every tick is a full cross-organization scan. It stays small because it only indexes the bounded candidate population.
- [x] **P4-9**: `issues:consolidate` queue task plus the throttled publishes from the two centroid-change sites, and the `issues:sweepCandidates` cron (Q2).
...
- [x] **P4-13**: Ordering half of the ClickHouse reconciliation, raised on #4482 by both reviewers.

  Two merges in a chain (`B → A`, then `A → C`) publish independent jobs and BullMQ orders neither, so running `A → C` first left B's rows untouched and the later `B → A` mutation stranded them on a soft-deleted `A`.

  Fixed with a merge pointer rather than a reconciliation-state table: `signals.merged_into_signal_id`, the shape `taxonomy_clusters.merged_into_cluster_id` already uses here. The consumer resolves its sweep set from Postgres at execution time by walking that pointer, so a merge absorbing a former survivor sweeps that survivor's own absorbed ids too and both execution orders converge on the same owner. `SignalsConsolidated` drops to identity only, and the mutation's partition floor comes from the survivor's earliest score, since a sweep set spanning several merges can reach further back than any one of them.

  **Built as a guard, not a repair.** The calibration measured zero chains in production: at `0.70` every merge group is a pair (6 groups, 6 losers, largest 2), and a chain would show as a component of three. Watch `largest_group` at M-4.
- [ ] **P4-14**: Detection for a mutation that never applies. `mutations_sync: "0"` returns when ClickHouse queues the rewrite, so a later failure is silent and leaves rows under a dead `signal_id` with nothing scheduled to move them. Unquantified: nothing records it today, which is the complaint.

  Cheapest first move is `mutations_sync: "1"`, which surfaces a failed rewrite as a failed job that BullMQ retries against an idempotent mutation. The risk to watch is that mutation cost tracks parts rewritten rather than rows matched, so a busy tenant could exceed the client's 30s timeout; if that shows up, fall back to async plus a bounded `system.mutations` check. The demo-seed precedent that argued against synchronous mutations is not comparable — it deletes a whole project across ten tables.
