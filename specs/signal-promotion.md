...
>
> **Status** — design settled; **PR1 built** (promotion computed, observed, and emitting `SignalPromoted`; nothing enforced yet), PR2 and PR3 pending. Decisions in §5 cover the volume-scaled threshold and its constants (D3b, D3c), uniform promotion with no bypasses (D6), placeholder-then-promote naming (D2), the full `promoted_at = created_at` backfill (D13), straight cutover with the constants as the kill switch (D14), default-deny at the repository (D15), and promotion on explicit user intent (D16). What remains open (§6) belongs to PR2 and PR3: the consolidation and expiry parameters, validating the naming change, and tuning the constants against the shadow data PR1 collects. Plan and PR split in §7.
...
- **D16 — Explicit user intent promotes.** A signal with an active evaluation counts as promoted: somebody deliberately tracked it, so it is real regardless of accumulated evidence. Generalizes D5.
  ...
  Enforced as a clause in PR2's re-evaluation pass (*promote every unpromoted signal that has an active evaluation*), not as a write inside `monitorSignal`. Same outcome, and it avoids adding a `promoteSignal` method to `EvaluationSignalRepository` — the deliberately tiny read-only view that exists so `@domain/evaluations` need not depend on `@domain/signals` — plus its adapter, its fake, and a call site. It is also harmless to defer: nothing reads `promoted_at` before enforcement, and after enforcement an unpromoted signal is not reachable to track in the first place. Note that the re-evaluation pass covers most of this case on its own, since it counts distinct sessions across *all* of a signal's scores, evaluation-sourced occurrences included.
...
- **PR2 — Enforcement.** Unpromoted signals disappear from every surface (§4.3, default-deny at the repository), and every consequence moves to promotion: the `signal.discovered` notification and the agent dispatch shift from `SignalCreated` to `SignalPromoted`, escalation stops running for unpromoted signals, `isSignalNew` anchors on `promoted_at`, the `promoted_at` index lands with the queries that need it, and the public surface is regenerated. One coherent change: the gate becomes real, in the UI and in the logic at once.

  **Before hiding anything, re-evaluate.** If the shadow window moved any constant, signals that qualify under the new numbers are still `NULL`, because promotion is only evaluated when a new score arrives. PR2 runs a one-time pass over unpromoted signals with the final constants first.
...
- [ ] **P2-1**: One-time re-evaluation pass over unpromoted signals with the final constants, before anything is hidden. Same pass promotes any unpromoted signal that has an active evaluation (D16).
- [ ] **P2-2**: Default-deny in `SignalRepository` (D15) with an `includeUnpromoted` opt-in for `hybridSearch`, `findByIdForUpdate`, and the consolidation search; port docs state which side each method sits on.
- [ ] **P2-3**: Verify every surface in §4.3 with a test per surface that a candidate is absent — `list-signals` (items, counts, histogram), signal detail / `getSignal`, `searchOrgWide`, `get-related-signals`, `list-session-signals`, `build-signals-export`.
- [ ] **P2-4**: The `promoted_at` index supporting those reads.
- [ ] **P2-5**: Move the notification and dispatch publishes from `SignalCreated` to `SignalPromoted` (replacing PR1's inert handler), passing `discoveredAt: promotedAt`.
- [ ] **P2-6**: `check-signal-escalation` skips unpromoted signals so an incident cannot route around the gate. (`sweep-escalating-signals` needs no change: it iterates open incidents, which an unpromoted signal cannot have.)
- [ ] **P2-7**: `isSignalNew` anchors on `promotedAt` (D9).
- [ ] **P2-8**: Public surface — `@repo/operations` excludes candidates; regenerate `openapi.json`, `mcp.json`, both SDKs, and the CLI reference in the same PR.
- [ ] **P2-9**: Seeds gain candidates that must appear in no UI or API response; docs updated to the end-state model.
