...
**Where the generation runs, and why not at the obvious place.** "At promotion" is a moment inside a Postgres transaction, and an LLM call cannot go there — the same rule that pushed the volume lookup outside it. Stamping `promoted_at` inside the transaction is what makes the signal visible and what emits `SignalPromoted`, so generating afterwards would publish a signal, and fire `signal.discovered`, while the row still carries the raw-feedback placeholder. That is strictly worse than today's naming: the notification is the one surface where the title is load-bearing.

The generation therefore runs in the **`SignalPromoted` consumer, before it publishes the notification and the dispatch**. The alternative — predicting promotion outside the lock and pre-generating — was rejected: it puts an AI dependency back on the ingestion hot path that PR3 exists to remove, it needs an extra unlocked count, and `count + 1` over-predicts whenever the incoming score belongs to a session the signal already has, so it burns model calls on assignments that do not promote.

The accepted cost is a window, of roughly one worker hop, in which a freshly promoted signal is visible carrying its placeholder. It is bounded, it lands on a signal nobody has navigated to yet, and the surface where the name actually matters — the notification — is correct by construction because it is published downstream of the regeneration. Same handler covers the born-promoted path (§4.2), so the creation path needs no model call in any configuration.
...
- [ ] **P3-1**: Pure `buildCandidatePlaceholder(feedback)` → `{ name, description }`: truncated first sentence within `SIGNAL_NAME_MAX_LENGTH`, description is the collapsed feedback. Tested for empty feedback, no sentence terminator, a first sentence longer than the cap, and whitespace collapse. Extract the existing `collapseWhitespace` / `truncateSignalName` helpers out of `generate-signal-details.ts` rather than duplicating them.
- [ ] **P3-2**: `createSignalFromScoreUseCase` writes the placeholder instead of calling `generateSignalDetailsUseCase`. Drops `GenerateSignalDetailsError` from `CreateSignalFromScoreError` and the `AI` requirement from the use-case — then drop `withAi(AIGenerateLive)` and `withActivityAIMetering` from the `createSignalFromScore` activity. `assignOrCreateSignal` keeps both, since promotion-time generation lives downstream of it.
- [ ] **P3-3**: An explicit way to suppress the stabilization baseline in `generateSignalDetailsUseCase`. It currently derives `previousName`/`previousDescription` from the signal row whenever given a `signalId`, which at promotion would feed the placeholder back as its own baseline — the exact anchoring §4.6 removes. Test that specifically; it fails silently otherwise.
- [ ] **P3-4**: Generation in the `SignalPromoted` consumer, ahead of the notification and dispatch publishes (D17). Carries the billing shape the `issues:refresh` handler already uses (`authorizeBillableAction` with `skipIfBlocked`, then `makeAIMeteringScope`), and must not fail the announcement if generation fails — degrade to the placeholder and let the throttled refresh retry.
- [ ] **P3-5**: The born-promoted path (§4.2) routes through the same consumer, so no configuration puts a model call back on the creation path.
- [ ] **P3-6**: Seeds carry a placeholder-named candidate; docs updated (§ Denoising's naming paragraph and `dev-docs/signals.md` § Naming, which still says details are generated at creation and refreshed on debounce).
