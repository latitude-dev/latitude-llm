...
- [ ] **P4-1**: Extract the **gate** (count distinct sessions → compare to threshold → emit `SignalQualifiedForPromotion`) into one shared helper. Three call sites after this PR — creation, assignment, consolidation — is where duplicating it stops being acceptable.
