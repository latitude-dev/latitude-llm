...
- [x] **T-h** `reopenSignalOnOccurrenceUseCase`: the conditional-claim UPDATE (6.4) + `SignalRegressed` outbox emit on claim; tests (idempotency within a cycle, `resolved_at < score.created_at` guard, ignored never reopens). PR review addition: `run-live-evaluation`'s dedupe paths load the already-persisted score (`findByEvaluationIdAndTraceId`, replacing the boolean exists check) and re-run the claim, so a worker that dies between the score commit and the claim transaction converges on retry instead of leaving the signal resolved.
