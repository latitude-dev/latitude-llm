...
- an **origin** (immutable, set at creation): `user` (built deliberately by a person) or `system` (auto-generated by Latitude's discovery — see [Discovery](#discovery-sinks-and-promotion)). Origin is the durable record of *how the signal was created*, independent of whether it has an evaluation, and it does **not** change when a discovered signal is later promoted. It is what gates annotation assignment — see [Discovery](#discovery-sinks-and-promotion).
...
- an optional **`evaluation`** (an `evaluations` row linked 1:1 via `evaluations.signal_id`, **one active per signal**): the membership detector, run at write time. No linked evaluation = a **sink** — a *state*, not a kind: no write-time detection, membership from annotations only. Discovered signals start as sinks, but "sink" is **not** the same as "auto-generated": a promoted discovered signal has an evaluation (no longer a sink) yet stays `origin = 'system'`. Tell auto-generated from hand-built by `origin`, never by the presence of an evaluation.
...
- **Users cannot create an evaluation-less signal.** A `user`-origin signal must have an evaluation; only `system`-origin signals may be evaluation-less (a sink). Because `origin` is fixed at creation, a `system` signal that is later promoted keeps `origin = 'system'` — and stays annotation-assignable — even though it now has an evaluation. This deliberately keeps "plain filter slices" out of signals — those stay **saved searches**. (A broad `filters`-only signal would write a score per trace; banning user-created evaluation-less signals removes that footgun entirely.)
...
Automatic discovery is **kept**, reframed: it produces **auto-generated signals** (`origin = 'system'`, immutable) that start as sinks (no evaluation), not a separate entity. The flagger + annotation machinery is unchanged.
...
**Annotation assignment is allowed exactly on auto-generated (`origin = 'system'`) signals** — gated on `origin`, not on whether an evaluation exists. So a promoted discovered signal keeps accepting hand-assigned annotations (which become its judge's alignment ground truth), while a `user`-origin signal is detector-defined and never hand-assigned into. While annotating, the UI suggests existing auto-generated signals via hybrid search over `search_document` (lexical) + `centroid_embedding` (the existing discovery path); the user links explicitly or lets discovery route.
...
// mutable sink state (no active evaluation), and the gate for annotation assignment.
...
3. **Automatic discovery is kept** as auto-generated (`origin = 'system'`) signals. Annotation assignment is gated on `origin` (immutable), not on the sink state — so a promoted discovered signal keeps accepting annotations. → [Discovery](#discovery-sinks-and-promotion)
