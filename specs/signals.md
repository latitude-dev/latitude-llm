...
> Phases 1–6 above are shipped. **R1 (builder UI, #3773) is shipped and R2's describe→script generation is shipped; R2's GEPA-over-scripts, R3, and Cleanup are the remaining open work.** Status legend as above. Out of scope for this roadmap: moments→evaluations + flagger consolidation, and the pure-similarity batch-runner *optimization* (the similarity execution question itself lives in R3).
