...
#### PR2 — Evaluation membership convention (`membership_on_pass`) `[ ] next`
...
- [ ] **Discovery eligibility** (`check-eligibility` on the score → `discover-signal` path): for **evaluation** scores, "present" must use the owning evaluation's `membership_on_pass` — otherwise a non-present run of a `membership_on_pass = true` evaluation (`passed = false`) would wrongly look discovery-eligible. The **annotation** branch is unchanged (negative = `passed = false` seeds discovery). (Evaluation `signal_id` is assigned directly by the writer to the evaluation's own signal; clustering discovery is the annotation path.)
- [ ] **Alignment scoring** (`evaluate-draft-against-examples`, `evaluate-optimization-candidate`): `predictedPositive = membership_on_pass ? passed === true : passed === false`.
- [ ] **Present-verdict instruction + new-eval default**: the convention text lives in `baseline-prompt.ts` (today "set `passed = false` when the issue is present") — flip it to "set `passed = true` when the behavior is present". The **GEPA proposer** (`packages/platform/op-gepa/src/prompts/proposer.ts`) holds no convention string itself — it preserves the `Passed()`/`Failed()` wrapper and surfaces Expected/Predicted-present labels that are derived in `evaluate-optimization-candidate.ts` (the alignment-scoring site above); adjust those labels' wording if it references polarity. Every newly created/optimized evaluation (discovery-tracked judges and PR3's user-created ones) is written with `membership_on_pass = true`.
