...
>
> **Depends on** — `specs/sandbox-runtime.md`, the execution contract for evaluations: an evaluation run returns a normalized score (`value` ∈ [0,1]), a `passed` verdict (membership — the script decides; no host-side threshold), and optional `feedback`. Phases 0–1 of that spec are built; Phase 2 (rule/script codegen + dry-run harness) is the substrate this spec consumes.
>
...
//   - the script returns { value (score 0..1), passed (membership), feedback? } — no host-side threshold.
...
- **The CH `scores` table is the single signal counting/aggregate surface** monitors read; it loses its issue-trend special-casing. Occurrence count = `countDistinct(trace_id) WHERE signal_id = ? AND passed`. Metric aggregates (`avg`/`p95`/`sum` of trace `duration`/`cost`/`tokens`) join the matched `trace_id` back to the traces analytics — the score's own duration/tokens/cost are the *evaluation's* (zero for scripts that don't call `llm()`), not the trace's.
...
#### PR1 — Engine cutover `[x] shipped — merged to development (#3621)`
...
> **PR1 shipped the engine (merged #3621) — this is PR2's starting point.** A fresh PR2 agent can assume all of the following is live on `development`:
> - Every active evaluation runs through the **`signals:match`** worker (off `TracesIngested`, gated `!isSandbox`, debounced) → the existing `live-evaluations:execute` queue → `runLiveEvaluationUseCase`. Every run writes a `scores` row with `signal_id` set (matched or not); **occurrence = `passed = true`**. `trace-end` no longer fans out evaluations.
> - `scores.source` is renamed to **`source_type`** in Postgres + the domain `Score` discriminator. The **ClickHouse** column and the public **`/scores` wire key** stay `source` (mapped at the API boundary).
> - `evaluations.legacy_polarity` exists (pre-cutover judges are inverted at the execution boundary; self-draining as judges re-optimize).
> - `ScoreRepository.listBySignalId` takes an optional `passed` filter; every occurrence-reader (signal-name generation, incident excerpts, the web carousel, `listFlaggerSlugsBySignalId`) passes `passed: true`.
> - The four PR1 PG migrations were consolidated into one — `drizzle/20260619101126_signals-engine-cutover`. CH flip + hourly-bucket MV rebuild are goose `00038`/`00039`. Prod `scores` is tiny (~75k rows / 3.5 MiB), so all PR1 migrations ran instantly.
> - **`evaluations.alignment` / `aligned_at` are still NOT NULL.** PR2 relaxes them (judge-only) — which is *why* `script_hash` lives in PR2: the score writer needs a non-null hash source once `alignment` can be null.
>
> **What PR2 adds:** the user-facing surface to create/manage signals. Today there is **no create/update/delete signal API**, and an `evaluations` row cannot be authored standalone or from a declarative `settings` form — only via the discovery → tracking path. PR2 closes that, for **script** signals (raw script, or a `settings` form compiled to a script). Judge-authoring is Phase 3; the builder UI is PR3; default-monitor provisioning + alerting stay Phase 4.

#### PR2 — User-created signals (API + MCP + backfill) `[ ] pending`

- [ ] **Contracts** (`@domain/shared`): `SIGNAL_ORIGINS = ['user','system']` + `signalOriginSchema`; `evaluationSettingsSchema = discriminatedUnion('kind', [{ kind: 'judge', criteria }])`, forward-open for future kinds. `settings` is NULL for raw / GEPA-generated scripts.
- [ ] **Additive PG migration** (one Drizzle migration; mirror the `pg:generate` + `pg:generate:custom` workflow):
    - `signals`: `origin varchar(16)` (backfill existing → `'system'`, then NOT NULL), `filters jsonb null`, `deleted_at timestamptz null`; relax `centroid` / `clustered_at` to **nullable** (user signals have no centroid); partial-unique slug `WHERE deleted_at IS NULL`.
    - `evaluations`: `settings jsonb null`; relax `alignment` / `aligned_at` to **nullable** (judge-only); `script` stays NOT NULL; add **`script_hash`** filled for all rows (backfill existing from `alignment.evaluationHash`).
    - **Dedupe before the unique index:** historically there can be multiple active evaluations per signal — keep the most-recently-aligned active, archive the rest (dry-run a COUNT + audit-log first), **then** create the active-detector partial-unique `(signal_id) WHERE deleted_at IS NULL AND archived_at IS NULL` + a lookup btree.
    - **Gotcha:** the `issues_centroid_embedding_consistency_check` CHECK constraint survived Phase 1's table rename and is **untracked by Drizzle** — relaxing `centroid` nullability must drop/recreate it by hand (`pg:generate:custom`) or the snapshot drifts.
    - **Null-safety fallout:** `toCentroidEmbedding` (`signal-repository.ts`) and the API evaluation response mapper (`apps/api/src/openapi/entities/evaluation.ts` — `alignment`/`alignedAt`) must now tolerate null.
- [ ] **`script_hash` writer wiring:** the score's `metadata.evaluationHash` reads `evaluations.script_hash` (not the now-nullable `alignment.evaluationHash`) — in `execute-live-evaluation` / `run-live-evaluation`.
- [ ] **Settings → script codegen** (`@domain/sandbox`, **NET-NEW — there is no existing codegen** despite earlier spec drafts claiming otherwise): `compileSettingsToScript(EvaluationSettings) → { source }`. The `judge` kind must single-source the existing `wrapPromptAsEvaluationScript` body so the capability regex (`llm(`) and the new-polarity `result.passed ? Passed() : Failed()` framing hold. Compile-on-save validation → `ScriptCompileError` (HTTP 422); ideally a fixture dry-run too (compile is syntax-only).
- [ ] **Signal-level `filters` pre-gate:** layer the signal's `filters` onto the matcher in `signals:match`, on top of the evaluation's existing `trigger.filter`.
- [ ] **CRUD use-cases** (`@domain/signals`):
    - `createSignalUseCase` — **reject evaluation-less create for `origin=user`** (only `system` signals may have no evaluation); raw-script path compiles via `ScriptRuntime.compile`; judge path defers to the existing `optimize-evaluation` workflow; sets `legacy_polarity = false` + `script_hash`; enqueues `signals:backfill` post-commit for deterministic scripts.
    - `updateSignalUseCase` — name / description / `filters`; `origin` is immutable.
    - `deleteSignalUseCase` — **PG soft-delete** (`deleted_at`) + archive the linked evaluation (auto write-stop via the active-detector scan in `signals:match`) + close open incidents. **No CH cleanup** (deleted-signal scores linger, excluded read-side via the PG lifecycle); **no Redis detector cache.**
- [ ] **API + MCP** (`apps/api/src/routes/signals.ts`, modeled on `monitors.ts`): `createSignal` / `updateSignal` / `deleteSignal` with rich `.describe()` body schemas (`openapi/entities/signal.ts`); thread `origin`; run `pnpm generate:sdk` to regen `openapi.json` / `mcp.json` / Fern SDKs. Confirm the 307 `/issues`→`/signals` redirect forwards POST bodies (test).
- [ ] **`signals:backfill`** worker + `backfillSignalScoresUseCase`: **deterministic scripts only** (`hasLlmCapability` gate — judges collect forward; backfilling an llm judge is a costed op); paginated via `TraceRepository.listByProjectId`; `windowStartIso` resolved once at enqueue (stable across chained batches); **sandbox traces excluded**; writes through the same execute/writer path; idempotent per `(evaluation, trace)`.

**Settled decisions PR2 must respect (do not relitigate):** `passed = true` = occurrence for every source; every evaluation score carries `signal_id`; occurrence reads gate `passed = true` counted as distinct `trace_id`; PG `source_type` / CH + wire `source`; `deleteSignal` is soft-delete with no CH cleanup; **no per-project signal cap**; custom-source `/scores` push (the public API accepting `signal_id`) stays POST-MVP.

**PR2 verification:** `apps/api` route tests (PGlite `setupTestApi`) — `createSignal` 201 (raw script) / 400 (evaluation-less `origin=user`) / 422 (compile error); `updateSignal` re-slugs; `deleteSignal` soft-deletes + archives the evaluation + the signal disappears from reads; backfill enqueued only for deterministic scripts (spy the queue publisher). Regen check: `openapi.json` / `mcp.json` gain the three ops under the `signals` group and the wire field stays `source`. Backfill: a deterministic signal over historical traces produces idempotent PG + CH scores, sandbox traces excluded, the window stable across chained batches; an llm judge is skipped.
