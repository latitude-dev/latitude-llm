...
- **The script returns a score and optional reasoning; the host derives the verdict.** Each run yields `value` (a normalized score ∈ [0,1], for sort/confidence/display) and optional `feedback`; the host derives the run's verdict by thresholding `value` (`isScoreMatch` against `DEFAULT_SCRIPT_SCORE_THRESHOLD` = 0.5). Membership is recorded as `signal_id`, not `passed` (see [Score](#score--the-membership-ledger)); the writer stamps the evaluation's signal when the verdict is *present*, and which verdict counts as present is per-evaluation (`evaluations.membership_on_pass`). Definition edits (script or settings) apply **forward only** — a definition-changed marker appears on charts, and existing scores are never re-evaluated.
...
- **`passed` is source-specific and does not define membership.** For an `annotation` it is human sentiment (thumbs-up = `passed = true`, thumbs-down = `passed = false`); for `custom` it is caller-defined. For an `evaluation` it is the detector's verdict — host-derived by thresholding the script's `value` (`isScoreMatch`) — and *which verdict means the signal's behavior is present* is per-evaluation, recorded by **`evaluations.membership_on_pass`**: `true` (every evaluation created under the current convention) ⇒ present when `passed = true`; `false` (deprecated pre-existing evaluations) ⇒ present when `passed = false`. `signal_id` is assigned independently of the `passed` value: the writer stamps the evaluation's own signal when its verdict is *present*, and discovery assigns a signal to negative (`passed = false`) annotations. `passed` is **never globally flipped** — only the per-evaluation `membership_on_pass` differs, so existing scores need no migration.
...
- **`signal_id` IS the materialized membership flag; `passed` is the source-specific verdict/sentiment** (see [Score](#score--the-membership-ledger)), never membership. For evaluations the host derives `passed` at write time by thresholding the script's `value` (`isScoreMatch` against `DEFAULT_SCRIPT_SCORE_THRESHOLD` = 0.5; `Passed()`/`Failed()` set `value` = 1/0), and the writer stamps `signal_id` when the verdict is *present* — present being `passed = true` or `passed = false` per the evaluation's `membership_on_pass` (current evaluations: `true`; deprecated: `false`). So **membership reads need no runtime threshold**: a signal's occurrences are the distinct traces in `scores WHERE signal_id = ?`. Definition edits apply forward-only — old rows keep their frozen `value`/`passed`.
...
evaluations
  ...
  membership_on_pass boolean NOT NULL default false  ▸NEW  -- present-verdict polarity: true = signal_id stamped when passed=true (current evals); false = when passed=false (deprecated). Additive backfill = false; NO scores migration.
...
10. **Membership polarity is per-evaluation, never global.** Membership is `signal_id`; which verdict counts as the behavior being *present* (and so stamps `signal_id`) is recorded per evaluation by `evaluations.membership_on_pass`. Current evaluations use present ⇒ `passed = true`; deprecated pre-existing ones use present ⇒ `passed = false`. `passed` is never flipped in storage, so no scores migration is needed — only the per-evaluation flag plus four convention-aware sites: three that **read** it at runtime (writer, discovery eligibility, alignment scoring) and the authoring path that **emits** the matching convention and sets the flag (GEPA/baseline prompt + create/optimize). → [Evaluation](#evaluation-the-detector) / [Score](#score--the-membership-ledger) / [Phase 2 PR2](#phase-2--evaluation-substrate--script-evaluations-mvp)
...
#### PR2 — Evaluation membership convention (`membership_on_pass`) `[~] in progress`

PR1 hard-codes one present-verdict convention (present ⇒ `passed = false`) for every evaluation. This PR makes that polarity **per-evaluation** so new evaluations use the intuitive `passed = true` = present while the existing (deprecated) ones keep `passed = false` — with **no `scores` data migration** (only an additive `evaluations` column, the three runtime sites that read it — writer, discovery eligibility, alignment scoring — and the authoring path that emits the matching convention and sets the flag). Membership stays `signal_id`; annotations and `custom` are untouched, so this cannot reach annotation sentiment.

- [ ] **`evaluations.membership_on_pass`** (`boolean NOT NULL default false`): additive PG migration. Existing evaluations backfill to `false` (scripts/scores stay present ⇒ `passed = false`); **no `passed` re-flip**.
- [ ] **Writer** (`run-live-evaluation`): stamp `signal_id` when the verdict is present, `present = membership_on_pass ? passed === true : passed === false` (today it is hard-coded `passed === false`). Absent runs still write a score with `signal_id` null.
- [ ] **Discovery eligibility** (`check-eligibility` on the score → `discover-signal` path): for **evaluation** scores, "present" must use the owning evaluation's `membership_on_pass` — otherwise a non-present run of a `membership_on_pass = true` evaluation (`passed = false`) would wrongly look discovery-eligible and `resolveLinkedSignalId` would assign that absent run to the signal. The **annotation** branch is unchanged (negative = `passed = false` seeds discovery). (Evaluation `signal_id` is assigned directly by the writer to the evaluation's own signal; clustering discovery is the annotation path.) Also drop the now-polarity-specific `passed = false` predicate from `scores_signal_discovery_work_idx` → `WHERE drafted_at IS NULL AND errored = false AND signal_id IS NULL` (polarity-agnostic; no query consumes the index by name).
- [ ] **Alignment scoring** (`evaluate-draft-against-examples`, `evaluate-optimization-candidate`): `predictedPositive = membership_on_pass ? passed === true : passed === false`. Thread the flag with the polarity of the *script being scored*: the **optimize** workflow regenerates a fresh (new-polarity) baseline → threads `true`; the **refresh** workflow (`refresh-evaluation-alignment-workflow`) re-scores the *persisted* script unchanged → threads the evaluation's **stored** `membership_on_pass`. Carry it on `LoadedEvaluationAlignmentState` (populated by `load-alignment-state-or-inactive`).
- [ ] **Present-verdict instruction + new-eval default**: the convention text lives in `baseline-prompt.ts` (today "set `passed = false` when the issue is present") — flip it **unconditionally** to "set `passed = true` when the behavior is present" (generation-only; sole caller `generate-baseline-draft`). The **GEPA proposer** (`packages/platform/op-gepa/src/prompts/proposer.ts`) holds no convention string today (it inherited polarity from the seed baseline + the alignment reward) — add an **explicit polarity rule** to `GEPA_PROPOSER_SYSTEM_PROMPT` ("instruct the judge to set `passed = true` when the behavior is present, `false` when absent") as drift insurance; hardcoded new-polarity is correct because GEPA only runs in the optimize workflow. `persist-alignment-result` sets `membership_on_pass` from an explicit param — **never hardcoded**: the **optimize** path persists `true` (fresh new-polarity script — this is how discovery-tracked judges and PR3's user-created ones become `true`, and how realigning an old eval upgrades it), while the **refresh** path preserves the evaluation's stored flag (it re-persists the unchanged script).
- [ ] **No changes** to occurrence reads, the `scores_hourly_buckets` MV, or annotation/`custom` write paths — membership is `signal_id`, unaffected by the per-evaluation verdict polarity.

**Exit gate (PR2):** a new evaluation authored as "`Passed` = present" assigns `signal_id` on `passed = true`; a deprecated (`membership_on_pass = false`) evaluation still assigns on `passed = false`; occurrence counts (by `signal_id`) are identical for both; annotations and their sentiment are unchanged; no `scores` migration ran.
...
- [ ] **P3-d** Execution linkage: verify the `EvaluationTrigger` path writes `signal_id`-bearing scores for a user-created (non-discovered) signal, created under the current convention (`membership_on_pass = true`, so the writer stamps `signal_id` when `passed = true`).
