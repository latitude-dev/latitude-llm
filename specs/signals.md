...
- **The script returns a score and optional reasoning; the host derives the verdict.** Each run yields `value` (a normalized score ∈ [0,1], for sort/confidence/display) and optional `feedback`; the host derives the run's verdict by thresholding `value` (`isScoreMatch` against `DEFAULT_SCRIPT_SCORE_THRESHOLD` = 0.5). Membership is recorded as `signal_id`, not `passed` (see [Score](#score--the-membership-ledger)). Definition edits (script or settings) apply **forward only** — a definition-changed marker appears on charts, and existing scores are never re-evaluated.
...
- **`passed` is source-specific and does not define membership.** For an `evaluation` it is the detector's verdict — host-derived by thresholding the script's `value` (`isScoreMatch`), with the signal's behavior *present* when `passed = false`; for an `annotation` it is human sentiment (thumbs-up = `passed = true`, thumbs-down = `passed = false`); for `custom` it is caller-defined. `signal_id` is assigned independently of what `passed` means: an evaluation stamps its own signal when the behavior is present, and discovery assigns a signal to negative (`passed = false`) annotations.
...
- **`signal_id` IS the materialized membership flag; `passed` is the source-specific verdict/sentiment** (see [Score](#score--the-membership-ledger)), never membership. For evaluations the host derives `passed` at write time by thresholding the script's `value` (`isScoreMatch` against `DEFAULT_SCRIPT_SCORE_THRESHOLD` = 0.5; `Passed()`/`Failed()` set `value` = 1/0), and the writer stamps `signal_id` when the behavior is present. So **membership reads need no runtime threshold**: a signal's occurrences are the distinct traces in `scores WHERE signal_id = ?`. Definition edits apply forward-only — old rows keep their frozen `value`/`passed`.
...
**Membership is `signal_id`; `passed` is the source-specific verdict/sentiment.** A trace is a member of a signal when it carries that signal's `signal_id`; occurrence reads are `scores WHERE signal_id = ?` and never gate on `passed`. `passed` means different things per source — for an evaluation it is the detector's verdict (host-derived from the script's `value` via `isScoreMatch`; behavior *present* when `passed = false`); for an annotation it is human sentiment (thumbs-up = `passed = true`, thumbs-down = `passed = false`). An evaluation stamps its own signal's `signal_id` when the behavior is present (absent runs are still written, with `signal_id` null); annotations get a `signal_id` from discovery, which triggers on negative (`passed = false`) annotations.

#### PR1 — Engine cutover `[x] complete`

- [x] **Sandbox `value` contract**: the script returns `value` (+ optional `feedback`) and does **not** return a `passed` field; the host derives the run's verdict by thresholding `value` via `isScoreMatch` against `DEFAULT_SCRIPT_SCORE_THRESHOLD` = 0.5 (`Passed()`/`Failed()` set `value` = 1/0).
- [x] **Verdict convention**: the baseline judge prompt (`baseline-prompt.ts`), the GEPA proposer, and alignment/optimization scoring use the problem-detector convention — the evaluation's behavior is *present* when `passed = false`. Discovery eligibility (`check-eligibility`) and the alignment-examples positive/negative selection key off the verdict for evaluations and off sentiment for annotations (negative = `passed = false`). The annotation sentiment write path (thumbs-up = `passed = true`) is independent.
...
- [x] **Read-side membership**: per-signal occurrence/trace reads use "`signal_id` present"; the `scores_signal_discovery_work_idx` predicate gates `passed = false` (the present/negative verdict) for discovery candidates.
- [x] **Data convention**: historical `evaluation` + `annotation` scores follow the problem-detector convention (behavior present ⇒ `passed = false`); the `scores_hourly_buckets` MV counts `signal_id`-bearing scores (no `passed` filter).
...
**Exit gate (PR1):** existing evaluations run through `signals:match`; membership is `signal_id` (occurrence reads count the distinct-trace subset carrying `signal_id`); the host derives each run's verdict via `isScoreMatch`; discovery + monitors still work; whole-workspace typecheck green. **Exit gate (PR2/PR3):** users create/manage script & judge signals via API/MCP/UI; new in-scope traces produce evaluation scores visible on the signal page; backfill works.
