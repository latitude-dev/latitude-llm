...
- **`@slack/oauth` adoption**: we use the official SDK for the OAuth machinery starting in Phase 2 (web callback handler) and Phase 3 (token resolution + refresh-on-use in the notifications worker). Phase 1 ships without the SDK on the import path because the CLI's flow (build URL → exchange code once → persist) doesn't have a consumer for the SDK's value-adds (`InstallationStore`, `authorize()`, refresh handling). The schema, repository, encryption pattern, and HMAC signature verifier all stay ours — the SDK plugs into them via a thin `LatitudeInstallationStore` adapter in Phase 2.
