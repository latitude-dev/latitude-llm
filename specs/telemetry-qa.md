...
| 54 | OpenClaw plugin | `packages/telemetry/openclaw` | OpenClaw plugin → streams agent runs as OTLP traces | ✅ | ✅ | **DONE — v0.1.0 rewrite, dogfooded live on OpenClaw 2026.9.2 (project `dummy`, Slack + web chat, 2026-09-09), user-approved after a final visual pass.** 0.0.9 emitted **nothing** on OpenClaw ≥ 2026.8.1 (`before_agent_start` removed upstream) and, once the operation gate landed (00040), would have carried no tokens/cost/conversation/tools anyway. Round 1 exposed the **Codex harness** (ChatGPT OAuth, and `openai/*` on the official endpoint even with an API key): empty history, per-turn transcript, no `model_call_*` hooks, usage only on the final message, memory writes via `apply_patch`, native subagents invisible. Rounds 2–3 validated: session history rebuilt from seen turns + cold start from the agent SQLite store (`history=sqlite:82 → memory:89`), `apply_patch` → `upsert_memory` (Memory page shows `openclaw/main`), stream-derived TTFT, Slack `user.id`, cron tag + name, envelope prompt dedupe. Round 3B on the **embedded runner** (`models.providers.openai.agentRuntime={"id":"openclaw"}`): per-call usage + OpenClaw cost, TTFB, response ids, reasoning parts, 41 tool definitions, `tool_call:sessions_spawn → subagent → interaction` nesting in one trace/session, standalone `compaction` trace. Ingest contract test: `packages/domain/spans/src/otlp/tests/openclaw-plugin.test.ts`. Design: `dev-docs/openclaw-telemetry.md`; public: `docs/telemetry/openclaw.md` (now recommends the plugin over the native exporter, which still scrubs session ids). Round 4 added sender names from the channel envelope, the injected runtime-context message as `system`, and a standalone compaction trace whose root is named `compaction`. Merge = publish both packages to npm. |
| 55 | OpenClaw CLI installer | `packages/telemetry/openclaw-cli` | One-shot installer for the OpenClaw plugin | ✅ | ✅ | v0.1.0: passes `--accept-capabilities` (required since OpenClaw 2026.8; 0.0.9 aborted with `requires capability consent`), min version 2026.8.1, `--runtime-spec=npm-pack:<tgz>` for pre-release builds. Verified install path manually on the VM (config sets, validate, restart). |
...
| 59 | Hermes telemetry | `packages/telemetry/hermes` | **pip** plugin for Nous Research's **Hermes Agent** harness → streams sessions as OTLP traces (user prompts, model turns, tool calls/results, tool definitions, memory operations, subagents, token usage, cost, real system prompt). Same family as claude-code/pi/openclaw. | ✅ | ✅ | **DONE — v0.2.0, dogfooded live on a real Hermes 0.20.5 VM over two rounds (project `alescript`).** Install into the **same** venv Hermes runs (`~/.hermes/hermes-agent/venv`); enable via `~/.hermes/config.yaml` (**not** `hermes plugins enable` — hermes-agent#23802 reports pip plugins as "not installed" to the CLI). **v0.1.2 was losing most content:** the Codex/Responses dialect sends Responses *items* whose tool/reasoning entries carry no `role`, so every tool call and result was dropped from the conversation and assistant text shipped as a JSON blob; no system instructions, no tool definitions, no memory, no TTFT, tool errors recorded as successes, retries and interruptions both `abandoned`. **Round 1 (session `20260825_145401_94f562`)** verified the rewrite live (80-message conversation, `system` first, `tool_call`/`tool_call_response` throughout, zero `abandoned` spans, interrupted turn reading as cancelled, memory read+write, planted `sk-live-…` masked, every POST 200 at 394 KB peak) and surfaced 6 emitter defects. **Round 2 (session `20260825_173058_6e9867` + subagent `20260825_173407_36f028`)** confirmed all of them fixed: `interaction.kind=user` on ordinary turns, only `aux:approval`/`aux:title_generation` (no phantom `aux:main`), aux spans priced and instantaneous, 40 tool definitions on the session's *first* `llm_request` with `source: snapshot`, no `subagent` platform tag, bare version tag, route metadata present, `hermes.thread.name` masked by attribute redaction, and **TTFT on 51/53 spans** once Hermes's `plugins.stream_reasoning_deltas: true` is set. **Tokens reconcile to Hermes's ledger exactly** (129,250 main + 23,569 background_review + 238 title + 782 approval + 34,694 subagent = 188,533). **Waived:** F6's retry path (needs a real rate limit) and `aux:compression` (only automatic context-pressure compaction routes through `auxiliary_client`; `/compress` does not) — both covered by unit tests, unverified only in production. 123 unit tests. Design: `dev-docs/hermes-telemetry.md`; public: `docs/telemetry/hermes.md`. |
| 61 | Prime Intellect (Verifiers) telemetry | `packages/telemetry/prime-intellect` | **pip** library/CLI for Prime Intellect **Verifiers** eval rollouts → OTLP traces + optional custom scores from rewards/metrics. Same family as hermes/claude-code/pi. | ✅ | ⬜ | **SMOKE DONE (local).** Python pkg `latitude-telemetry-prime-intellect` v0.1.0. Fixture export via `export_results_dir` → ingest `:3002` + scores `:3001`. Session/run `qa-eval-run-3`, trace `0501cc8398f04907ba1fadb5144f2be4`. **✅ spans in CH:** `interaction` + 2× `llm_request` + `tool_call:reverse`, `service_name=prime-intellect`, model/tokens, systemInstructions + messages. **✅ score:** `verifiers.reward.exact_match` value=1. **🐛 FIX:** scores raced ahead of ClickHouse ingest (API 404); flush OTLP before score posts + retry 404s. Unit tests 10/10. Docs: `docs/telemetry/prime-intellect.md`. |
| 60 | ElevenLabs Agents | `docs/telemetry/frameworks/elevenlabs.mdx` (docs-only) | Hosted voice-agent platform; observed via ElevenLabs **Custom LLM** → point the agent at a self-run OpenAI-compatible proxy instrumented with Latitude. | ⚠️ | ✅ | **DONE (review-only).** Not a Latitude package — the actual telemetry is the **OpenAI instrumentation** (#1) running on your proxy, so span parsing is already covered. Docs `elevenlabs.mdx` reviewed = good + nav-registered. **Icon:** custom `elevenlabs.tsx` (theme-aware bars). **In-app instructions ADDED:** TS+Py, custom-LLM proxy snippets (express / fastapi) with the OpenAI instrumentation subpath factory, packages `openai @traceloop/instrumentation-openai express` / `openai fastapi uvicorn`. |
...
| 2026-07-22 08:38 | Prime Intellect / Verifiers (#61) local smoke | `qa-eval-run-3` | `prime-intellect, verifiers` | `0501cc8398f04907ba1fadb5144f2be4` | ✅ smoke — interaction + llm_request×2 + tool_call:reverse in CH; score `verifiers.reward.exact_match=1`; fixed score/ingest race |
