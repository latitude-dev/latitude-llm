...
| 61 | Prime Intellect (Verifiers) telemetry | `packages/telemetry/prime-intellect` | **pip** library/CLI for Prime Intellect **Verifiers** eval rollouts → OTLP traces + optional custom scores from rewards/metrics. Same family as hermes/claude-code/pi. | ⚠️ | ⬜ | Python pkg `latitude-telemetry-prime-intellect` v0.1.0. No host plugin entry point — call `export_episodes` / `make_on_complete` from eval scripts, or `latitude-prime-intellect-export export <results_dir>` post-hoc. Docs: `docs/telemetry/prime-intellect.md`. Has `tests/test_mapper.py` + `tests/test_export.py`. |
