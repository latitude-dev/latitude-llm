...
/**
 * Soft DoS sniff for catastrophic backtracking patterns. Doesn't reject —
 * surfaces in the iteration table's Notes column to nudge attention.
 *
 * v1: simple text-level heuristic for nested unbounded quantifiers like
 * `(a+)+`, `(.*)*`, `(.+)*`. Misses some pathological patterns but catches
 * the common ones cheaply.
 */
