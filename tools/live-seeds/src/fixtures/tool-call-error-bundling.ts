...
export const toolCallErrorBundlingFixture: LiveSeedFixtureDefinition = {
  ...
  generateCase: ({ fixtureKey, rng }) => {
    ...
    const answerOffsetMs =
      planningSpan.durationMs + toolSpan.durationMs + (recovered ? 1_400 : 0) + (recovered ? 200 : 0)
    const answerSpan = {
      ...createChatSpan(rng, {
        label: "tool-recovery",
        ...
        outputMessages: [
          assistantTextMessage(
            recovered
              ...
              : `I couldn't finish the routing check for ${orderId} because the pickup policy service is unavailable.`,
          ),
        ],
